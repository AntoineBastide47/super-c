...
`sh ci/gate.sh` runs it (and, with generation two, the one-worker versus every-core
identity check under each `SC_TASK_DELAY` seed of `ci/contract.sh`: emitted tree, type
