...
/// Wait until the idle task-block pool retains at most `bytes`: every worker has parked once since its
/// last task (its stash drained into the shared pool) and the pool released what is past its budget.
