...
static void test_arithmetic(void) {
  sc_run_program("precedence", PRE "fn main() i32 { unsafe exit(1 + 2 * 3 - 4 / 2); }\n", 5, "");
  sc_run_program("mixed precedence", PRE "fn main() i32 { unsafe exit(17 % 5 + 100 / 7 + 6 & 3); }\n", 2, "");
  sc_run_program(
      "bitwise",
      PRE "fn main() i32 { let a: i32 = 6 & 3; let b: i32 = 6 | 1; let c: i32 = 1 << 4; unsafe exit(a + b + c); }\n", 25, "");
  sc_run_program("unary neg", PRE "fn main() i32 { let y: i32 = -5; unsafe exit(0 - y); }\n", 5, "");
  sc_run_program("unary not", PRE "fn main() i32 { unsafe exit(switch !false { true => 7, _ => 0, }); }\n", 7, "");
}

static void test_control_flow(void) {
  sc_run_program(
      "while break",
      PRE "fn main() i32 { let mut i: i32 = 0; while true { if i >= 5 { break; } i = i + 1; } unsafe exit(i); }\n", 5, "");
  sc_run_program(
      "for continue",
      PRE "fn main() i32 { let mut t: i32 = 0; for i in 0..5 { if i == 2 { continue; } t = t + i; } unsafe exit(t); }\n", 8,
      "");
  sc_run_program(
      "nested for",
      PRE "fn main() i32 { let mut t: i32 = 0; for i in 0..3 { for j in 0..3 { t = t + 1; } } unsafe exit(t); }\n", 9, "");
  sc_run_program( // body runs once before the test: sum 0+1+2+3+4 = 10
      "do while",
      PRE "fn main() i32 { let mut i: i32 = 0; let mut s: i32 = 0; do { s = s + i; i = i + 1; } while i < 5; unsafe exit(s); }\n",
      10, "");
  sc_run_program( // do-while body always runs at least once even when the condition is false up front
      "do while runs once",
      PRE "fn main() i32 { let mut n: i32 = 0; do { n = n + 7; } while false; unsafe exit(n); }\n", 7, "");
}
...
static void test_enums(void) {
  ...
  sc_run_program(
      ...
      PRE "enum Color { Red, Green, Blue, }\n"
          ...
          "fn main() i32 { unsafe exit(code(Color::Blue)); }\n",
      3, "");
  ...
  sc_run_program(
      ...
      PRE "enum Shape { Dot, Circle(i32), Rect(i32, i32), }\n"
          ...
          "fn main() i32 { unsafe exit(area(Shape::Circle(5)) + area(Shape::Rect(3, 4))); }\n",
      37, ""); // 25 + 12
  ...
  sc_run_program(
      ...
      PRE "enum Opt { None, Some(i32), }\n"
          ...
          "fn main() i32 { let a: Opt = Opt::None; let b: Opt = Opt::Some(9); unsafe exit(unwrap(a) + unwrap(b)); }\n",
      9, "");
  // explicit discriminants on a plain enum, read back via an enum->int cast.
  sc_run_program(
      "explicit discriminants",
      PRE "enum Code { Ok = 0, NotFound = 404, }\nfn main() i32 { unsafe exit(Code::NotFound as i32 - 397); }\n", 7, "");
}
...
static void test_if_expression(void) {
  ...
  sc_run_program(
      ...
      PRE "fn classify(n: i32) i32 {\n"
          ...
          "}\n"
          "fn main() i32 { unsafe exit(classify(8)); }\n",
      57, ""); // n=8 -> 50 + 7
  ...
  sc_run_program(
      ...
      PRE "fn pick(b: bool) i32 { return if b { 9; } else { 4; }; }\n"
          "fn main() i32 { unsafe exit(pick(true) + pick(false)); }\n",
      13, "");
}
...
static void test_tuple_destructure(void) {
  ...
  sc_run_program(
      ...
      PRE "fn pair<T>(x: T) (T, T) { return x, x; }\n"
          ...
          "extend<T> Two<T> {\n"
          "  pub fn both(self: &Two<T>) (T, T) { return unsafe self.v, unsafe self.v; }\n"
          "  pub fn with<U>(self: &Two<T>, u: U) (T, U) { return unsafe self.v, u; }\n"
          "}\n"
  ...
  sc_run_program(
      ...
      PRE "fn divmod(a: i32, b: i32) (i32, i32) { return a / b, a % b; }\n"
          "fn main() i32 { let mut (a, b) = divmod(100, 7); a = a + 1; unsafe exit(a + b); }\n",
      17, ""); // (14+1) + 2
}
...
static void test_slices(void) {
  ...
  sc_run_program(
      ...
      PRE "fn sum(s: []i32) i32 { let mut t = 0; for x in s { t = t + x; } return t; }\n"
          "fn main() i32 { let a: [i32; 4] = [1, 2, 3, 4]; unsafe exit(sum(a)); }\n",
      10, NULL);
  ...
  sc_run_program(
      ...
      PRE "fn f(s: []i32) i32 { return (s.len() as i32) + s.get(1) + s.first() + s.last(); }\n"
          "fn main() i32 { unsafe exit(f([10, 20, 30])); }\n", // 3 + 20 + 10 + 30
      63, NULL);
  ...
  sc_run_program(
      ...
      PRE "fn dbl(s: []mut i32) { let mut i: usize = 0; while i < s.len { s[i] = s[i] * 2; i = i + 1; } }\n"
          "fn main() i32 { let mut a: [i32; 3] = [3, 4, 5]; dbl(a); unsafe exit(a[0] + a[1] + a[2]); }\n", // (6+8+10)
      24, NULL);
}
...
static void test_range_slicing(void) {
  sc_run_program("range half-open a[1..4]",
                 PRE RSUM "fn main() i32 { let a = [10,20,30,40,50]; unsafe exit(sum(a[1..4])); }\n", 90, NULL);
  sc_run_program("range open-start a[..2]",
                 PRE RSUM "fn main() i32 { let a = [10,20,30,40,50]; unsafe exit(sum(a[..2])); }\n", 30, NULL);
  sc_run_program("range open-end (inferred len) a[3..]",
                 PRE RSUM "fn main() i32 { let a = [10,20,30,40,50]; unsafe exit(sum(a[3..])); }\n", 90, NULL);
  sc_run_program("range open-end (annotated len) a[2..]",
                 PRE RSUM "fn main() i32 { let a: [i32; 5] = [10,20,30,40,50]; unsafe exit(sum(a[2..])); }\n", 120, NULL);
  sc_run_program("range inclusive a[0..=2]",
                 PRE RSUM "fn main() i32 { let a = [10,20,30,40,50]; unsafe exit(sum(a[0..=2])); }\n", 60, NULL);
  sc_run_program("slice of a slice mid[1..3]",
                 PRE RSUM "fn main() i32 { let a = [10,20,30,40,50]; let mid = a[1..4]; unsafe exit(sum(mid[1..3])); }\n",
                 70, NULL);
}
...
static void test_range_value(void) {
  sc_run_program("range value: fields", PRE "fn main() i32 { let r = 1..5; unsafe exit(r.start + r.end); }\n", 6, NULL);
  sc_run_program("range value: iterate half-open",
                 PRE "fn main() i32 { let r = 1..5; let mut s = 0; for x in r { s = s + x; } unsafe exit(s); }\n", 10,
                 NULL); // 1+2+3+4
  sc_run_program("range value: iterate inclusive",
                 PRE "fn main() i32 { let r = 0..=4; let mut s = 0; for x in r { s = s + x; } unsafe exit(s); }\n", 10,
                 NULL); // 0+1+2+3+4
  sc_run_program("range value: passed to a fn",
                 PRE "fn span(r: Range<i32>) i32 { return r.end - r.start; }\n"
                     "fn main() i32 { let r = 3..10; unsafe exit(span(r)); }\n",
                 7, NULL);
  sc_run_program("literal range for-loop still counts (no Range value built)",
                 PRE "fn main() i32 { let mut s = 0; for i in 0..=5 { s = s + i; } unsafe exit(s); }\n", 15, NULL);
}
...
static void test_opaque_extern(void) {
  sc_run_program(
      ...
      "fn main() i32 { let t: clock_t = unsafe clock(); pass(t); return 7; }\n",
      7, NULL);
}
...
static void test_variadics(void) {
  ...
  sc_run_program(
      ...
      "  va_start(ap, fmt); unsafe vsnprintf(&mut buf[0], 64, fmt, ap); va_end(ap);\n"
      "  unsafe puts(&buf[0]);\n"
      "}\n"
...
static void test_complex(void) {
  sc_run_program(
      ...
      PRE "extern \"C\" { fn csqrt(z: c64) c64; fn creal(z: c64) f64; fn cimag(z: c64) f64; fn cabs(z: c64) f64; }\n"
          "fn main() i32 { let i: c64 = unsafe csqrt(-1.0); let z: c64 = 3.0 + 4.0 * i;\n"
          "  unsafe exit((creal(z) as i32) + (cimag(z) as i32) + (cabs(z) as i32)); }\n", // 3 + 4 + 5
      12, NULL);
}
...
static void test_atomics(void) {
  sc_run_program(
      ...
      PRE "extern \"C\" {\n"
          ...
          "fn main() i32 { let mut x: i32 = 0; unsafe __sc_atomic_store_i32(&mut x, 40);\n"
          "  let p: i32 = unsafe __sc_atomic_add_i32(&mut x, 2); unsafe __sc_atomic_fence();\n"
          "  let ok: bool = unsafe __sc_atomic_cas_i32(&mut x, 42, 6);\n"
          "  unsafe exit(p + unsafe __sc_atomic_load_i32(&x) + (ok as i32)); }\n", // 40 + 6 + 1
      47, NULL);
}
...
static void test_array_literals(void) {
  ...
  sc_run_program(
      ...
      PRE "fn third(a: [i32; 3]) i32 { return a[2]; }\n"
          "fn main() i32 { unsafe exit(third([7, 8, 9])); }\n",
      9, "");
  ...
  sc_run_program(
      ...
      PRE "fn main() i32 {\n"
          ...
          "  let k: [i32; 128] = [['a'] = 5];\n"
          "  let m: [i32; 4] = [1, [3] = 9];\n"
          "  unsafe exit(t[1] + t[3] + t[7] + k['a'] + m[0] + m[3]);\n" // 10+30+70+5+1+9 = 125
          "}\n",
      125, "");
  ...
  sc_run_program(
      ...
      PRE "fn lut(i: i32) i32 { const T: [i32; 5] = [2, 3, 5, 7, 11]; return T[i]; }\n"
          "fn main() i32 { unsafe exit(lut(0) + lut(4)); }\n", // 2 + 11
      13, "");
}
...
static void test_pointers(void) {
  sc_run_program(
      ...
      PRE "fn deref(p: *const i32) i32 { return unsafe *p; }\n"
          "fn main() i32 { let x: i32 = 42; unsafe exit(deref(&x)); }\n",
      42, "");
  ...
  sc_run_program(
      ...
      PRE "fn main() i32 {\n"
          ...
          "  unsafe exit(unsafe *p + *q);\n"
          "}\n",
      60, "");
  // &mut yields a mutable pointer the callee can write through.
  sc_run_program(
      "&mut address-of writes back",
      PRE "fn bump(p: *mut i32) void { unsafe *p = unsafe *p + 1; }\n"
          "fn main() i32 { let mut x: i32 = 41; bump(&mut x); unsafe exit(x); }\n",
      42, "");
}

static void test_misc(void) {
  sc_run_program("integer cast truncation", PRE "fn main() i32 { let x: i64 = 300; unsafe exit(x as u8 as i32); }\n", 44, "");
  sc_run_program(
      "keyword-mangled identifiers",
      PRE "fn main() i32 {\n"
          "  let register: i32 = 5; let switch_v: i32 = 3;\n"
          "  let mut volatile: i32 = register * switch_v; volatile = volatile + 1; unsafe exit(volatile);\n"
          "}\n",
      16, "");
  sc_run_program(
      "global const + forward-declared fn",
      PRE "const N: i32 = 35;\nfn main() i32 { unsafe exit(N + g()); }\nfn g() i32 { return 7; }\n", 42, "");
  sc_run_program(
      "putchar to stdout", PRE "fn main() i32 { for c in 0..2 { unsafe putchar(65 + c); } unsafe putchar(10); unsafe exit(0); }\n", 0,
      "AB\n");
}
...
static void test_let_owning_mut(void) {
  ...
  sc_run_program(
      ...
      PRE "extern \"C\" { fn malloc(n: usize) *mut void; fn free(p: *mut void) void; }\n"
          ...
          "extend Box {\n"
          "  fn make() Box { return Box { p: unsafe malloc(1) as *mut u8, }; }\n"
          "  fn set(self: &mut Box, v: u8) { unsafe self.p[0] = v; }\n"
          "  fn get(self: &Box) u8 { return unsafe self.p[0]; }\n"
          "  fn deinit(self: &mut Box) { unsafe free(self.p as *mut void); self.p = null; }\n"
          "}\n"
          "fn main() i32 { let mut b: Box = Box::make(); b.set(42); let r: u8 = b.get(); b.deinit(); unsafe exit(r as i32); }\n",
      42, "");
}
...
static void test_field_vs_method(void) {
  ...
  sc_run_program(
      ...
      PRE "struct S { pub len: i32, }\n"
          ...
          "fn main() i32 { let s: S = S::new(5); unsafe exit(s.len + s.len()); }\n",
...
static void test_generic_self_receiver(void) {
  sc_run_program(
      ...
      PRE "struct Wrap<T> { pub a: T, pub b: T }\n"
          "extend<T> Wrap<T> {\n"
          "  fn first(self: &Self) T { return unsafe self.a; }\n"
          "  fn swap(self: &Self) Wrap<T> { return Wrap::<T> { a: self.b, b: unsafe self.a }; }\n"
          "}\n"
          "fn main() i32 { let w = Wrap::<i32> { a: 30, b: 12 }; let s = w.swap(); unsafe exit(w.first() + s.first()); }\n",
      42, ""); // 30 + 12
}
...
static void test_str(void) {
  ...
  sc_run_program(
      ...
      PRE "fn first(s: str) i32 { return unsafe s.ptr[0] as i32; }\n"
          "fn main() i32 { let g: str = \"ABC\"; unsafe exit(first(g) + g.len as i32); }\n",
      68, ""); // 'A'=65 + len 3
  // Multi-byte UTF-8: `.len` counts bytes, and the raw bytes round-trip to stdout unchanged.
  sc_run_program(
      "str utf8 bytes to stdout",
      PRE "fn main() i32 { let s: str = \"é!\"; for i in 0..s.len { unsafe putchar(s.ptr[i] as i32); } unsafe exit(s.len as i32); }\n",
      3, "\xc3\xa9!"); // é is 2 UTF-8 bytes, '!' is 1 -> 3 bytes
  // An empty literal has length 0 (the for-loop body never runs).
  sc_run_program(
      "empty str", PRE "fn main() i32 { let s: str = \"\"; for i in 0..s.len { unsafe putchar(63); } unsafe exit(s.len as i32); }\n", 0,
      "");
}
...
static void test_generics_over_user_types(void) {
  sc_run_program(
      ...
      PRE "struct P { pub a: i32 }\n"
          ...
          "  unsafe exit(o.unwrap_or(P { a: 0 }).a + n.unwrap_or(P { a: 2 }).a); }\n", // 40 + 2
      42, "");
  sc_run_program(
...
static void test_container_conformances(void) {
  sc_run_program(
      ...
      PRE "struct P { pub a: i32 }\n"
          "extend P as Clone { fn clone(self: &Self) P { return P { a: unsafe self.a }; } }\n"
          "extend P as Eq { fn eq(self: &Self, other: &Self) bool { return unsafe self.a == other.a; } }\n"
          "extend P as Hash { fn hash(self: &Self) u64 { return unsafe self.a as u64; } }\n"
          "fn main() i32 { let mut v = Vector::<P>::new(); v.push(P{a:3}); v.push(P{a:4});\n"
          "  let mut w = v.clone();\n"
          "  let mut acc = 0;\n"
          "  if v.eq(&w) { acc = acc + 6; }\n"               // 6
          "  if v.hash() == w.hash() { acc = acc + 36; }\n"  // 42
          "  v.free(); w.free(); unsafe exit(acc); }\n",
      42, "");
  sc_run_program(
      ...
      PRE "struct P { pub a: i32 }\n"
          ...
          "  b.free(); b2.free(); unsafe exit(acc); }\n",
      42, "");
}
...
static void test_map(void) {
  sc_run_program(
      ...
      PRE "fn main() i32 {\n"
          ...
          "  let total = acc; m.free(); unsafe exit(total - 1100); }\n", // 49
      49, "");
}
...
static void test_format_printing(void) {
  ...
  sc_run_program(
      ...
      PRE "fn main() i32 {\n"
          ...
          "  unsafe putchar(10); unsafe exit(0); }\n",
  ...
  sc_run_program(
      ...
      PRE "extern \"C\" { fn malloc(n: usize) *mut void; fn realloc(p: *mut void, n: usize) *mut void; fn free(p: *mut void) void; }\n"
          ...
          "  fn alloc(self: &mut Mallocator, n: usize, align: usize) *mut void { return unsafe malloc(n); }\n"
          "  fn realloc(self: &mut Mallocator, p: *mut void, o: usize, n: usize, align: usize) *mut void { return unsafe realloc(p, n); }\n"
          "  fn dealloc(self: &mut Mallocator, p: *mut void, n: usize, align: usize) void { unsafe free(p); }\n"
          "}\n"
          ...
          "  out.println(); out.free(); unsafe exit(0); }\n",
...
static void test_checked_arith(void) {
  sc_run_program( // ordinary in-range signed arithmetic is unchanged
      ...
      PRE "fn main() i32 { let a: i32 = 40; let b: i32 = 2; unsafe exit(a + b); }\n", 42, "");
...
static void test_builtin_conformances(void) {
  sc_run_program( // a user extension method on a builtin -> i32__doubled
      ...
      PRE "extend i32 { fn doubled(self: i32) i32 { return self * 2; } }\n"
          "fn main() i32 { let x: i32 = 21; unsafe exit(x.doubled()); }\n",
      42, "");
  ...
  sc_run_program( // Ord on a builtin reached through a generic bound -> i32__cmp
      ...
      PRE "fn max_of<T: Ord>(a: T, b: T) T { if a.cmp(&b) < 0 { return b; } return a; }\n"
          "fn main() i32 { unsafe exit(max_of::<i32>(13, 42)); }\n",
      42, "");
  sc_run_program( // associated function on a builtin reached through a bound -> i32__default_
      ...
      PRE "fn zero<T: Default>() T { return T::default(); }\n"
          "fn main() i32 { unsafe exit(zero::<i32>() + 42); }\n",
      42, "");
  sc_run_program( // Copy is a marker interface, but generic bound checking still needs builtin conformance
      ...
      PRE "fn id<T: Copy>(x: T) T { return x; }\n"
          "fn main() i32 { unsafe exit(id::<i32>(42)); }\n",
      42, "");
  sc_run_program( // direct complex value methods avoid total Eq/Ord/Hash while still proving value semantics
      ...
      PRE "extern \"C\" { fn csqrt(z: c64) c64; fn cabs(z: c64) f64; }\n"
          "fn main() i32 { let i: c64 = unsafe csqrt(-1.0); let z: c64 = 3.0 + 4.0 * i; let b = z.clone();\n"
          "  let c: c64 = 0.0; unsafe exit((cabs(b) + cabs(c)) as i32 + 37); }\n",
      42, "");
  sc_run_program(
      "builtin inherent scalar methods",
      PRE "fn main() i32 {\n"
          "  let x: i32 = -5; let hi: i32 = 9; let a = x.abs() + x.signum() + hi.clamp(0, 7);\n"
          "  let y: u32 = 8; if !y.is_power_of_two() { unsafe exit(1); }\n"
          "  let f: f64 = 9.0; let g: f64 = -2.5; let h: f64 = 8.0;\n"
          "  let p: f32 = 0.0; let q: f32 = 4.0; let s: f32 = p.sin(); let r: f32 = q.sqrt();\n"
          "  let n: f64 = (0.0 as f64) / (0.0 as f64); if !n.is_nan() { unsafe exit(2); }\n"
          "  let c: f64 = f.sqrt() + g.abs() + h.log2(); let d: f32 = s + r;\n"
          "  unsafe exit(a + c as i32 + d as i32);\n"
          "}\n",
      21, "");
}
...
static void test_default_generic_args(void) {
  sc_run_program( // type-annotation path fills B=i32
      ...
      PRE "struct Pair<A, B = i32> { pub a: A, pub b: B }\n"
          "fn main() i32 { let p: Pair<bool> = Pair::<bool> { a: true, b: 42 }; unsafe exit(p.b); }\n",
      42, "");
  sc_run_program( // turbofish + a method monomorphized over the default-filled instance
      ...
      PRE "struct Pair<A, B = i32> { pub a: A, pub b: B }\n"
          ...
          "fn main() i32 { let p = Pair::<bool> { a: false, b: 42 }; unsafe exit(p.second()); }\n",
      42, "");
  sc_run_program( // a default that names an earlier param: `B = A` binds to the supplied A
      ...
      PRE "struct Dup<A, B = A> { pub a: A, pub b: B }\n"
          "fn main() i32 { let d = Dup::<i32> { a: 19, b: 23 }; unsafe exit(d.a + d.b); }\n",
      42, "");
}
...
static void test_custom_allocator(void) {
  sc_run_program(
      ...
      PRE "extern \"C\" { fn malloc(size: usize) *mut void; fn realloc(p: *mut void, size: usize) *mut void; fn free(p: *mut void) void; }\n"
          "struct Mallocator {}\n"
          "extend Mallocator as Allocator {\n"
          "  fn alloc(self: &mut Mallocator, size: usize, align: usize) *mut void { return unsafe malloc(size); }\n"
          "  fn realloc(self: &mut Mallocator, p: *mut void, old_size: usize, new_size: usize, align: usize) *mut void { return unsafe realloc(p, new_size); }\n"
          "  fn dealloc(self: &mut Mallocator, p: *mut void, size: usize, align: usize) void { unsafe free(p); }\n"
          "}\n"
          "extend Mallocator as Default { fn default() Mallocator { return Mallocator {}; } }\n"
          "fn main() i32 {\n"
          ...
          "  unsafe exit(*b.get() + *g.get() + last + 21); }\n",                   // 2 + 0 + 19 + 21 = 42
      42, "");
  ...
  sc_run_program(
      ...
      PRE "extern \"C\" { fn malloc(n: usize) *mut void; fn realloc(p: *mut void, n: usize) *mut void; fn free(p: *mut void) void; }\n"
          ...
          "extend Mallocator as Allocator {\n"
          "  fn alloc(self: &mut Mallocator, n: usize, align: usize) *mut void { return unsafe malloc(n); }\n"
          "  fn realloc(self: &mut Mallocator, p: *mut void, old_n: usize, n: usize, align: usize) *mut void { return unsafe realloc(p, n); }\n"
          "  fn dealloc(self: &mut Mallocator, p: *mut void, n: usize, align: usize) void { unsafe free(p); }\n"
          "}\n"
          "extend Mallocator as Default { fn default() Mallocator { return Mallocator {}; } }\n"
          "fn run() i32 {\n"
...
static void test_stateful_allocator(void) {
  sc_run_program(
      ...
      PRE "extern \"C\" { fn malloc(n: usize) *mut void; fn free(p: *mut void) void; fn memcpy(d: *mut void, s: *const void, n: usize) *mut void; }\n"
          ...
          "extend Arena {\n"
          "  fn make(cap: usize) Arena { return Arena { buf: unsafe malloc(cap) as *mut u8, off: 0, cap: cap, }; }\n"
          "  fn handle(self: &mut Arena) ArenaRef { return ArenaRef { a: self, }; }\n"
          "  fn used(self: &Arena) usize { return self.off; }\n"
          "  fn free(self: &mut Arena) { unsafe free(self.buf as *mut void); self.buf = null; self.off = 0; self.cap = 0; }\n"
          "}\n"
          "struct ArenaRef { pub a: *mut Arena, }\n"
          "extend ArenaRef as Allocator {\n"
          "  fn alloc(self: &mut ArenaRef, size: usize, align: usize) *mut void {\n"
          "    let mut off = unsafe self.a[0].off; let rem = off % align;\n"
          "    if rem != 0 { off = off + (align - rem); }\n"                          // align the bump pointer
          "    let p = ((unsafe self.a[0].buf as usize) + off) as *mut void;\n"
          "    unsafe self.a[0].off = off + size; return p;\n"
          "  }\n"
          ...
          "    if ptr != null { if old_size > 0 { unsafe memcpy(np, ptr as *const void, old_size); } }\n" // copy old_size bytes
          "    return np;\n"
          "  }\n"
          ...
          "  if touched && str_ok { unsafe exit((sum - 2255) as i32); }\n"                   // 1000+1225+70+2 = 2297 -> 42
          "  unsafe exit(1); }\n",
      42, "");
}
...
static void test_generics(void) {
  ...
  sc_run_program(
      ...
      PRE "fn id<T>(x: T) T { return x; }\n"
          "fn main() i32 { let a: i32 = id::<i32>(40); let b: bool = id::<bool>(true); if b { unsafe exit(a + 2); } unsafe exit(0); }\n",
      42, "");
  ...
  sc_run_program(
      ...
      PRE "fn id<T>(x: T) T { return x; }\n"
          ...
          "fn main() i32 { let x: i32 = 37; unsafe exit(id(5) + deref(&x)); }\n",
      42, ""); // 5 + 37
  ...
  sc_run_program(
      ...
      PRE "struct Pair<T> { pub a: T, pub b: i32 }\n"
          ...
          "  unsafe exit(if q.a { p.a + p.b + q.b; } else { 0; }); }\n",
      42, ""); // 20 + 5 + 17
  ...
  sc_run_program(
      ...
      PRE "struct Box<T> { pub v: T }\n"
          ...
          "fn main() i32 { unsafe exit(unwrap(Box::<i32> { v: 42 })); }\n",
      42, "");
  ...
  sc_run_program(
      ...
      PRE "enum Opt<T> { Some(T), None }\n"
          ...
          "  unsafe exit(x + switch b { Some(_) => 99, None => 0, }); }\n",
      42, ""); // 42 + 0
  ...
  sc_run_program(
      ...
      PRE "enum Opt<T> { Some(T), None }\n"
          ...
          "  unsafe exit(switch a { Some(v) => v, None => 0, } + bv + switch c { Ok(n) => n, Err(_) => 0, }); }\n",
      42, ""); // 20 + 5 + 17
  ...
  sc_run_program(
      ...
      PRE "enum Opt<T> { Some(T), None }\n"
          ...
          "  let b: Opt<i32> = Opt::<i32>::None;\n"
          "  unsafe exit(unwrap_or(a, 0) - unwrap_or(b, 0)); }\n",
      42, "");
}
...
static void test_nested_generic_by_value(void) {
  sc_run_program(
      ...
      PRE "fn main() i32 { let mut v = Vector::<i32>::new(); v.push(40); v.push(2);\n"
          ...
          "  let mut acc = 0; for x in w.iter() { acc = acc + *x; } w.free(); unsafe exit(acc); }\n", // iter borrows -> &i32
      42, "");
  ...
  sc_run_program(
      ...
      PRE "fn main() i32 { let s = String::from(\"hi\");\n"
          ...
          "  let n = w.len() as i32; w.free(); unsafe exit(n); }\n",
      2, "");
  ...
  sc_run_program(
      ...
      PRE "struct Holder { pub o: Option<i32> }\n"
          "fn main() i32 { let h = Holder { o: Option::<i32>::some(42) }; unsafe exit(h.o.unwrap_or(0)); }\n",
      42, "");
}
...
static void test_std_types(void) {
  ...
  sc_run_program("sizeof", PRE "fn main() i32 { unsafe exit(((sizeof(i32) + sizeof(u8)) as i32) * 8 + 2); }\n", 42, ""); // (4+1)*8+2
  ...
  sc_run_program(
      ...
      PRE "fn pick<T: Ord>(a: T, b: T) T { if a.cmp(&b) >= 0 { return a; } return b; }\n"
          ...
          "  let o: Option<i32> = Default::default(); acc = acc + o.unwrap_or(0); v.free(); x.free(); y.free(); big.free(); d.free(); unsafe exit(acc); }\n",
      111, "banana"); // 1 + 10 + 100
  ...
  sc_run_program(
      ...
      PRE "fn dbl(x: i32) i32 { return x * 2; }\n"
          ...
          "fn main() i32 { unsafe exit(apply(dbl, 21)); }\n",
      42, "");
  ...
  sc_run_program(
      ...
      PRE "fn dbl(x: &i32) i32 { return *x * 2; }\n"
          ...
          "  v.free(); m.free(); unsafe exit(r + 16); }\n",             // 26 + 16
      42, "");
  ...
  sc_run_program(
      ...
      PRE "fn dbl(x: i32) i32 { return x * 2; }\n"
          ...
          "  let r: i32 = *c.get(); c.free(); unsafe exit(r); }\n", // get borrows -> &i32
      42, "");
}
...
static void test_closures(void) {
  ...
  sc_run_program(
      ...
      PRE "fn run(f: fn(i32) void, n: i32) void { f(n); }\n"
          "fn main() i32 { run(fn(c: i32) { unsafe putchar(c); }, 65); unsafe exit(0); }\n",
      0, "A");
...
static void test_unions(void) {
  ...
  sc_run_program(
      ...
      PRE "union Bits { pub i: u32, pub bytes: [u8; 4] }\n"
          ...
          "  unsafe exit(u.bytes[0] as i32 + u.bytes[3] as i32 - 100); }\n", // 65 + 68 - 100
      33, "");
  sc_run_program(
...
static void test_string_sso(void) {
  ...
  sc_run_program(
      ...
      PRE "fn main() i32 { let mut s: String = String::new();\n"
          ...
          "  if s.len() != 40 { unsafe exit(1); }\n"
          "  if s.capacity() < 40 { unsafe exit(2); }\n"                                  // grew onto the heap
          "  let mut j: usize = 0; while j < 40 { if s.byte(j) != 65 { unsafe exit(3); } j = j + 1; }\n" // bytes survived
          "  s.truncate(5); s.shrink_to_fit();\n"
          "  if s.capacity() != 23 { unsafe exit(4); }\n"                                 // moved back inline
          "  let r: i32 = s.len() as i32 + 37; s.free(); unsafe exit(r); }\n",            // 5 + 37
      42, "");
}
...
static void test_interfaces(void) {
  sc_run_program(
      ...
      PRE "interface Writer { fn write(self: *mut Self, n: i32) i32; }\n"
          ...
          "extend File as Writer { fn write(self: *mut Self, n: i32) i32 { unsafe { self.count = self.count + n; } return unsafe self.count; } }\n"
          "fn use_w<T: Writer>(w: &mut T, n: i32) i32 { return w.write(n); }\n"
          "fn main() i32 { let mut f = File { count: 0 };\n"
          "  let a = use_w(&mut f, 42); unsafe exit(a + use_w::<File>(&mut f, 8)); }\n", // 42 + 50
      92, "");
  sc_run_program(
      ...
      PRE "interface A { fn a(self: *mut Self) i32; }\ninterface B { fn b(self: *mut Self) i32; }\n"
          ...
          "extend S as B { fn b(self: *mut Self) i32 { return unsafe self.v + 1; } }\n"
          "fn both<T>(x: &mut T) i32 where T: A + B { return x.a() + x.b(); }\n"
          "fn main() i32 { let mut s = S { v: 10 }; unsafe exit(both(&mut s)); }\n", // 10 + 11
      21, "");
  sc_run_program(
      ...
      PRE "interface Free { fn free(self: *mut Self) i32; }\n"
          ...
          "fn main() i32 { let mut b = Box::<Res> { inner: Res { id: 99 } }; unsafe exit(dispose(&mut b)); }\n",
      99, "");
  sc_run_program(
      "interface: Self-returning method",
      PRE "interface Clone { fn dup(self: *mut Self) Self; }\n"
          "struct P { pub x: i32 }\nextend P as Clone { fn dup(self: *mut Self) Self { return P { x: unsafe self.x }; } }\n"
          "fn copy_of<T: Clone>(w: &mut T) T { return w.dup(); }\n"
          "fn main() i32 { let mut p = P { x: 7 }; let q = copy_of(&mut p); unsafe exit(q.x); }\n",
      7, "");
  ...
  sc_run_program(
      ...
      PRE "interface Default { fn default() Self; }\n"
          ...
          "fn main() i32 { let q = make::<Q>(); unsafe exit(q.x); }\n",
      7, "");
  ...
  sc_run_program(
      ...
      PRE "interface Default { fn default() Self; }\n"
          ...
          "fn main() i32 { let r: R = Default::default(); unsafe exit(r.x + r.y); }\n",
      14, "");
  ...
  sc_run_program(
      ...
      PRE "interface From<T> { fn from(value: T) Self; }\n"
          ...
          "fn main() i32 { let c = C { d: 100 }; let f: F = c.into(); unsafe exit(f.d); }\n",
      212, "");
  ...
  sc_run_program(
      ...
      PRE "interface TryFrom<T> { fn try_from(value: T) Result<Self, i32>; }\n"
          ...
          "fn main() i32 { let r: Result<Sm, i32> = (200).try_into(); let s = r.unwrap_or(Sm { v: 0 }); unsafe exit(s.v); }\n",
      200, "");
  ...
  sc_run_program(
      ...
      PRE "interface Eq { fn eq(self: &Self, other: &Self) bool; }\n"
          ...
          "  if a == b { acc = acc + 1000; } unsafe exit(acc + least::<P>(a, b).n); }\n",
      114, ""); // 1 + 10 + 100 + 3
  ...
  sc_run_program(
      ...
      PRE "interface Iterator<T> { fn next(self: &mut Self) Option<T>; }\n"
          ...
          "fn main() i32 { let mut sum = 0; let c = Counter { cur: 1, end: 6 }; for x in c { sum = sum + x; } unsafe exit(sum); }\n",
      15, ""); // 1+2+3+4+5
  ...
  sc_run_program(
      ...
      PRE "interface Eq { fn eq(self: &Self, other: &Self) bool; fn ne(self: &Self, other: &Self) bool { return !self.eq(other); } }\n"
          ...
          "  if a.lt(&b) { acc = acc + 1; } if a.ge(&b) { acc = acc + 10; } if a.ne(&b) { acc = acc + 100; } unsafe exit(acc); }\n",
      101, "");
}
...
static void test_operator_overloading(void) {
  ...
  sc_run_program(
      ...
      PRE "struct Wrap<T> { pub a: T, pub b: T }\n"
          ...
          "fn main() i32 { let w = Wrap::<i32> { a: 30, b: 12 }; unsafe exit(w[0] + w[1]); }\n", // 42
      42, "");
...
static void test_index_interface(void) {
  sc_run_program(
      ...
      PRE "struct Buf { pub data: [i32; 8], pub n: usize }\n"
          ...
          "    let hi = if r.inclusive { r.end + 1; } else { r.end; };\n"
          "    return SliceMut::<i32> { ptr: unsafe (((&mut self.data[0]) as *mut i32) + r.start), len: hi - r.start, };\n"
          "  }\n"
          "}\n"
          "fn main() i32 {\n"
          ...
          "  unsafe exit(b[0] + s1.get(1) + s2.len() as i32 + s3.get(1) + s4.get(0)); }\n", // 3+5+2+80+60 = 150
      150, "");
  sc_run_program(
      ...
      PRE "fn main() i32 {\n"
          ...
          "  unsafe exit(v[0] + v[1] + a.get(0) + b.len() as i32 + c.get(1) + d.get(2)); }\n", // 7+13+20+3+70+60 = 173
      173, "");
  sc_run_program(
  ...
  sc_run_program(
      ...
      PRE "struct R { pub id: i32 }\n"
          "extend R as Free { fn free(self: &mut R) { unsafe putchar(48 + self.id); } }\n"
          "fn main() i32 {\n"
...
static void test_question_operator(void) {
  sc_run_program(
      ...
      PRE "fn checked(n: i32) Option<i32> { let o = Option::<i32>::some(n); let v = o?; return Option::<i32>::some(v + 1); }\n"
          ...
          "  let b = switch first(Option::<i32>::none()) { Some(_) => 1, None => 100, }; unsafe exit(a + b - 100); }\n", // 42
      42, "");
  sc_run_program(
      ...
      PRE "fn rdiv(a: i32, b: i32) Result<i32, i32> { if b == 0 { return Result::<i32, i32>::err(9); }\n"
          ...
          "fn main() i32 { unsafe exit(switch calc() { Ok(v) => v, Err(e) => 33 + e, }); }\n", // rdiv(x,0) -> Err(9) -> 42
      42, "");
  sc_run_program(
...
#define FREE_PRE                                                                                                        \
  PRE "interface Free { fn free(self: &mut Self); }\n"                                                                  \
      ...
      "extend R as Free { fn free(self: &mut Self) { unsafe putchar(self.tag); } }\n"
  // RAII: locals are freed at scope exit in reverse construction order.
  sc_run_program(
      "free: reverse-order scope-exit cleanup",
      FREE_PRE "fn run() void { let a = R { tag: 65 }; let b = R { tag: 66 }; unsafe putchar(88); }\n"
               "fn main() i32 { run(); unsafe exit(0); }\n",
  ...
  sc_run_program(
      ...
      FREE_PRE "fn run() void { let a = R { tag: 65 }; let b = a; unsafe putchar(88); }\n"
               "fn main() i32 { run(); unsafe exit(0); }\n",
  ...
  sc_run_program(
      ...
      FREE_PRE "fn consume(r: R) void { unsafe putchar(67); }\n"
               "fn run() void { let a = R { tag: 65 }; consume(a); unsafe putchar(88); }\n"
               "fn main() i32 { run(); unsafe exit(0); }\n",
  ...
  sc_run_program(
      ...
      FREE_PRE "fn run() void { let a = R { tag: 65 }; defer unsafe putchar(68); let b = R { tag: 66 }; unsafe putchar(88); }\n"
               "fn main() i32 { run(); unsafe exit(0); }\n",
...
#define CM_PRE                                                                                                          \
  ...
  sc_run_program(
      ...
      CM_PRE "fn run(c: bool) void { let a = R { tag: 65 }; if c { consume(a); } }\n"
             "fn main() i32 { run(true); run(false); unsafe exit(0); }\n",
  ...
  sc_run_program(
      ...
      CM_PRE "fn main() i32 { let mut i = 0; while i < 3 { let a = R { tag: 65 }; if i == 1 { consume(a); } i = i + 1; } unsafe exit(0); }\n",
      0, "AAA");
#undef CM_PRE
}
...
#define BOX_PRE                                                                                                         \
  PRE "interface Free { fn free(self: &mut Self); }\n"                                                                  \
      ...
      "extend Leaf as Free { fn free(self: &mut Self) { unsafe putchar(self.id); } }\n"                                        \
      "struct Box<T> { pub inner: T }\n"                                                                                \
  ...
  sc_run_program("container RAII: scope-exit recursive auto-free",
                 BOX_PRE "fn run() void { let a = Box::<Leaf> { inner: Leaf { id: 65 } }; unsafe putchar(88); }\n"
                         "fn main() i32 { run(); unsafe exit(0); }\n",
                 0, "XA"); // X, then free a -> its inner Leaf (A)
  sc_run_program("container RAII: move into consumer suppresses source free",
                 BOX_PRE "fn run() void { let a = Box::<Leaf> { inner: Leaf { id: 65 } }; take(a); unsafe putchar(88); }\n"
                         "fn main() i32 { run(); unsafe exit(0); }\n",
...
#define FI_PRE                                                                                                         \
  ...
  sc_run_program("free intrinsic: Free type frees, non-Free is a no-op (via a generic)",
                 FI_PRE "fn dispose<T>(mut x: T) void { x.free(); }\n"
                        "fn main() i32 { dispose::<Leaf>(Leaf { id: 65 }); dispose::<i32>(7); unsafe putchar(10); unsafe exit(0); }\n",
...
static void test_std_container_auto_free(void) {
  ...
  sc_run_program(
      ...
      PRE "struct Tr { pub id: i32 }\n"
          ...
          "  if freed.is_none() { unsafe putchar(63); }\n"            // not reached
          "}\n"
          "fn main() i32 { run(); unsafe putchar(10); unsafe exit(0); }\n",
...
static void test_switch_binding_modes(void) {
  ...
  sc_run_program(
      ...
      PRE "fn main() i32 { let mut o = Option::<i32>::Some(41);\n"
          ...
          "  unsafe exit(o.unwrap_or(0) + r - 84); }\n",                            // o is now Some(42); r == 42
      0, "");
...
#define TR_PRE                                                                                                          \
  ...
  sc_run_program(
      ...
      TR_PRE "fn run() i32 { let o = Option::<Tr>::Some(Tr { id: 65 });\n"
             ...
             "  return r - 65; }\n"
             "fn main() i32 { let r = run(); unsafe putchar(10); unsafe exit(r); }\n",
      0, "A\n");
...
static void test_defer(void) {
  ...
  sc_run_program(
      "defer LIFO",
      PRE "fn run() void { unsafe putchar('1'); defer unsafe putchar('a'); defer unsafe putchar('b'); unsafe putchar('2'); }\n"
          "fn main() i32 { run(); unsafe exit(0); }\n",
      0, "12ba");
  ...
  sc_run_program(
      ...
      PRE "fn f() i32 { let mut x: i32 = 5; defer x = 99; return x; }\nfn main() i32 { unsafe exit(f()); }\n", 5, "");
  // nested-scope defer + return: inner Y runs before outer X.
  sc_run_program(
      "defer nested scope on return",
      PRE "fn g() void { defer unsafe putchar('X'); if true { defer unsafe putchar('Y'); return; } }\n"
          "fn main() i32 { g(); unsafe exit(0); }\n",
      0, "YX");
  // a loop body's defer runs at the end of each iteration, including on `continue`.
  sc_run_program(
      "defer in loop with continue",
      PRE "fn main() i32 { for i in 0..3 { defer unsafe putchar('.'); if i == 1 { continue; } unsafe putchar(('0' as i32) + i); } unsafe exit(0); }\n",
      0, "0..2.");
}
...
static void test_bug_regressions(void) {
  ...
  sc_run_program("discarded owning temporary freed",
      PRE "struct T { pub tag: i32 }\n"
          "extend T as Free { fn free(self: &mut T) { unsafe putchar(self.tag); } }\n"
