...
fn text_line_starts_and_positions() {
    ...
    // X(1) + emoji(2).
    assert_eq(p.character, 3 as u32);
...
fn text_clamping() {
    ...
    // Line clamps to the last.
    assert_eq(text::pos_to_offset(src, &ls, 99, 0), 3 as u32);
    // Character clamps to the line end.
    assert_eq(text::pos_to_offset(src, &ls, 0, 99), 2 as u32);
...
fn text_utf16_len() {
    ...
    // End clamps.
    assert_eq(text::utf16_len("abc", 1, 99), 2 as u32);
}
