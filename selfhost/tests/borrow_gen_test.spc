...
struct Buf2048 { pub b: [char; 2048] }
struct Buf8192 { pub b: [char; 8192] }
struct IdxBuf { pub b: [i32; 17] }
...
fn buf_str(p: *const char) str {
    return str { ptr: p as *const u8, len: unsafe cstring::strlen(p) };
}
...
fn case_pre(pre: str, body: str, label: str, ok: bool) {
    let mut src = Buf2048 {};
    unsafe stdio::snprintf((&mut src.b[0]) as *mut char, 2048,
        "%s%s".ptr as *const char, pre.ptr as *const char, body.ptr as *const char);
            ...
            while p1 < 3 {
                ...
                while p2 < 3 {
                    ...
                    unsafe stdio::snprintf((&mut src.b[0]) as *mut char, 2048,
                        ...
                        PRE.ptr as *const char, kinds[k1].ptr as *const char, places[p1].ptr as *const char,
                        kinds[k2].ptr as *const char, places[p2].ptr as *const char,
                        (&u1.b[0]) as *const char, (&u2.b[0]) as *const char);
                    let want = !(overlap(p1, p2) && (k1 != 0 || k2 != 0));
                    check_case("aliasing", buf_str((&src.b[0]) as *const char), want);
                    p2 = p2 + 1;
                }
                p1 = p1 + 1;
            }
...
fn use_while_borrowed() {
    ...
    while k < 2 {
        ...
        while p1 < 3 {
            ...
            while p2 < 3 {
                let mut ur = Buf64 {};
                use_ref((&mut ur.b[0]) as *mut char, "r", p1);
                let mut src = Buf2048 {};
                unsafe stdio::snprintf((&mut src.b[0]) as *mut char, 2048,
                    "%sfn main() i32 { let mut p = P { a: 1, b: 2 };\n  let r = &%s%s;\n  let y = %s;\n  let keep = %s; return keep; }\n".ptr as *const char,
                    PRE.ptr as *const char, kinds[k].ptr as *const char, places[p1].ptr as *const char,
                    places[p2].ptr as *const char, (&ur.b[0]) as *const char);
                let want = !(overlap(p1, p2) && k != 0);
                check_case("use while borrowed", buf_str((&src.b[0]) as *const char), want);
                p2 = p2 + 1;
            }
            p1 = p1 + 1;
        }
        ...
        while p1 < 3 {
            ...
            while p2 < 3 {
                ...
                unsafe stdio::snprintf((&mut src.b[0]) as *mut char, 2048,
                    "%sfn main() i32 { let mut p = P { a: 1, b: 2 };\n  let r = &%s%s;\n  let used = %s;\n  let y = %s; return used; }\n".ptr as *const char,
                    PRE.ptr as *const char, kinds[k].ptr as *const char, places[p1].ptr as *const char,
                    (&ur.b[0]) as *const char, places[p2].ptr as *const char);
                check_case("nll", buf_str((&src.b[0]) as *const char), true);
                p2 = p2 + 1;
            }
            p1 = p1 + 1;
        }
...
@test
fn moves() {
    case_pre(OWN, "fn main() i32 { let s = Own { id: 1 }; let r = &s; let t = s; let keep = r.id; return t.id + keep; }\n",
        "move s while &s live", false);
    case_pre(OWN, "fn main() i32 { let s = Own { id: 1 }; let r = &s; let keep = r.id; let t = s; return t.id + keep; }\n",
        "move s after &s dead (NLL)", true);
}
...
fn lifetimes() {
    ...
    check_case("return stored ref to local", "fn f() &i32 { let x = 5; let r = &x; return r; }\nfn main() i32 { return *f(); }\n", false);
    check_case("return ref chained to local", "fn f() &i32 { let x = 5; let r = &x; let s = r; return s; }\nfn main() i32 { return *f(); }\n", false);
    check_case("return &by-value param", "fn f(x: i32) &i32 { return &x; }\nfn main() i32 { return *f(5); }\n", false);
    check_case("return &field of local", "struct Q { pub a: i32 }\nfn f() &i32 { let q = Q { a: 1 }; return &q.a; }\nfn main() i32 { return *f(); }\n", false);
    check_case("return ref param", "fn f(x: &i32) &i32 { return x; }\nfn main() i32 { let v = 5; return *f(&v); }\n", true);
    check_case("return reborrow of ref param", "fn f(x: &i32) &i32 { let r = x; return r; }\nfn main() i32 { let v = 5; return *f(&v); }\n", true);
}

// Family F -- scope-scoped borrows and disjoint-field mutation: valid patterns the checker must ACCEPT.
@test
fn valid_patterns() {
    check_case("scoped &mut released before use", "fn main() i32 { let mut x = 5; { let r = &mut x; *r = 1; } let y = x; return y; }\n", true);
    case_pre(PRE, "fn add2(x: &mut i32, y: &mut i32) i32 { return *x + *y; }\nfn main() i32 { let mut p = P { a: 1, b: 2 }; return add2(&mut p.a, &mut p.b); }\n",
        "two disjoint &mut fields coexist", true);
    check_case("sequential borrows of x", "fn main() i32 { let mut x = 5; let a = &mut x; *a = 2; let b = &x; return *b; }\n", true);
    check_case("many shared borrows", "fn main() i32 { let x = 5; let a = &x; let b = &x; let c = &x; return *a + *b + *c; }\n", true);
}
...
@test
fn closed_gaps() {
    case_pre(CDEF, "fn main() i32 { let mut c = C { n: 0 }; let r = &c; c.bump(); return r.n; }\n",
        "method &mut self while &c live", false);
    case_pre(CDEF, "fn main() i32 { let mut c = C { n: 0 }; let r = &mut c; let v = c.get(); return v + r.n; }\n",
        "method &self read while &mut c live", false);
    check_case("two-phase v.push(v.len())",
        "fn main() i32 { let mut v = Vector::<i32>::new(); v.push(1); v.push(v.len() as i32); return v.len() as i32; }\n", true);
    case_pre(CDEF, "fn main() i32 { let mut c = C { n: 0 }; let r = &c; let v = r.n; c.bump(); return v + c.get(); }\n",
        "method after ref dead (NLL)", true);
    check_case("assign x while &x live", "fn main() i32 { let mut x = 5; let r = &x; x = 9; return *r; }\n", false);
    check_case("assign x after &x dead", "fn main() i32 { let mut x = 5; let r = &x; let v = *r; x = 9; return x + v; }\n", true);
    check_case("mutate origin while reborrow live", "fn main() i32 { let mut x = 5; let r = &mut x; let s = &*r; x = 9; return *s; }\n", false);
    check_case("assign arg while call-result ref live",
        "fn pick(a: &i32) &i32 { return a; }\nfn main() i32 { let mut x = 5; let r = pick(&x); x = 9; return *r; }\n", false);
    case_pre(SINK, "fn main() i32 { let s = Own { id: 1 }; let mut i = 0; while i < 3 { let k = sink(s); i = i + 1; } return 0; }\n",
        "move in loop, no reassign", false);
    case_pre(SINK, "fn main() i32 { let mut s = Own { id: 1 }; let mut i = 0; while i < 3 { let k = sink(s); s = Own { id: 2 }; i = i + 1; } return 0; }\n",
        "move + reassign in loop", true);
    case_pre(SINK, "fn main() i32 { let mut s = Own { id: 1 }; let a = sink(s); s = Own { id: 2 }; let b = sink(s); return a + b; }\n",
        "reassign then move again", true);
    check_case("read scrutinee while &mut payload binding live",
        "enum E { V(i32) }\nfn main() i32 { let mut e = E::V(1); let r = switch &mut e { V(y) => y, }; let z = e; *r = 2; return z; }\n", false);
    check_case("match &e shared peek",
        "enum E { V(i32) }\nfn main() i32 { let e = E::V(1); let n = switch &e { V(y) => *y, }; return n; }\n", true);
    check_case("assign arg while tuple-destructured ref live",
    ...
    check_case("tuple refs scoped, then assign arg",
        "fn split(a: &mut i32, b: &mut i32) (&i32, &i32) { return a, b; }\nfn main() i32 { let mut x = 5; let mut y = 6; { let (r, s) = split(&mut x, &mut y); let v = *r + *s; } x = 9; return x; }\n", true);
    check_case("non-reference tuple destructure",
...
@test
fn ref_semantics() {
    check_case("A1 reassigned ref aliases new target",
        "fn main() i32 { let mut x = 0; let mut y = 0; let mut r = &mut x; r = &mut y; let b = y; return *r + b; }\n", false);
    check_case("A1 reassigned ref frees old target",
        "fn main() i32 { let mut x = 0; let mut y = 0; let mut r = &mut x; r = &mut y; let a = x; return *r + a; }\n", true);
    check_case("A1 reassign ref that had no prior borrow",
        "fn g(p: &mut i32) i32 { let mut y = 0; let mut r = p; r = &mut y; let b = y; return *r + b; }\nfn main() i32 { let mut x = 0; return g(&mut x); }\n", false);
    check_case("A2 copy of &mut moves it",
        "fn main() i32 { let mut x = 0; let r1 = &mut x; let r2 = r1; *r1 = 1; *r2 = 2; return x; }\n", false);
    check_case("A2 copy of & duplicates it",
        "fn main() i32 { let x = 0; let r1 = &x; let r2 = r1; let a = *r1; let b = *r2; return a + b; }\n", true);
    check_case("A2 copied & keeps origin tracked",
        "fn main() i32 { let mut x = 0; let r1 = &x; let r2 = r1; x = 9; return *r1 + *r2; }\n", false);
    check_case("A3 use origin while reborrow live",
        "fn main() i32 { let mut x = 0; let r = &mut x; let r2 = &mut *r; *r = 1; return *r2; }\n", false);
    check_case("A3 reborrow scoped, then use origin",
        "fn main() i32 { let mut x = 0; let r = &mut x; { let r2 = &mut *r; *r2 = 1; } *r = 2; return *r; }\n", true);
    case_pre(PRE, "fn main() i32 { let mut p = P { a: 1, b: 2 }; let r = &mut p; let x = &mut r.a; let y = &mut r.a; return *x + *y; }\n",
        "A4 two &mut same field through ref", false);
    case_pre(PRE, "fn main() i32 { let mut p = P { a: 1, b: 2 }; let r = &mut p; let x = &mut r.a; let y = &mut r.b; return *x + *y; }\n",
        "A4 disjoint fields through ref", true);
    case_pre(PRE, "fn main() i32 { let mut p = P { a: 1, b: 2 }; let r = &mut p; let x = &mut r.a; let v = r.a; return *x + v; }\n",
        "A4 read field through ref while &mut field live", false);
    case_pre(PRE, "fn main() i32 { let mut p = P { a: 1, b: 2 }; let r = &mut p; let x = &mut r.a; let z = &mut *r; return *x + z.a; }\n",
        "A4 whole reborrow overlaps field reborrow", false);
    check_case("B5 distinct constant slots coexist",
        "fn main() i32 { let mut a = [1, 2, 3]; let x = &mut a[0]; let y = &mut a[1]; return *x + *y; }\n", true);
    check_case("B5 read disjoint slot while &mut other slot",
        "fn main() i32 { let mut a = [1, 2, 3]; let x = &mut a[0]; let v = a[1]; return *x + v; }\n", true);
    check_case("B5 same constant slot conflicts",
        "fn main() i32 { let mut a = [1, 2, 3]; let x = &mut a[0]; let y = &mut a[0]; return *x + *y; }\n", false);
    check_case("B5 variable indices conservative",
...
@test
fn third_audit() {
    check_case("K1 inner-scope rebind keeps borrow",
        "fn main() i32 { let mut x = 1; let y = 2; let mut r = &y; { r = &x; } let m = &mut x; return *r + *m; }\n", false);
    check_case("K1 inner-scope rebind, ref dead",
        "fn main() i32 { let mut x = 1; let y = 2; let mut r = &y; { r = &x; } let v = *r; let m = &mut x; *m = 3; return v; }\n", true);
    check_case("K2 stored ref outlives referent",
        "fn main() i32 { let x = 1; let mut r = &x; { let y = 2; r = &y; } return *r; }\n", false);
    check_case("K2 stored ref to longer-lived value",
        "fn main() i32 { let y = 2; let x = 1; let mut r = &x; { r = &y; } return *r; }\n", true);
    case_pre(CDEF2, "fn main() i32 { let mut h = H { v: 1 }; let r = h.geti(); h.seti(5); return *r; }\n",
        "K3 mutate while method-returned ref live", false);
    case_pre(CDEF2, "fn main() i32 { let mut h = H { v: 1 }; let v = *h.geti(); h.seti(5); return v + h.v; }\n",
        "K3 method ref deref'd before mutate", true);
    check_case("K4 return ref reassigned to local",
        "fn f(p: &i32) &i32 { let mut r = p; let x = 1; r = &x; return r; }\nfn main() i32 { let v = 5; return *f(&v); }\n", false);
    check_case("K4 return ref reassigned to param",
        "fn f(p: &i32) &i32 { let x = 1; let mut r = &x; r = p; return r; }\nfn main() i32 { let v = 5; return *f(&v); }\n", true);
    case_pre(OWN, "fn main() i32 { let s = Own { id: 1 }; let t = move s; let u = s; return t.id + u.id; }\n",
        "K5 move keyword still moves", false);
    case_pre(OWN, "fn main() i32 { let s = Own { id: 1 }; let t = move s; return t.id; }\n",
        "K5 move keyword, no reuse", true);
    check_case("K6 init only inside while",
        "fn main() i32 { let mut x: i32; let c = false; while c { x = 1; } return x; }\n", false);
    check_case("K6 init inside do-while",
        "fn main() i32 { let mut x: i32; do { x = 1; } while false; return x; }\n", true);
    case_pre(OWN, "fn main() i32 { let mut s = Own { id: 1 }; let r = &mut s; r.free(); return 0; }\n",
        "K7 free through local ref", false);
    case_pre(OWN, "fn kill(o: &mut Own) { o.free(); }\nfn main() i32 { let mut s = Own { id: 1 }; kill(&mut s); return 0; }\n",
        "K7 free through param ref", true);
    check_case("K8 overlapping reborrows through param",
        "fn f(p: &mut i32) i32 { let a = &mut *p; let b = &mut *p; return *a + *b; }\nfn main() i32 { let mut x = 0; return f(&mut x); }\n", false);
    case_pre(PRE, "fn f(p: &mut P) i32 { let a = &mut p.a; let b = &mut p.b; return *a + *b; }\nfn main() i32 { let mut q = P { a: 1, b: 2 }; return f(&mut q); }\n",
        "K8 disjoint fields through param", true);
    check_case("K9 union members alias",
        "union U { pub a: i32, pub b: f32 }\nfn main() i32 { let mut u = U { a: 1 }; let r = &u.a; let m = &mut u.b; *m = 2.0; return *r; }\n", false);
    check_case("K9 union shared+shared ok",
        "union U { pub a: i32, pub b: f32 }\nfn main() i32 { let u = U { a: 1 }; let r = &u.a; let s = &u.a; return *r + *s; }\n", true);
    check_case("K10 tuple element double &mut",
        "fn two() (i32, i32) { return 1, 2; }\nfn main() i32 { let mut (a, b) = two(); let r = &a; let m = &mut a; *m = 9; return *r + b; }\n", false);
    check_case("K10 tuple element sequential borrows",
        "fn two() (i32, i32) { return 1, 2; }\nfn main() i32 { let mut (a, b) = two(); let r = &a; let v = *r; let m = &mut a; *m = 9; return v + b; }\n", true);
    case_pre(OWN, "fn g(v: Own, m: &mut Own) i32 { return v.id + m.id; }\nfn main() i32 { let mut s = Own { id: 1 }; return g(s, &mut s); }\n",
        "K11 move + &mut same value in one call", false);
    case_pre(OWN, "fn g(v: Own, w: Own) i32 { return v.id + w.id; }\nfn main() i32 { let s = Own { id: 1 }; return g(s, s); }\n",
        "K11 move same value twice in one call", false);
    case_pre(OWN, "fn g(v: Own, w: Own) i32 { return v.id + w.id; }\nfn main() i32 { let s = Own { id: 1 }; let u = Own { id: 2 }; return g(s, u); }\n",
        "K11 move two distinct values", true);
    case_pre(PRE, "fn make() P { return P { a: 1, b: 2 }; }\nfn main() i32 { let r = &make(); return r.a; }\n",
        "K12 address of call result", false);
    case_pre(PRE, "fn main() i32 { let r = &P { a: 1, b: 2 }; return r.a; }\n",
        "K12 address of struct literal", true);
    case_pre(OWN, "fn main() i32 { let mut s = Own { id: 1 }; defer s.free(); let v = s.id; return v; }\n",
        "K13 use before deferred free", true);
    case_pre(OWN, "fn main() i32 { let mut s = Own { id: 1 }; defer s.free(); defer s.free(); return 0; }\n",
        "K13 two defers free one value", false);
    check_case("K14 last use before &mut in one call",
        "fn f(v: i32, m: &mut i32) { *m = v; }\nfn main() i32 { let mut x = 1; let r = &x; f(*r, &mut x); return x; }\n", true);
    check_case("K14 assign from own borrow",
        "fn main() i32 { let mut x = 5; let r = &x; x = *r + 1; return x; }\n", true);
    check_case("K14 use after keeps borrow live",
        "fn f(v: i32, m: &mut i32) i32 { *m = v; return *m; }\nfn main() i32 { let mut x = 1; let r = &x; let v = f(*r, &mut x); return v + *r; }\n", false);
    check_case("K15 out-param init via &mut",
        "fn init(p: &mut i32) { *p = 42; }\nfn main() i32 { let mut x: i32; init(&mut x); return x; }\n", true);
    check_case("K15 shared borrow does not init",
                    ...
                    while p2 < 3 {
                        ...
                        while p3 < 3 {
                            ...
                            unsafe stdio::snprintf((&mut src.b[0]) as *mut char, 2048,
                                ...
                                PRE.ptr as *const char, kinds[k1].ptr as *const char, places[p1].ptr as *const char,
                                kinds[k2].ptr as *const char, places[p2].ptr as *const char,
                                kinds[k3].ptr as *const char, places[p3].ptr as *const char,
                                (&u1.b[0]) as *const char, (&u2.b[0]) as *const char, (&u3.b[0]) as *const char);
                            let bad = (overlap(p1, p2) && (k1 != 0 || k2 != 0)) || (overlap(p1, p3) && (k1 != 0 || k3 != 0)) || (overlap(p2, p3) && (k2 != 0 || k3 != 0));
                            check_case("aliasing3", buf_str((&src.b[0]) as *const char), !bad);
                            p3 = p3 + 1;
                        }
                        p2 = p2 + 1;
                    }
...
fn composition() {
    ...
    while j < 16 {
        if !snippet_ok(j) {
            ...
            let mut k: i32 = 0;
            let mut t: i32 = 0;
            while t < nv { idx.b[k as usize] = av.b[t as usize]; k = k + 1; t = t + 1; }
