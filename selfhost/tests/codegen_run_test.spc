...
struct Buf4096 { pub b: [char; 4096] }
...
fn arithmetic() {
    ...
    run_exit("bitwise", "fn main() i32 { let a: i32 = 6 & 3; let b: i32 = 6 | 1; let c: i32 = 1 << 4; unsafe exit(a + b + c); }\n", 25);
...
@test
fn control_flow() {
    run_exit("while break", "fn main() i32 { let mut i: i32 = 0; while true { if i >= 5 { break; } i = i + 1; } unsafe exit(i); }\n", 5);
    run_exit("for continue", "fn main() i32 { let mut t: i32 = 0; for i in 0..5 { if i == 2 { continue; } t = t + i; } unsafe exit(t); }\n", 8);
    run_exit("nested for", "fn main() i32 { let mut t: i32 = 0; for i in 0..3 { for j in 0..3 { t = t + 1; } } unsafe exit(t); }\n", 9);
    run_exit("do while", "fn main() i32 { let mut i: i32 = 0; let mut s: i32 = 0; do { s = s + i; i = i + 1; } while i < 5; unsafe exit(s); }\n", 10);
    run_exit("do while runs once", "fn main() i32 { let mut n: i32 = 0; do { n = n + 7; } while false; unsafe exit(n); }\n", 7);
}

@test
fn recursion() {
    run_exit("fib + range sum",
...
fn structs_and_methods() {
    run_exit("method dispatch (&self / &mut self)",
        ...
        27);
    run_exit("nested struct field access",
...
fn mut_match_binding() {
    run_exit("mut binding: &mut self method + reassign",
