...
for target in macos linux windows; do
    ...
    if ! SC_LEAK_CHECK=fatal ./super-c lint --target="$target"; then
...
printf 'check: tests\n'
if ! SC_LEAK_CHECK=fatal ./super-c test --quiet; then
...
racelog=$(mktemp)
for prog in parallel_smoke race_hunt cancel_hunt queue_hunt io_hunt; do
