...
if ! files=$(SC_LEAK_CHECK=fatal ./super-c fmt src std ffi tests bench --check); then
    printf 'check: FAILED -- these files are not canonically formatted:\n' >&2
    printf '%s\n' "$files" | sed 's/^/  /' >&2
    printf 'fix with: ./super-c fmt src std ffi tests bench\n' >&2
    exit 1
fi
...
for target in macos linux windows; do
    ...
    if ! SC_LEAK_CHECK=fatal ./super-c lint src std ffi tests bench --target="$target"; then
