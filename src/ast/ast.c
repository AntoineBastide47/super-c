...
NodeList ast_commit(Ast *a, uint32_t mark) {
  NodeList list = {(uint32_t)a->children.len, (uint32_t)a->scratch.len - mark};
...
static const char *kind_name(NodeKind kind) {
  static const char *const names[] = {
      "None", "Program", "Identifier", "Literal", "Function", "Parameter", "Struct", "Field", "Enum", "Variant",
      "Trait", "Impl", "TypeAlias", "Const", "ExternBlock", "GenericParam", "WherePredicate", "TypePath",
      "PointerType", "ReferenceType", "SliceType", "ArrayType", "FunctionType", "Block", "Let", "Return", "Break",
      "Continue", "Defer", "If", "While", "For", "ExpressionStatement", "Unary", "Binary", "Assignment", "Call",
      "Index", "Member", "Cast", "GenericSpecialization", "Match", "MatchArm", "New", "StructInitializer",
      "FieldInitializer", "PatternWildcard", "PatternLiteral", "PatternName", "PatternTuple", "PatternStruct",
      "PatternField", "PatternRange",
  };
...
static void print_node(FILE *out, const Ast *a, NodeId id, const char *source, unsigned depth);

static void print_list(FILE *out, const Ast *a, NodeList list, const char *source, unsigned depth) {
  const NodeId *ids = ast_list(a, list);
  for (uint32_t i = 0; i < list.len; i++)
    print_node(out, a, ids[i], source, depth);
}

static void print_child(FILE *out, const Ast *a, NodeId id, const char *source, unsigned depth) {
  if (id != NODE_NONE)
...
static void print_node(FILE *out, const Ast *a, NodeId id, const char *source, unsigned depth) {
  ...
  switch (n->kind) {
    case NODE_PROGRAM: print_list(out, a, n->as.program.items, source, depth + 1); break;
    case NODE_FUNCTION:
      ...
      print_child(out, a, n->as.function.return_type, source, depth + 1);
    ...
    case NODE_FIELD:
      ...
      break;
    case NODE_STRUCT: case NODE_ENUM:
    ...
    case NODE_TYPE_PATH:
      ...
      break;
    case NODE_POINTER_TYPE: case NODE_REFERENCE_TYPE: case NODE_SLICE_TYPE:
      print_child(out, a, n->as.indirect_type.type, source, depth + 1); break;
    ...
    case NODE_FUNCTION_TYPE:
      ...
      print_child(out, a, n->as.function_type.return_type, source, depth + 1);
      break;
    case NODE_BLOCK: print_list(out, a, n->as.block.statements, source, depth + 1); break;
    case NODE_LET:
      ...
      break;
    case NODE_RETURN: case NODE_DEFER: case NODE_EXPRESSION_STATEMENT:
      print_child(out, a, n->as.single.value, source, depth + 1); break;
    case NODE_IF:
    ...
    case NODE_FOR:
      ...
      break;
    case NODE_UNARY: print_child(out, a, n->as.unary.operand, source, depth + 1); break;
    case NODE_BINARY: case NODE_ASSIGNMENT:
    ...
    case NODE_FIELD_INITIALIZER:
      ...
      break;
    case NODE_PATTERN_NAME: case NODE_PATTERN_TUPLE: case NODE_PATTERN_STRUCT: case NODE_PATTERN_FIELD:
