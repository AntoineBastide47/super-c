...
static int cmd_status(const int st) {
  if (st < 0)
    return 1;
#if defined(_WIN32)
  return st;
...
static void sc_mkdir(const char *const path) {
#if defined(_WIN32)
  mkdir(path);
#else
  mkdir(path, 0775);
...
static void ext_c_collect(Package *p, ExtC *x, char ***kept, size_t *nkept, size_t *kcap, bool *keep_ok) {
  ...
  for (size_t m = 0; m < p->count; m++) {
    ...
    for (size_t i = 0; i < a->attrs.len; i++) {
      ...
      if (!sc_realpath(rel, rsl, sizeof rsl)) {
    ...
    for (uint32_t i = 0; i < items.len; i++) {
      ...
      if (!sc_realpath(rel, habs, sizeof habs))
      ...
      if (sc_realpath(rel, cabs, sizeof cabs))
...
#if defined(_WIN32)
  const char *const bslash = strrchr(path, '\\');
  if (!slash || bslash > slash)
    slash = bslash;
#endif
