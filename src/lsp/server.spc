...
pub fn run(std_dir: str, target: i32) i32 {
    ...
    loop {
        ...
        if method == "initialize" {
            ...
            sv.on_semantic_tokens(&req, fout);
...
extend Server {
    ...
    fn on_initialize(self: &mut Self, req: &json::JSON, f: *mut stdio::FILE) {
        ...
        switch bman::load("build.toml") {
            Some(man) => {
                ...
                self.out_skip = String::from_str(man.out_dir.as_str());
            },
            None => {},
        };
                ...
                switch params.value("contentChanges") {
                    Some(ch) => {
                        if ch.is_array() && ch.size() != 0 {
                            ...
                            self.docs[di as usize].txt = String::from_str(last.value_str("text"));
    ...
    fn on_references(self: &Self, req: &json::JSON, f: *mut stdio::FILE) {
        ...
        let locs = feat::references(&self.roots.at(h.r).pkg, h.m, h.off, include_decl);
    ...
    fn on_rename(self: &Self, req: &json::JSON, f: *mut stdio::FILE) {
        ...
        let def_file = self.self_def_file(h.r, dm as usize);
        let wr = self.ws_root.as_str();
        ...
        for i in 0..locs.len() {
            let l = locs.at(i);
            let uri = text::path_to_uri(self.roots.at(h.r).files.at(l.module as usize).as_str());
            let src = self.roots.at(h.r).pkg.modules.at(l.module as usize).source.as_str();
            let ls = text::line_starts(src);
    ...
    fn on_semantic_tokens(self: &Self, req: &json::JSON, f: *mut stdio::FILE) {
        let nullv = json::JSON::default();
        let mut uri = "";
        switch req.value("params") {
            Some(params) => switch params.value("textDocument") {
    ...
    fn on_completion(self: &Self, req: &json::JSON, f: *mut stdio::FILE) {
        ...
        if dot || path2 {
            items = self.complete_via_probe(self.docs.at(di as usize).path.as_str(), txt, off, true);
        } else {
            let r = self.owning_root(self.docs.at(di as usize).path.as_str());
            let mut m: i32 = -1;
            if r >= 0 {
                m = self.root_module(r as usize, self.docs.at(di as usize).path.as_str());
            }
            if m >= 0 && self.roots.at(r as usize).pkg.modules.at(m as usize).has_ast {
                ...
                items = self.complete_via_probe(self.docs.at(di as usize).path.as_str(), txt, off, false);
                if items.len() == 0 {
