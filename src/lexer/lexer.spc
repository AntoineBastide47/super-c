    ...
    while !is_eof(&*l) {
        ...
        if b == '\'' as u8 {
            if !malformed && count != 1 {
                if byte_character { lexer_error(&mut *l, "byte character literal must contain exactly one byte"); }
                else { lexer_error(&mut *l, "character literal must contain exactly one Unicode scalar value"); }
...
fn number(l: &mut Lexer) void {
    ...
    if unsafe l.bytes[l.start] == '0' as u8 {
        ...
        if radix != 10 {
            ...
            while i < l.len && is_id_part_byte(unsafe l.bytes[i]) {
                ...
                if digit(b) { saw_digit = true; }
                    ...
                    if error_at == USIZE_MAX {
                        error_at = i;
                        if radix == 2 { error = "invalid digit in binary literal"; }
                        else if radix == 8 { error = "invalid digit in octal literal"; }
                        else { error = "invalid digit in hexadecimal literal"; }
                    }
                }
                i = i + 1;
            }
            ...
            if !hex_float && peek_byte(&*l) == '.' as u8 {
                if error_at == USIZE_MAX {
                    error_at = l.current;
                    if radix == 16 { error = "a hexadecimal float needs a fraction digit and a binary exponent: 0x1.8p3"; }
                    else { error = "octal and binary floating-point literals are not supported"; }
                }
                l.current = l.current + 1;
                while !is_eof(&*l) {
                    let b = peek_byte(&*l);
    ...
    if error_at != USIZE_MAX { error = "invalid numeric separator"; }
    if peek_byte(&*l) == '.' as u8 && peek_byte_n(&*l, 1) != '.' as u8 {
        is_float = true;
        l.current = l.current + 1;
        let fraction_start = l.current;
        digits(&mut *l, fraction_start, &mut error_at, is_dec);
        if error_at != USIZE_MAX && error.len() == 0 { error = "invalid numeric separator"; }
    }
...
fn scan_token(l: &mut Lexer) void {
    ...
    switch c {
        ...
        '#' => { let start = l.start; lexer_error_at(&mut *l, start, 1, "'#' is not valid in Super-C source; Super-C has no preprocessor"); return; },
        ...
        _ => {
            if c >= 0x80u8 {
                ...
                if size == 0 {
                    ...
                    let start = l.start;
                    if cp == 0xFEFF { lexer_error_at(&mut *l, start, size, "UTF-8 BOM is allowed only at the start of a file"); }
                    else { lexer_error_at(&mut *l, start, size, "identifiers may contain only ASCII letters, digits, and '_'"); }
                }
            } else {
