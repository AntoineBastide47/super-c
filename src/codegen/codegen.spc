        ...
        while i < end {
            ...
            if e == b'n' || e == b'r' || e == b't' || e == b'\\' || e == b'"' || e == b'\'' {
                ...
                if is_char {
                    ...
                    for kk in 0..bn {
                        self.emit_octal_escape(b[kk as usize] as u32);
                    }
                }
            } else {
                self.buf.push_byte(b'\\');
    ...
    fn emit_raw_c_string(self: &mut Self, content: tok::Span) void {
        ...
        while i < content.end {
            ...
            if b == b'"' || b == b'\\' {
                ...
                self.emit_octal_escape(b as u32);
            } else {
                self.buf.push_byte((b as i32) as u8);
            }
            i = i + 1;
            ...
            if self.package != null && cm as usize < unsafe (*self.package).modules.len() {
                if unsafe (*self.package).modules[cm as usize].file.len() != 0 {
                    ...
                    if ra != null && rr != null {
                        ...
                        if unsafe cstring::strncmp(&absb[0], &rootb[0], rl) == 0 && absb[rl] == '/' as char {
                            self.emit_rel_prefix();
                            self.emit_str("../");
                            self.emit_cstr(unsafe ((&absb[0]) as *const char + (rl + 1)));
                        } else {
