...
extend ConstEval {
    ...
    fn aggregate_layout(self: &mut Self, dm: ModuleId, dn: NodeId, env: *const LayoutEnv, depth: i32) Layout {
        ...
        for i in 0..fs.len {
            ...
            let mut ftn = fid;
            if !is_tuple {
                ftn = unsafe (*a).at_const(fid).as_data.field.ty;
            }
    ...
    fn ce_teq(self: &Self, ma: ModuleId, ta: TypeId, mb: ModuleId, tb: TypeId) bool {
        ...
        if a.kind == TypeKind::TYPE_INSTANCE {
            ...
            for i in 0..ia.n {
                if !self.ce_teq(ma, ia.args[i as usize], mb, ib.args[i as usize]) {
                    return false;
                }
            }
            return true;
    ...
    fn ce_subst_deep(self: &mut Self, f: *mut CeFrame, m: ModuleId, t: TypeId, depth: i32) TypeId {
        ...
        if y.kind == TypeKind::TYPE_INSTANCE {
            ...
            for i in 0..it.n {
                ...
                if na[i as usize] == TYPE_NONE {
                    return TYPE_NONE;
                }
                if na[i as usize] != it.args[i as usize] {
                    changed = true;
                }
            }
            if !changed {
    ...
    fn ce_field_count(self: &Self, dm: ModuleId, dn: NodeId) u32 {
        ...
        for i in 0..ms.len {
            let mid = unsafe ((*a).list(ms))[i as usize];
            if unsafe (*a).at_const(mid).kind == NodeKind::NODE_FIELD {
                n = n + 1;
            }
        }
        return n;
    ...
    fn ce_container_of(self: &Self, fm: ModuleId, fnode: NodeId, out: *mut NodeId) i32 {
        ...
        for i in 0..items.len {
            ...
            if ik == NodeKind::NODE_EXTEND {
                ms = unsafe (*a).at_const(iid).as_data.extend_def.items;
                is_ext = true;
            }
            else if ik == NodeKind::NODE_INTERFACE {
                ms = unsafe (*a).at_const(iid).as_data.interface_def.items;
            }
            else {
                continue;
            }
    ...
    fn ce_int_op(self: &mut Self, op: TokenType, l: CeVal, r: CeVal, tm: ModuleId, rt: TypeId, b: BuiltinType, ob: BuiltinType) CeVal {
        ...
        if uns {
            ...
            switch op {
                ...
                Pipe => { u = ul | ur; },
                Caret => { u = ul ^ ur; },
                LeftShift => { if r.as_data.i < 0 || r.as_data.i >= (bits as i64) {
                        self.ce_trap("shift out of range");
                        return cv_nil();
                    }
                    u = ul << (r.as_data.i as u64); },
                RightShift => { if r.as_data.i < 0 || r.as_data.i >= (bits as i64) {
                        self.ce_trap("shift out of range");
                        return cv_nil();
                    }
                    u = ul >> (r.as_data.i as u64); },
        ...
        switch op {
            Plus => { let o = add_ovf(li, ri); if o.ovf {
                    self.ce_trap("arithmetic overflow");
                    return cv_nil();
                }
                v = o.v; },
            Minus => { let o = sub_ovf(li, ri); if o.ovf {
                    self.ce_trap("arithmetic overflow");
                    return cv_nil();
                }
                v = o.v; },
            Star => { let o = mul_ovf(li, ri); if o.ovf {
                    self.ce_trap("arithmetic overflow");
                    return cv_nil();
                }
                v = o.v; },
    ...
    fn eval_str_literal(self: &mut Self, f: *mut CeFrame, m: ModuleId, id: NodeId) CeVal {
        ...
        if nb != 0 {
            ...
            unsafe (*bo).esz = 1;
            for k in 0..nb {
                unsafe (*self.obj_ptr(block)).slots.set(k as usize, CeVal {
                        kind: CV_INT, tm: 0, ty: Ast::builtin(BuiltinType::BT_U8), as_data: CeValAs { i: bytes.b[k as usize] as i64 }
                    });
            }
        }
        ...
        unsafe (*self.obj_ptr(so)).slots.set(ptr_i as usize, CeVal {
                kind: CV_PTR, tm: 0, ty: TYPE_NONE, as_data: CeValAs { p: CvPtr { obj: block, off: 0 } }
            });
        unsafe (*self.obj_ptr(so)).slots.set(len_i as usize, CeVal {
                kind: CV_INT, tm: 0, ty: Ast::builtin(BuiltinType::BT_USIZE), as_data: CeValAs { i: nb as i64 }
            });
...
extend ConstEval {
    ...
    fn ce_coerce(self: &mut Self, v: CeVal, wm: ModuleId, wt: TypeId) CeVal {
        ...
        if y.kind == TypeKind::TYPE_INSTANCE && v.kind == CV_AGG && v.ty != TYPE_NONE
            ...
            unsafe (*self.obj_ptr(so)).slots.set(0, CeVal {
                    kind: CV_PTR, tm: 0, ty: TYPE_NONE, as_data: CeValAs { p: CvPtr { obj: v.as_data.p.obj, off: 0 } }
                });
            unsafe (*self.obj_ptr(so)).slots.set(1, CeVal {
                    kind: CV_INT, tm: 0, ty: Ast::builtin(BuiltinType::BT_USIZE), as_data: CeValAs { i: arrlen }
                });
    ...
    fn ev_place(self: &mut Self, f: *mut CeFrame, m: ModuleId, id0: NodeId) ValRes {
        ...
        if n_kind == NodeKind::NODE_IDENTIFIER {
            ...
            if d.node == NODE_NONE {
                d = DefId { module: m, node: unsafe (*a).resolution(id) };
            }
            if d.module != m {
                return ValRes { ok: false };
            }
    ...
    fn ev(self: &mut Self, f: *mut CeFrame, m: ModuleId, id: NodeId) CeVal {
        ...
        if n.kind == NodeKind::NODE_IDENTIFIER {
            ...
            if dk == NodeKind::NODE_FUNCTION {
                return CeVal { kind: CV_FN, tm: m, ty: rt, as_data: CeValAs { fnv: CvFn { m: d.module, fn_id: d.node } } };
            }
        ...
        if n.kind == NodeKind::NODE_MEMBER {
            if n.as_data.member.path {
                ...
                if unsafe (*self.ast_ptr(d.module)).at_const(d.node).as_data.variant.payload.len > 0 {
                    return cv_nil();
                }
                ...
                unsafe (*self.obj_ptr(o)).slots.set(0, CeVal {
                        kind: CV_INT, tm: 0, ty: TYPE_NONE, as_data: CeValAs { i: vp.pos as i64 }
                    });
...
extend ConstEval {
    ...
    fn ev_struct_init(self: &mut Self, f: *mut CeFrame, m: ModuleId, id: NodeId) CeVal {
        ...
        if vd.node == NODE_NONE {
            vd = DefId { module: m, node: unsafe (*a).resolution(tn) };
        }
    ...
    fn pat_match(self: &mut Self, f: *mut CeFrame, m: ModuleId, pid: NodeId, v: CeVal, uns: bool, refobj: u32) i32 {
        ...
        if pk == NodeKind::NODE_PATTERN_RANGE {
            ...
            if start_n != NODE_NONE {
                let mut sn = start_n;
                if unsafe (*a).at_const(start_n).kind == NodeKind::NODE_PATTERN_LITERAL {
                    sn = unsafe (*a).at_const(start_n).as_data.single.value;
                }
            ...
            if end_n != NODE_NONE {
                let mut en = end_n;
                if unsafe (*a).at_const(end_n).kind == NodeKind::NODE_PATTERN_LITERAL {
                    en = unsafe (*a).at_const(end_n).as_data.single.value;
                }
        ...
        if pk == NodeKind::NODE_PATTERN_TUPLE {
            ...
            if d.node == NODE_NONE || unsafe (*self.ast_ptr(d.module)).at_const(d.node).kind != NodeKind::NODE_VARIANT {
                return -1;
            }
            let o = self.obj_ptr(v.as_data.p.obj);
            let vp = self.ce_variant_pos(d.module, d.node);
        ...
        if pk == NodeKind::NODE_PATTERN_STRUCT {
            ...
            if !unsafe (*da).at_const(d.node).as_data.variant.struct_payload {
                return -1;
            }
            let children = unsafe (*a).at_const(pid).as_data.pattern.children;
            for k in 0..children.len {
                ...
                if unsafe (*a).at_const(cid).kind != NodeKind::NODE_PATTERN_FIELD || pfname == NODE_NONE {
                    return -1;
                }
                let mut slot: i32 = -1;
    ...
    fn exec_match(self: &mut Self, f: *mut CeFrame, m: ModuleId, id: NodeId, out: *mut CeVal) Flow {
        ...
        for i in 0..arms.len {
            ...
            if unsafe (*a).at_const(body).kind == NodeKind::NODE_BLOCK {
    ...
    fn exec_expr_stmt(self: &mut Self, f: *mut CeFrame, m: ModuleId, id0: NodeId) Flow {
        ...
        loop {
            ...
            let op = unsafe (*a).at_const(id).as_data.unary.op;
            if (op != TokenType::Move && op != TokenType::Unsafe)
                || unsafe (*a).at_const(unsafe (*a).at_const(id).as_data.unary.operand).kind == NodeKind::NODE_BLOCK {
                break;
            }
            id = unsafe (*a).at_const(id).as_data.unary.operand;
        }
        ...
        switch k {
            NODE_BLOCK => {
                ...
                while j > dbase {
                    ...
                    let dv = unsafe (*f).defers[j as usize];
                    let mut ds = Flow::Ok;
                    if unsafe (*a).at_const(dv).kind == NodeKind::NODE_BLOCK {
                        ds = self.exec_stmt(f, m, dv);
                    }
                    else {
                        ds = self.exec_expr_stmt(f, m, dv);
            ...
            NODE_IF => {
                ...
                if c.as_data.i != 0 {
                    return self.exec_stmt(f, m, unsafe (*a).at_const(id).as_data.if_stmt.then_branch);
                }
            ...
            NODE_BREAK => {
                ...
                if lbl.end > lbl.start || unsafe (*a).at_const(id).as_data.flow.value != NODE_NONE {
                    return Flow::Bail;
                }
                return Flow::Break;
            },
    ...
    fn exec_let(self: &mut Self, f: *mut CeFrame, m: ModuleId, id: NodeId) Flow {
        ...
        if nmk == NodeKind::NODE_PATTERN_TUPLE {
            ...
            if unsafe (*a).at_const(value_n).kind == NodeKind::NODE_CALL {
                ...
                for i in 0..cr.n {
                    vals[i as usize] = cr.vals[i as usize];
                }
                nvals = cr.n as u32;
            }
            if nvals <= 1 {
                ...
                for i in 0..nbind {
                    vals[i as usize] = unsafe (*self.obj_ptr(tv.as_data.p.obj)).slots[i as usize];
                }
                nvals = nbind;
            }
            if nvals != nbind {
            ...
            for i in 0..nbind {
                let cid = unsafe ((*a).list(children))[i as usize];
                if vals[i as usize].kind == CV_NIL_K || !self.ce_bind(f, cid, vals[i as usize]) {
                    return xfail(f);
                }
            }
            return Flow::Ok;
    ...
    fn exec_for(self: &mut Self, f: *mut CeFrame, m: ModuleId, id: NodeId) Flow {
        ...
        if unsafe (*a).at_const(iter_n).kind == NodeKind::NODE_RANGE {
            ...
            loop {
                ...
                unsafe *slot = CeVal {
                    kind: CV_INT, tm: m, ty: et, as_data: CeValAs { i: v }
                };
                let st = self.exec_stmt(f, m, body);
                if st == Flow::Break {
                    return Flow::Ok;
...
extend ConstEval {
    ...
    fn ce_bind_extend(self: &Self, g: *mut CeFrame, xm: ModuleId, extnode: NodeId, recv: *const CeRecv) bool {
        ...
        if unsafe (*xa).at_const(target).kind != NodeKind::NODE_TYPE_PATH
            || unsafe (*xa).at_const(target).as_data.type_path.args.len != (unsafe (*recv).n as u32) {
            return false;
        }
        ...
        for i in 0..unsafe (*recv).n {
            ...
            if ad.node != NODE_NONE && (ad.module as usize) < self.nmods
                ...
                if at2 == TYPE_NONE || !self.ce_teq(xm, at2, unsafe (*recv).am[i as usize], unsafe (*recv).at[i as usize]) {
                    return false;
                }
            }
        }
    ...
    fn ce_intercept(self: &mut Self, fm: ModuleId, fnode: NodeId, args: *const CeVal, nargs: u32, m: ModuleId, callId: NodeId) Rets {
        ...
        if self.ce_span_is(fm, nm, "memset") {
            ...
            let mut fill = cv_nil();
            if esz == 1 {
                fill = CeVal { kind: CV_INT, tm: 0, ty: Ast::builtin(BuiltinType::BT_U8), as_data: CeValAs { i: unsafe args[1].as_data.i & 0xff } };
            }
            else {
    ...
    fn ce_invoke(self: &mut Self, fm: ModuleId, fnode: NodeId, extend_node: NodeId, recv: *const CeRecv,
        ...
        if !closure && unsafe (*fa).at_const(fnode).kind != NodeKind::NODE_FUNCTION {
            return out;
        }
        let mut params = NodeList { start: 0, len: 0 };
        let mut body = NODE_NONE;
        if closure {
            params = unsafe (*fa).at_const(fnode).as_data.closure.params;
            body = unsafe (*fa).at_const(fnode).as_data.closure.body;
        }
        else {
            params = unsafe (*fa).at_const(fnode).as_data.function.params;
            body = unsafe (*fa).at_const(fnode).as_data.function.body;
        }
        ...
        if extend_node != NODE_NONE {
            ...
            if !bound {
                ...
                for i in 0..xg.len {
                    if unsafe monot[i as usize] == TYPE_NONE || !unsafe (*self.ast_ptr(unsafe monom[i as usize])).type_concrete(unsafe monot[i as usize]) {
                        return out;
                    }
                    if !self.ce_subst_add(gp, fm, unsafe gids[i as usize], unsafe monom[i as usize], unsafe monot[i as usize]) {
                        return out;
                    }
                }
            }
        ...
        if !closure {
            ...
            if fg.len != 0 {
                ...
                for i in 0..fg.len {
                    ...
                    if unsafe monot[idx] == TYPE_NONE || !unsafe (*self.ast_ptr(unsafe monom[idx])).type_concrete(unsafe monot[idx]) {
                        return out;
                    }
        ...
        if cacheable {
            for i in 0..nargs {
                ck.kinds[i as usize] = unsafe args[i as usize].kind;
                ck.bits[i as usize] = unsafe args[i as usize].as_data.i;
            }
            if let Some(hit) = self.calls.get(&ck) {
                out.n = hit.nrets;
                for j in 0..hit.nrets {
                    out.vals[j as usize] = hit.rets[j as usize];
                }
                out.ok = true;
                return out;
            }
        ...
        for j in 0..unsafe (*gp).nrets {
            out.vals[j as usize] = unsafe (*gp).rets[j as usize];
        }
        if cacheable && out.n <= 8 {
            let mut pure = true;
            let mut k: u8 = 0;
            while pure && k < out.n {
                pure = cv_is_scalar(out.vals[k as usize]);
                k = k + 1;
            }
            if pure && self.calls.len() < CE_CALLS_MAX {
                let mut hit = CeCallHit { nrets: out.n };
                for z in 0..out.n {
                    hit.rets[z as usize] = out.vals[z as usize];
                }
    ...
    fn ce_call(self: &mut Self, f: *mut CeFrame, m: ModuleId, id: NodeId) Rets {
            ...
            if is_path {
                if obj_n != NODE_NONE && self.ce_type(m, obj_n) != TYPE_NONE {
                    ...
                    if recv_syn.node == NODE_NONE {
                        recv_syn = DefId { module: m, node: unsafe (*a).resolution(obj_n) };
                    }
                }
            } else {
                recv_expr = obj_n;
        ...
        if fkind == NodeKind::NODE_FUNCTION && unsafe (*self.ast_ptr(fam)).at_const(fnode).as_data.function.is_extern {
            ...
            for i in 0..nargs {
                ...
                if argv[i as usize].kind == CV_NIL_K {
                    return out;
                }
            }
        ...
        let mut params = NodeList { start: 0, len: 0 };
        if fkind == NodeKind::NODE_CLOSURE {
            params = unsafe (*self.ast_ptr(fam)).at_const(fnode).as_data.closure.params;
        }
        else {
            params = unsafe (*self.ast_ptr(fam)).at_const(fnode).as_data.function.params;
        }
        if recv_expr != NODE_NONE {
            ...
            if want_ref || du != null {
                ...
                if recv_is_ptr {
                    ...
                    if argv[na as usize].kind != CV_PTR {
                        return out;
                    }
                } else {
                    let pr = self.ev_place(f, m, recv_expr);
                ...
                if argv[na as usize].kind == CV_NIL_K {
                    return out;
                }
            }
            if du != null {
                for hi in 0..unsafe (*du).n {
                    ...
                    if !hrr.ok || self.ce_container_of(unsafe (*du).method[hi as usize].module, unsafe (*du).method[hi as usize].node, (&mut hext) as *mut NodeId) != 1 {
                        return out;
                    }
