...
pub struct Buf96 { pub b: [char; 96] }
pub struct Buf512 { pub b: [char; 512] }
pub struct Defs4 { pub d: [DefId; 4] }
pub struct Tys4 { pub t: [TypeId; 4] }
pub struct Defs8 { pub d: [DefId; 8] }
pub struct Tys8 { pub t: [TypeId; 8] }
pub struct BoundArr8 { pub b: [BoundIface; 8] }
pub struct Names14 { pub n: [*const char; 16] }
pub struct Steps16 { pub s: [PStep; 16] }
pub struct Keep256 { pub k: [bool; 256] }
pub struct Cover4 { pub c: [u64; 4] }
pub struct Buf128 { pub b: [char; 128] }
pub struct NodeArr16 { pub n: [NodeId; 16] }
...
pub struct BoundIface { pub iface: DefId, pub n: u8, pub args: [TypeId; 4] }
...
pub struct ClosScope { pub node: NodeId } // (clos_stack element wrapper not needed; use [NodeId;8])
...
fn ph_lookup(pkg: usize, name: str) loader::LookupHit {
    ...
    if package == null { return loader::LookupHit { node: NODE_NONE, mid: 0 }; }
    return unsafe (*package).prelude_lookup(name, true);
}

// --- result structs (out-params) ----------------------------------------------------------------
pub struct FnSig { pub n: i32, pub params: [TypeId; 4], pub ret: TypeId }
pub struct SliceKind { pub kind: i32, pub elem: TypeId }
pub struct BoxOf { pub ok: bool, pub inner: TypeId, pub global_alloc: bool }
pub struct RecvSubst { pub n: i32, pub p: [DefId; 4], pub a: [TypeId; 4] }
...
fn builtin_name(b: BuiltinType) str {
    if b == BuiltinType::BT_BOOL { return "bool"; }
    if b == BuiltinType::BT_CHAR { return "char"; }
    if b == BuiltinType::BT_I8 { return "i8"; }
    if b == BuiltinType::BT_I16 { return "i16"; }
    if b == BuiltinType::BT_I32 { return "i32"; }
    if b == BuiltinType::BT_I64 { return "i64"; }
    if b == BuiltinType::BT_ISIZE { return "isize"; }
    if b == BuiltinType::BT_U8 { return "u8"; }
    if b == BuiltinType::BT_U16 { return "u16"; }
    if b == BuiltinType::BT_U32 { return "u32"; }
    if b == BuiltinType::BT_U64 { return "u64"; }
    if b == BuiltinType::BT_USIZE { return "usize"; }
    if b == BuiltinType::BT_F32 { return "f32"; }
    if b == BuiltinType::BT_F64 { return "f64"; }
    if b == BuiltinType::BT_C32 { return "c32"; }
    if b == BuiltinType::BT_C64 { return "c64"; }
    if b == BuiltinType::BT_VALIST { return "va_list"; }
    return "void";
}

fn builtin_of(src: *const u8, s: tok::Span) i32 {
    for i in 0..(BuiltinType::BT_COUNT as i32) {
        if span_is(src, s, builtin_name(i as BuiltinType)) { return i; }
    }
    return -1;
}

fn bt_is_int(b: BuiltinType) bool { return (b as u8) >= (BuiltinType::BT_I8 as u8) && (b as u8) <= (BuiltinType::BT_USIZE as u8); }
fn bt_is_float(b: BuiltinType) bool { return b == BuiltinType::BT_F32 || b == BuiltinType::BT_F64; }
fn bt_is_complex(b: BuiltinType) bool { return b == BuiltinType::BT_C32 || b == BuiltinType::BT_C64; }

fn bt_int_max(b: BuiltinType) u64 {
    if b == BuiltinType::BT_I8 { return 127u64; }
    if b == BuiltinType::BT_I16 { return 32767u64; }
    if b == BuiltinType::BT_I32 { return 2147483647u64; }
    if b == BuiltinType::BT_I64 || b == BuiltinType::BT_ISIZE { return 9223372036854775807u64; }
    if b == BuiltinType::BT_U8 { return 255u64; }
    if b == BuiltinType::BT_U16 { return 65535u64; }
    if b == BuiltinType::BT_U32 { return 4294967295u64; }
...
fn tc_lit_in_range(b: BuiltinType, mag: u64, neg: bool) bool {
    let mx = bt_int_max(b);
    if neg {
        let sgn = b == BuiltinType::BT_I8 || b == BuiltinType::BT_I16 || b == BuiltinType::BT_I32
...
fn bt_widens(from: BuiltinType, to: BuiltinType) bool {
    if from == BuiltinType::BT_F32 && to == BuiltinType::BT_F64 { return true; }
    ...
    if !(fs || fu) || !(ts || tu) { return false; }
    let mut fw = (from as i32) - (BuiltinType::BT_U8 as i32);
    if fs { fw = (from as i32) - (BuiltinType::BT_I8 as i32); }
    let mut tw = (to as i32) - (BuiltinType::BT_U8 as i32);
    if ts { tw = (to as i32) - (BuiltinType::BT_I8 as i32); }
    if fu { return tw > fw; }
    return ts && tw > fw;
}
...
fn lit_base_prefix(p: *const u8, len: usize) (u64, usize) {
    if len >= 2 && unsafe p[0] == '0' as u8 {
        let c1 = unsafe p[1] | 0x20u8;
        if c1 == 'x' as u8 { return 16u64, 2; }
        if c1 == 'b' as u8 { return 2u64, 2; }
        if c1 == 'o' as u8 { return 8u64, 2; }
    }
    return 10u64, 0;
}

fn hex_digit(c: u8) u32 {
    if c <= '9' as u8 { return (c - '0' as u8) as u32; }
    return ((c | 0x20u8) - 'a' as u8 + 10u8) as u32;
}
...
extend TypeChecker {
    ...
    fn cur_ast(self: &Self) *mut Ast { return (&self.ast) as *mut Ast; }
    fn cur_module(self: &Self) ModuleId { return self.ast.module; }
    fn has_pkg(self: &Self) bool { return self.package != null; }
    ...
    fn at_not_fn(self: &Self, x: TypeId) bool { return self.type_at(x).kind != TypeKind::TYPE_FUNCTION; }
    ...
    fn is_numeric(self: &Self, x: TypeId) bool {
        let y = self.type_at(x);
        return y.kind == TypeKind::TYPE_BUILTIN && (bt_is_int(y.as_data.builtin) || bt_is_float(y.as_data.builtin) || bt_is_complex(y.as_data.builtin));
    }
    ...
    fn is_plain_enum(self: &Self, x: TypeId) bool {
        let y = *self.type_at(x);
        if y.kind != TypeKind::TYPE_ENUM { return false; }
        ...
        for i in 0..ms.len {
            ...
            if unsafe (*a).at_const(mid).as_data.variant.payload.len > 0 { return false; }
        }
        return true;
    }
    ...
    fn tc_ref(self: &mut Self, elem: TypeId, mut2: bool) TypeId {
        ...
        if mut2 { q = TypeQualifier::TYPE_QUAL_MUT; }
    ...
    fn tc_loop_push(self: &mut Self, label: tok::Span, node: NodeId, value_loop: bool) i32 {
        ...
        let n = self.nloops;
        self.loop_stack[n as usize] = LoopEntry { label: label, node: node, break_ty: TYPE_NONE, value_loop: value_loop, saw_value: false, saw_bare: false };
        self.nloops = n + 1;
        return n as i32;
    }
    fn tc_loop_pop(self: &mut Self, le: i32, sp: tok::Span) void {
        ...
        if self.loop_stack[le as usize].saw_value && self.loop_stack[le as usize].saw_bare {
            self.errors.emit(sp.start, sp.end - sp.start, format("every 'break' in a value-yielding 'loop' must carry a value"));
        }
    ...
    fn tc_is_test_fn(self: &Self, m: ModuleId, fnode: NodeId) bool {
        ...
        for i in 0..unsafe (*a).attrs.len() {
            ...
            if at.owner == fnode && (at.kind == (AttrKind::ATTR_TEST as u8) || at.kind == (AttrKind::ATTR_TEST_INIT as u8) || at.kind == (AttrKind::ATTR_TEST_FREE as u8)) { return true; }
        }
        return false;
    }
    ...
    fn tc_check_test_ref(self: &mut Self, d: DefId, sp: tok::Span) void {
        ...
        self.errors.emit(sp.start, sp.end - sp.start, format("a '@test' function can only be called from other test functions"));
        self.errors.note(format("test functions are not compiled outside 'super-c --test'; move shared logic into a plain function"));
    }

    // ---- literal helpers ----
    fn tc_literal_pinned(self: &Self, id: NodeId) bool {
        let n = unsafe (*self.cur_ast()).at_const(id);
        if n.kind != NodeKind::NODE_LITERAL { return false; }
        let tt = n.as_data.literal.token_type;
        if tt != TokenType::IntegerLiteral && tt != TokenType::FloatLiteral { return false; }
    ...
    fn is_integer_literal_node(self: &Self, id: NodeId) bool {
        ...
        if n0.kind == NodeKind::NODE_UNARY && n0.as_data.unary.op == TokenType::Minus { nid = n0.as_data.unary.operand; }
    ...
    fn tc_literal_u64(self: &Self, id: NodeId) BoxOf { // reuse BoxOf as {ok, inner=mag(as TypeId? no)} -> use a dedicated struct
    ...
    fn lit_mag(self: &Self, id: NodeId, out: *mut u64) bool {
        ...
        let mut p = unsafe (self.source + lr.start as usize);
        ...
        for i in 0..len {
            let ch = unsafe p[i];
            if ch == '_' as u8 { continue; }
            let mut d: u64 = 0;
            if ch <= '9' as u8 { d = (ch - '0' as u8) as u64; } else { d = ((ch | 0x20u8) - 'a' as u8 + 10u8) as u64; }
            if d >= base || acc > (0xFFFFFFFFFFFFFFFFu64 - d) / base { return false; }
            acc = acc * base + d;
        }
        unsafe *out = acc;
        return true;
    ...
    fn char_literal_cp(self: &Self, s: tok::Span) u32 {
        ...
        if i >= s.end as usize { return 0; }
        if unsafe src[i] != '\\' as u8 {
            let b = unsafe src[i];
            if b < 0x80u8 { return b as u32; }
            if b <= 0xDFu8 { return (((b & 0x1Fu8) as u32) << 6) | ((unsafe src[i + 1] & 0x3Fu8) as u32); }
            if b <= 0xEFu8 { return (((b & 0x0Fu8) as u32) << 12) | (((unsafe src[i + 1] & 0x3Fu8) as u32) << 6) | ((unsafe src[i + 2] & 0x3Fu8) as u32); }
            return (((b & 0x07u8) as u32) << 18) | (((unsafe src[i + 1] & 0x3Fu8) as u32) << 12) | (((unsafe src[i + 2] & 0x3Fu8) as u32) << 6) | ((unsafe src[i + 3] & 0x3Fu8) as u32);
        }
        i = i + 1;
        let e = unsafe src[i];
        i = i + 1;
        if e == 'x' as u8 { return (hex_digit(unsafe src[i]) << 4) | hex_digit(unsafe src[i + 1]); }
        if e == 'u' as u8 {
            if i < s.end as usize && unsafe src[i] == '{' as u8 { i = i + 1; }
            let mut cp: u32 = 0;
            while i < s.end as usize && unsafe src[i] != '}' as u8 { cp = (cp << 4) | hex_digit(unsafe src[i]); i = i + 1; }
            return cp;
        }
        return 0;
    }
    ...
    fn through_raw_pointer(self: &Self, ty0: TypeId) bool {
        ...
        while y.kind == TypeKind::TYPE_REFERENCE || y.kind == TypeKind::TYPE_POINTER {
            if y.kind == TypeKind::TYPE_POINTER { return true; }
            ty = y.as_data.elem;
            y = self.type_at(ty);
        }
        return false;
    ...
    fn render_type(self: &Self, tid: TypeId, buf: *mut char, cap: usize) void {
        ...
        if ty.kind == TypeKind::TYPE_BUILTIN {
            unsafe stdio::snprintf(buf, cap, "%s".ptr() as *const char, builtin_name(ty.as_data.builtin).ptr() as *const char);
    ...
    fn err_mismatch(self: &mut Self, node: NodeId, expected: TypeId) void {
        ...
        self.errors.emit(sp.start, sp.end - sp.start, format("mismatched types: expected '{}', found '{}'", diag::cstr((&e.b[0]) as *const char), diag::cstr((&f.b[0]) as *const char)));
    }
    ...
    fn generic_fn_bound(self: &Self, m: ModuleId, decl: NodeId) NodeId {
        ...
        if gp.kind != NodeKind::NODE_GENERIC_PARAM { return NODE_NONE; }
        let bounds = gp.as_data.generic_param.bounds;
        for i in 0..bounds.len {
            let bid = unsafe ((*a).list(bounds))[i as usize];
            if unsafe (*a).at_const(bid).kind == NodeKind::NODE_FUNCTION_TYPE { return bid; }
        }
            ...
            for w in 0..wc.len {
                ...
                if unsafe (*self.cur_ast()).resolution(wp.ty) == decl {
                    for b in 0..wp.bounds.len {
                        ...
                        if unsafe (*self.cur_ast()).at_const(wbid).kind == NodeKind::NODE_FUNCTION_TYPE { return wbid; }
                    }
                }
            }
        }
    ...
    fn fn_owns(self: &mut Self, fid: TypeId) bool {
        ...
        for i in 0..caps.len {
            ...
            if ((mut_caps >> (i as u64)) & 1u64) == 0 {
                ...
                if self.tc_type_is_free(rt) { return true; }
            }
        }
        return false;
    }
    ...
    fn tc_mark_capture_mut(self: &mut Self, expr0: NodeId) void {
        if self.nclos == 0 { return; }
        let a = self.cur_ast();
        let mut expr = expr0;
        loop {
            let n = unsafe (*a).at_const(expr);
            if n.kind == NodeKind::NODE_UNARY && (n.as_data.unary.op == TokenType::Move || n.as_data.unary.op == TokenType::Unsafe) { expr = n.as_data.unary.operand; }
            else if n.kind == NodeKind::NODE_MEMBER && !n.as_data.member.path { expr = n.as_data.member.object; }
            else if n.kind == NodeKind::NODE_INDEX { expr = n.as_data.index.object; }
            else { break; }
        }
        if unsafe (*a).at_const(expr).kind != NodeKind::NODE_IDENTIFIER { return; }
        let d = unsafe (*a).resolution_def(expr);
        if d.module != self.ast.module || d.node == NODE_NONE { return; }
    ...
    fn fn_compatible(self: &mut Self, exid: TypeId, acid: TypeId) bool {
        if exid != acid && self.fn_is_capturing(acid) { return false; }
    ...
    fn dynfn_sig_ok(self: &mut Self, exid: TypeId, acid: TypeId) bool {
        ...
        let en = self.fn_sig(exid, (&mut ep.t[0]) as *mut TypeId, 4, (&mut er) as *mut TypeId);
        let an = self.fn_sig(acid, (&mut ap.t[0]) as *mut TypeId, 4, (&mut ar) as *mut TypeId);
        if en != an || en > 4 || !self.ret_eq(er, ar) { return false; }
        for i in 0..en { if ep.t[i as usize] != ap.t[i as usize] { return false; } }
        return true;
    }

    fn tc_dyn_fn_sig(self: &mut Self, ty: &Ty) TypeId {
        if ty.kind != TypeKind::TYPE_DYN { return TYPE_NONE; }
        if unsafe (*self.mod_ast(ty.module)).at_const(ty.as_data.decl).kind != NodeKind::NODE_FUNCTION_TYPE { return TYPE_NONE; }
    ...
    fn tc_dyn_same(self: &mut Self, a: &Ty, b: &Ty) bool {
        ...
        if (as2 != TYPE_NONE) != (bs != TYPE_NONE) { return false; }
        if as2 != TYPE_NONE { return as2 == bs || self.fn_compatible(as2, bs); }
        return a.module == b.module && a.as_data.decl == b.as_data.decl;
    }
}

fn if_node(c: bool, a: NodeId, b: NodeId) NodeId { if c { return a; } return b; }
fn if_ty(c: bool, a: TypeId, b: TypeId) TypeId { if c { return a; } return b; }
...
extend TypeChecker {
    ...
    fn aggregate_of(self: &Self, ty: TypeId, mod_out: *mut ModuleId, decl_out: *mut NodeId, params: *mut DefId, args: *mut TypeId, n_out: *mut i32) bool {
        unsafe *n_out = 0;
        ...
        if y.kind == TypeKind::TYPE_INSTANCE {
            ...
            let mut nn: i32 = 0;
            let mut i: u32 = 0;
            while i < gens.len && (i as u8) < it.n && nn < 4 {
    ...
    fn subst_type(self: &mut Self, ty: TypeId, params: *const DefId, args: *const TypeId, n: i32) TypeId {
        if ty == TYPE_NONE || n == 0 { return ty; }
        let y = *self.type_at(ty);
        if y.kind == TypeKind::TYPE_GENERIC {
            for i in 0..n {
                if unsafe params[i as usize].module == y.module && unsafe params[i as usize].node == y.as_data.decl { return unsafe args[i as usize]; }
            }
            return ty;
        }
        ...
        if y.kind == TypeKind::TYPE_INSTANCE {
            ...
            for i in 0..src.n {
                na.t[i as usize] = self.subst_type(src.args[i as usize], params, args, n);
                if na.t[i as usize] != src.args[i as usize] { changed = true; }
            }
    ...
    fn unify_infer(self: &mut Self, param_ty: TypeId, arg_ty: TypeId, params: *const DefId, bound: *mut TypeId, n: i32) void {
        if param_ty == TYPE_NONE || arg_ty == TYPE_NONE { return; }
        ...
        if p.kind == TypeKind::TYPE_GENERIC {
            for i in 0..n {
                if unsafe params[i as usize].module == p.module && unsafe params[i as usize].node == p.as_data.decl {
                    if unsafe bound[i as usize] == TYPE_NONE { unsafe bound[i as usize] = arg_ty; }
                    return;
                }
            }
            return;
        ...
        if aT.kind == p.kind && (p.kind == TypeKind::TYPE_POINTER || p.kind == TypeKind::TYPE_REFERENCE || p.kind == TypeKind::TYPE_SLICE || p.kind == TypeKind::TYPE_ARRAY) {
            ...
            if pi.decl == ai.decl && pi.module == ai.module && pi.n == ai.n {
                for i in 0..pi.n { self.unify_infer(pi.args[i as usize], ai.args[i as usize], params, bound, n); }
            }
            ...
            let mut ar: TypeId = TYPE_NONE;
            let pn = self.fn_sig(param_ty, (&mut pp.t[0]) as *mut TypeId, 4, (&mut pr) as *mut TypeId);
            let an = self.fn_sig(arg_ty, (&mut ap.t[0]) as *mut TypeId, 4, (&mut ar) as *mut TypeId);
            if pn == an && pn <= 4 {
                for i in 0..pn { self.unify_infer(pp.t[i as usize], ap.t[i as usize], params, bound, n); }
    ...
    fn named_type_of(self: &mut Self, m: ModuleId, decl: NodeId) TypeId {
        ...
        if dk == NodeKind::NODE_STRUCT { return unsafe (*self.cur_ast()).intern_type(Ty { kind: TypeKind::TYPE_STRUCT, module: m, as_data: TyAs { decl: decl } }); }
        if dk == NodeKind::NODE_ENUM { return unsafe (*self.cur_ast()).intern_type(Ty { kind: TypeKind::TYPE_ENUM, module: m, as_data: TyAs { decl: decl } }); }
        ...
        if dk == NodeKind::NODE_GENERIC_PARAM || dk == NodeKind::NODE_INTERFACE {
            return unsafe (*self.cur_ast()).intern_type(Ty { kind: TypeKind::TYPE_GENERIC, module: m, as_data: TyAs { decl: decl } });
        }
        return TYPE_ERROR;
    }
    ...
    fn apply_default_args(self: &mut Self, dmod: ModuleId, dn: NodeId, ta: *mut TypeId, tn: *mut u8) void {
        let da = self.mod_ast(dmod);
        let gens = unsafe (*da).at_const(dn).as_data.aggregate.generics;
        if unsafe *tn >= (gens.len as u8) { return; }
        let mut i = (unsafe *tn) as u32;
        while i < gens.len && unsafe *tn < 4 {
            let gid = unsafe ((*da).list(gens))[i as usize];
            let dft = unsafe (*da).at_const(gid).as_data.generic_param.default_type;
            if dft == NODE_NONE { break; }
            let mut d = self.lower_type_in(dmod, dft);
            if unsafe *tn > 0 {
                let mut prm = Defs4 {};
                for j in 0..unsafe *tn {
                    let gj = unsafe ((*da).list(gens))[j as usize];
                    prm.d[j as usize] = DefId { module: dmod, node: gj };
                }
                d = self.subst_type(d, (&prm.d[0]) as *const DefId, ta as *const TypeId, (unsafe *tn) as i32);
            }
            let k = unsafe *tn;
    ...
    fn prelude_str_type(self: &mut Self) TypeId {
        ...
        if self.ph_str.node != NODE_NONE { return self.named_type_of(self.ph_str.mid, self.ph_str.node); }
        return TYPE_ERROR;
    }
    fn prelude_slice_type(self: &mut Self, elem: TypeId, mut2: bool) TypeId {
        if self.package == null { return TYPE_ERROR; }
        let mut hit = self.ph_slice;
        if mut2 { hit = self.ph_slicemut; }
    ...
    fn prelude_range_type(self: &mut Self, elem: TypeId) TypeId {
        if self.package == null { return TYPE_ERROR; }
        if self.ph_range.node != NODE_NONE { return unsafe (*self.cur_ast()).intern_instance(self.ph_range.mid, self.ph_range.node, (&elem) as *const TypeId, 1); }
        return TYPE_ERROR;
    }
    fn prelude_tuple_type(self: &mut Self, args: *const TypeId, n: u32) TypeId {
        ...
        if n == 3 { hit = self.ph_t3; }
        else if n == 4 { hit = self.ph_t4; }
    ...
    fn prelude_instance_args_hit(self: &Self, tid: TypeId, hit: loader::LookupHit, out: *mut TypeId, maxn: i32) i32 {
        if self.package == null || tid == TYPE_NONE || hit.node == NODE_NONE { return -1; }
        let ty = self.type_at(tid);
        if ty.kind != TypeKind::TYPE_INSTANCE { return -1; }
        let it = *unsafe (*self.cur_ast()).instance(ty.as_data.inst);
        if it.module != hit.mid || it.decl != hit.node { return -1; }
        let mut i: i32 = 0;
        while i < (it.n as i32) && i < maxn { unsafe out[i as usize] = it.args[i as usize]; i = i + 1; }
        return it.n as i32;
    }
    ...
    fn range_instance_elem(self: &Self, tid: TypeId) TypeId {
        ...
        let it = *unsafe (*self.cur_ast()).instance(ty.as_data.inst);
        if it.n == 1 && it.module == self.ph_range.mid && it.decl == self.ph_range.node { return it.args[0]; }
        return TYPE_NONE;
    }
    ...
    fn slice_kind(self: &Self, tid: TypeId, elem: *mut TypeId) i32 {
        ...
        if it.n != 1 { return 0; }
        let mut kind: i32 = 0;
        if it.module == self.ph_slice.mid && it.decl == self.ph_slice.node { kind = 1; }
        else if it.module == self.ph_slicemut.mid && it.decl == self.ph_slicemut.node { kind = 2; }
        if kind != 0 && elem != null { unsafe *elem = it.args[0]; }
        return kind;
    }
    fn tc_box_of(self: &Self, y: &Ty, inner: *mut TypeId, global_alloc: *mut bool) bool {
        ...
        let it = *unsafe (*self.cur_ast()).instance(y.as_data.inst);
        if it.module != self.ph_box.mid || it.decl != self.ph_box.node || it.n < 1 { return false; }
    ...
    fn decl_type_in(self: &mut Self, m: ModuleId, decl: NodeId) TypeId {
        if decl == NODE_NONE { return TYPE_NONE; }
        let local = self.package == null || m == self.ast.module;
        if local {
            let cached = unsafe (*self.cur_ast()).type_of(decl);
            if cached != TYPE_NONE { return cached; }
        }
        let a = self.mod_ast(m);
        let dk = unsafe (*a).at_const(decl).kind;
        let mut result = TYPE_NONE;
        if dk == NodeKind::NODE_PARAMETER { result = self.lower_type_in(m, unsafe (*a).at_const(decl).as_data.parameter.ty); }
        else if dk == NodeKind::NODE_FIELD { result = self.lower_type_in(m, unsafe (*a).at_const(decl).as_data.field.ty); }
    ...
    fn decl_type(self: &mut Self, decl: NodeId) TypeId { return self.decl_type_in(self.ast.module, decl); }

    fn type_of_type_node(self: &mut Self, id: NodeId) TypeId {
        if id == NODE_NONE { return TYPE_NONE; }
        if unsafe (*self.cur_ast()).at_const(id).kind != NodeKind::NODE_IDENTIFIER { return self.resolve_type(id); }
    ...
    fn ce_array_len(self: &mut Self, m: ModuleId, lenNode: NodeId) u32 {
        let ceptr = self.ceval();
        if ceptr == null { return 0; }
        let lv = unsafe (*ceptr).eval(m, lenNode);
        if lv.kind == ce::CONST_INT && lv.as_data.i > 0 && lv.as_data.i <= (0xFFFFFFFFi64) { return lv.as_data.i as u32; }
        return 0;
    }
    ...
    fn lower_type_in(self: &mut Self, m: ModuleId, id: NodeId) TypeId {
        if self.package == null || m == self.ast.module { return self.resolve_type(id); }
        if id == NODE_NONE { return TYPE_NONE; }
        let a = self.mod_ast(m);
        ...
        if nk == NodeKind::NODE_TYPE_PATH {
            ...
            if d.node != NODE_NONE {
                ...
                if bb >= 0 { return Ast::builtin(bb as BuiltinType); }
                ...
                if args.len == 1 && self.package != null {
                    ...
                    if an.kind == NodeKind::NODE_DYN_TYPE && an.as_data.indirect_type.qualifier == TypeQualifier::TYPE_QUAL_NONE {
                        if d.module == self.ph_box.mid && d.node == self.ph_box.node { return self.lower_type_in(m, a0); }
                    }
                }
                if (dnk == NodeKind::NODE_STRUCT || dnk == NodeKind::NODE_ENUM) && unsafe (*self.mod_ast(d.module)).at_const(d.node).as_data.aggregate.generics.len > 0 && (args.len > 0 || self.agg_has_default_at(d.module, d.node, args.len)) {
                    let mut ta = Tys4 {};
                    let mut tn: u8 = 0;
                    let mut i: u32 = 0;
                    while i < args.len && tn < 4 {
                        ...
                        if ta.t[tn as usize] != TYPE_NONE && self.type_at(ta.t[tn as usize]).kind == TypeKind::TYPE_ARRAY && self.type_at(ta.t[tn as usize]).as_data.arr.len == 0 {
                            ...
                            self.errors.emit(asp.start, asp.end - asp.start, format("a fixed-size array cannot be a generic type argument; use a slice '[]T' or wrap it in a struct"));
                    ...
                    return unsafe (*self.cur_ast()).intern_instance(d.module, d.node, (&ta.t[0]) as *const TypeId, tn);
                }
        ...
        if nk == NodeKind::NODE_TUPLE_TYPE {
            ...
            if elems.len > 4 { return TYPE_ERROR; }
            let mut targs = Tys4 {};
            for i in 0..elems.len { targs.t[i as usize] = self.lower_type_in(m, unsafe ((*a).list(elems))[i as usize]); }
            return self.prelude_tuple_type((&targs.t[0]) as *const TypeId, elems.len);
        }
        if nk == NodeKind::NODE_POINTER_TYPE || nk == NodeKind::NODE_REFERENCE_TYPE {
            ...
            return unsafe (*self.cur_ast()).intern_type(Ty { kind: k, qualifier: it.qualifier as u8, as_data: TyAs { elem: self.lower_type_in(m, it.ty) } });
        }
        if nk == NodeKind::NODE_ARRAY_TYPE {
            let at = unsafe (*a).at_const(id).as_data.array_type;
            let alen = self.ce_array_len(m, at.length);
            return unsafe (*self.cur_ast()).intern_type(Ty { kind: TypeKind::TYPE_ARRAY, as_data: TyAs { arr: TyArr { elem: self.lower_type_in(m, at.element), len: alen } } });
        }
        if nk == NodeKind::NODE_FUNCTION_TYPE { return unsafe (*self.cur_ast()).intern_type(Ty { kind: TypeKind::TYPE_FUNCTION, module: m, as_data: TyAs { decl: id } }); }
        if nk == NodeKind::NODE_DYN_TYPE {
            let it = unsafe (*a).at_const(id).as_data.indirect_type;
            let inner = it.ty;
            if unsafe (*a).at_const(inner).kind == NodeKind::NODE_FUNCTION_TYPE { return self.tc_intern_dynfn(m, inner, it.qualifier); }
            ...
            if d.node == NODE_NONE || unsafe (*self.mod_ast(d.module)).at_const(d.node).kind != NodeKind::NODE_INTERFACE { return TYPE_ERROR; }
            return unsafe (*self.cur_ast()).intern_type(Ty { kind: TypeKind::TYPE_DYN, qualifier: it.qualifier as u8, module: d.module, as_data: TyAs { decl: d.node } });
        }
        return TYPE_ERROR;
    }
        ...
        switch nk {
            NODE_TYPE_PATH => {
                ...
                while i < args.len && self.package != null {
                    ...
                    if args.len == 1 && hd.module == bh.mid && hd.node == bh.node {
                        ...
                        self.errors.emit(sp.start, sp.end - sp.start, format("a bare 'dyn' type can only be the generic argument of 'Box'"));
                    }
                ...
                if d.node != NODE_NONE {
                    ...
                    if bb >= 0 {
                        ...
                        if generic_agg && args.len == 0 && self.current_extend != NODE_NONE && d.module == self.ast.module && d.node == self.current_self {
                            ...
                            if dnk == NodeKind::NODE_INTERFACE && parts.len != 0 && !span_is(self.source, self.name_span(unsafe ((*a).list(parts))[0]), "Self") {
                                let isp = self.name_span(unsafe ((*a).list(parts))[0]);
                                self.errors.emit(isp.start, isp.end - isp.start, format("an interface is not a type; use '&dyn {}', 'Box<dyn {}>', or a generic bound", diag::span_str(self.source, isp.start, isp.end), diag::span_str(self.source, isp.start, isp.end)));
                            }
            ...
            NODE_DYN_TYPE => {
                ...
                if q == TypeQualifier::TYPE_QUAL_NONE {
                    let sp = unsafe (*a).at_const(id).span;
                    self.errors.emit(sp.start, sp.end - sp.start, format("a 'dyn' type must be '&dyn I', '&mut dyn I', or 'Box<dyn I>'"));
                } else {
    ...
    fn tc_intern_dynfn(self: &mut Self, m: ModuleId, sig: NodeId, qual: TypeQualifier) TypeId {
        ...
        loop {
            ...
            if idx >= self.dynfn_list.len() { break; }
            ...
            if esig == mysig || self.fn_compatible(esig, mysig) {
                return unsafe (*self.cur_ast()).intern_type(Ty { kind: TypeKind::TYPE_DYN, qualifier: qual as u8, module: e.module, as_data: TyAs { decl: e.as_data.decl } });
            }
        }
        return unsafe (*self.cur_ast()).intern_type(Ty { kind: TypeKind::TYPE_DYN, qualifier: qual as u8, module: m, as_data: TyAs { decl: sig } });
    }
    ...
    fn resolve_dyn_node(self: &mut Self, id: NodeId, qual: TypeQualifier) TypeId {
        ...
        if unsafe (*a).at_const(inner).kind == NodeKind::NODE_FUNCTION_TYPE {
            ...
            if !concrete { self.errors.emit(sp.start, sp.end - sp.start, format("a 'dyn fn' is always called through a shared view; write '&dyn fn(..) ..'")); }
            ...
            else if qual != TypeQualifier::TYPE_QUAL_MUT { self.errors.emit(sp.start, sp.end - sp.start, format("a 'dyn fn' signature cannot name a generic parameter")); }
            unsafe (*self.cur_ast()).set_type(id, result);
            return result;
        }
        let mut d = DefId { module: 0, node: NODE_NONE };
    ...
    fn tc_mentions_self(self: &Self, imod: ModuleId, tn: NodeId, iface: DefId) bool {
        if tn == NODE_NONE { return false; }
        let ia = self.mod_ast(imod);
        let n = unsafe (*ia).at_const(tn);
        if n.kind == NodeKind::NODE_TYPE_PATH {
            let d = unsafe (*ia).resolution_def(tn);
            if d.module == iface.module && d.node == iface.node { return true; }
            let args = n.as_data.type_path.args;
            for i in 0..args.len { if self.tc_mentions_self(imod, unsafe ((*ia).list(args))[i as usize], iface) { return true; } }
            return false;
        }
        ...
        if n.kind == NodeKind::NODE_ARRAY_TYPE { return self.tc_mentions_self(imod, n.as_data.array_type.element, iface); }
        if n.kind == NodeKind::NODE_TUPLE_TYPE {
            let elems = n.as_data.array_literal.elements;
            for i in 0..elems.len { if self.tc_mentions_self(imod, unsafe ((*ia).list(elems))[i as usize], iface) { return true; } }
            return false;
        }
        if n.kind == NodeKind::NODE_FUNCTION_TYPE {
            let ft = n.as_data.function_type;
            for i in 0..ft.params.len { if self.tc_mentions_self(imod, unsafe ((*ia).list(ft.params))[i as usize], iface) { return true; } }
            for i in 0..ft.returns.len { if self.tc_mentions_self(imod, unsafe ((*ia).list(ft.returns))[i as usize], iface) { return true; } }
            return false;
        }
        return false;
    }
    ...
    fn dyn_compatible(self: &mut Self, iface: DefId, at: tok::Span) bool {
        ...
        if idn.generics.len != 0 { why = "the interface has generic parameters"; }
        else if idn.bounds.len != 0 { why = "the interface has superinterfaces"; }
        let mut i: u32 = 0;
        while why.len() == 0 && i < idn.items.len {
            ...
            if self.dyn_method(iface.module, mid) {
                ...
                if m.generics.len != 0 { why = "a method has its own generic parameters"; }
                else if sk != NodeKind::NODE_REFERENCE_TYPE && sk != NodeKind::NODE_POINTER_TYPE { why = "a method takes 'Self' by value"; }
                else if m.returns.len > 1 { why = "a method has multiple return values"; }
                let mut p: u32 = 1;
                while why.len() == 0 && p < m.params.len {
                    let pid = unsafe ((*ia).list(m.params))[p as usize];
                    if self.tc_mentions_self(iface.module, unsafe (*ia).at_const(pid).as_data.parameter.ty, iface) { why = "a method mentions 'Self' outside the receiver"; }
                    p = p + 1;
                }
                let mut r: u32 = 0;
                while why.len() == 0 && r < m.returns.len {
                    ...
                    let tn = if_node(rn.kind == NodeKind::NODE_PARAMETER, rn.as_data.parameter.ty, rid);
                    if self.tc_mentions_self(iface.module, tn, iface) { why = "a method mentions 'Self' outside the receiver"; }
                    r = r + 1;
                }
            }
            i = i + 1;
        }
        if why.len() == 0 { return true; }
        let inm = unsafe (*ia).at_const(idn.name).as_data.name.text;
        self.errors.emit(at.start, at.end - at.start, format("interface '{}' is not dyn-compatible: {}", diag::span_str(isrc, inm.start, inm.end), why));
        if mn.end > mn.start { self.errors.note(format("offending method: '{}'", diag::span_str(isrc, mn.start, mn.end))); }
        return false;
    }
    ...
    fn ext_scopes(self: &mut Self) i32 {
        if self.n_ext_scope < 0 {
            ...
            if self.package != null {
                ...
                for i in 0..closure.len() { self.ext_scope.push(closure[i]); }
            }
    ...
    fn tc_peel_target(self: &mut Self, tg: DefId) DefId {
        ...
        if dn.kind != NodeKind::NODE_TYPE_ALIAS || dn.as_data.type_alias.ty == NODE_NONE || dn.as_data.type_alias.generics.len != 0 { return tg; }
        let ty = *self.type_at(self.named_type_of(tg.module, tg.node));
        if ty.kind == TypeKind::TYPE_BUILTIN {
            let mut bd = NODE_NONE;
            if self.package != null { bd = unsafe (*self.package).builtin_decl(ty.as_data.builtin); }
            if bd != NODE_NONE { return DefId { module: unsafe (*self.package).core_module, node: bd }; }
            return tg;
        }
        if ty.kind == TypeKind::TYPE_STRUCT || ty.kind == TypeKind::TYPE_ENUM { return DefId { module: ty.module, node: ty.as_data.decl }; }
    ...
    fn find_member(self: &Self, m: ModuleId, decl: NodeId, name: tok::Span) NodeId {
        ...
        if d.kind != NodeKind::NODE_STRUCT && d.kind != NodeKind::NODE_ENUM { return NODE_NONE; }
        ...
        for i in 0..members.len {
            ...
            if spans_eq2(self.source, name, src, unsafe (*a).at_const(mname).as_data.name.text) { return mid; }
        }
        return NODE_NONE;
    }
    fn find_member_cstr(self: &Self, m: ModuleId, decl: NodeId, name: str) NodeId {
        ...
        for i in 0..members.len {
            ...
            if (sp.end - sp.start) as usize == nl && unsafe cstring::memcmp((src + sp.start as usize), name.ptr(), nl) == 0 { return mid; }
        }
        return NODE_NONE;
    }
    ...
    fn enclosing_extend(self: &Self, m: ModuleId, method: NodeId) NodeId {
        ...
        if let Some(v) = self.encl_ext_memo.get(&key) { return *v; }
        let a = self.mod_ast(m);
        let items = unsafe (*a).at_const((*a).root).as_data.program.items;
        ...
        while i < items.len && res == NODE_NONE {
            ...
            if unsafe (*a).at_const(iid).kind == NodeKind::NODE_EXTEND {
                ...
                for j in 0..ms.len { if unsafe ((*a).list(ms))[j as usize] == method { res = iid; break; } }
            }
            i = i + 1;
        }
    ...
    fn enclosing_trait(self: &Self, m: ModuleId, method: NodeId) NodeId {
        let key = ((m as u64) << 32) | (method as u64);
        if let Some(v) = self.encl_trait_memo.get(&key) { return *v; }
        let a = self.mod_ast(m);
        let items = unsafe (*a).at_const((*a).root).as_data.program.items;
        let mut res = NODE_NONE;
        let mut i: u32 = 0;
                    ...
                    if tg.module == m && tg.node == decl {
                        ...
                        for j in 0..ms.len {
                            ...
                            if mn.kind == NodeKind::NODE_FUNCTION {
                                ...
                                let mut hit = false;
                                if lit.len() != 0 { hit = span_is(self.mod_src(mm), mname, lit); }
                                else { hit = spans_eq2(self.source, name, self.mod_src(mm), mname); }
                                if hit {
    ...
    fn find_method(self: &mut Self, m: ModuleId, decl: NodeId, name: tok::Span) DefId { return self.find_method_impl(m, decl, name, ""); }
    fn find_method_cstr(self: &mut Self, m: ModuleId, decl: NodeId, lit: str) DefId { return self.find_method_impl(m, decl, tok::Span::empty(), lit); }
                    ...
                    if tg.module == m && tg.node == decl {
                        ...
                        for j in 0..ms.len {
                            ...
                            if cn.kind == NodeKind::NODE_CONST && !(sm != self.ast.module && !cn.as_data.const_def.is_public) {
                                if spans_eq2(self.source, name, self.mod_src(sm), unsafe (*a).at_const(cn.as_data.const_def.name).as_data.name.text) { return DefId { module: sm, node: cid }; }
                            }
                        }
                    }
                }
    ...
    fn find_interface_method(self: &Self, m: ModuleId, iface: NodeId, name: tok::Span, depth: i32) DefId {
        if depth > 8 { return DefId { module: 0, node: NODE_NONE }; }
        let a = self.mod_ast(m);
        let tn = unsafe (*a).at_const(iface);
        if tn.kind != NodeKind::NODE_INTERFACE { return DefId { module: 0, node: NODE_NONE }; }
        let items = tn.as_data.interface_def.items;
        for i in 0..items.len {
            let mid = unsafe ((*a).list(items))[i as usize];
            let mn = unsafe (*a).at_const(mid);
            if mn.kind == NodeKind::NODE_FUNCTION && spans_eq2(self.source, name, self.mod_src(m), unsafe (*a).at_const(mn.as_data.function.name).as_data.name.text) { return DefId { module: m, node: mid }; }
        }
                ...
                if it.as_data.extend_def.interface_type != NODE_NONE {
                    ...
                    if tg.module == tmod && tg.node == tdecl {
                        ...
                        if iff.node != NODE_NONE {
                            ...
                            if mth.node != NODE_NONE && unsafe (*self.mod_ast(mth.module)).at_const(mth.node).as_data.function.body != NODE_NONE { return mth; }
                        }
                    }
                }
            }
    ...
    fn add_bound_ifaces_full(self: &mut Self, m: ModuleId, bounds: NodeList, out: *mut BoundIface, n: *mut i32, cap: i32) void {
        let a = self.mod_ast(m);
        let mut i: u32 = 0;
    ...
    fn collect_param_bounds_full(self: &mut Self, pmod: ModuleId, pdecl: NodeId, out: *mut BoundIface, cap: i32) i32 {
        ...
        self.add_bound_ifaces_full(pmod, unsafe (*pa).at_const(pdecl).as_data.generic_param.bounds, out, (&mut n) as *mut i32, cap);
        if pmod == self.ast.module && self.current_fn != NODE_NONE {
            let wc = unsafe (*self.cur_ast()).at_const(self.current_fn).as_data.function.where_clause;
            for w in 0..wc.len {
                let wid = unsafe ((*self.cur_ast()).list(wc))[w as usize];
                let wp = unsafe (*self.cur_ast()).at_const(wid).as_data.where_predicate;
                if unsafe (*self.cur_ast()).resolution(wp.ty) == pdecl { self.add_bound_ifaces_full(self.ast.module, wp.bounds, out, (&mut n) as *mut i32, cap); }
            }
        }
        return n;
    }
    fn trait_contains_method(self: &Self, iface: DefId, method: DefId, depth: i32) bool {
        if iface.node == NODE_NONE || depth > 8 { return false; }
        let ia = self.mod_ast(iface.module);
        let idn = unsafe (*ia).at_const(iface.node);
        if idn.kind != NodeKind::NODE_INTERFACE { return false; }
        let items = idn.as_data.interface_def.items;
        for i in 0..items.len {
            if iface.module == method.module && unsafe ((*ia).list(items))[i as usize] == method.node { return true; }
        }
        let bounds = idn.as_data.interface_def.bounds;
        for i in 0..bounds.len {
            if self.trait_contains_method(unsafe (*ia).resolution_def(unsafe ((*ia).list(bounds))[i as usize]), method, depth + 1) { return true; }
        }
        return false;
    }
    fn bound_method_subst(self: &mut Self, pmod: ModuleId, pdecl: NodeId, method: DefId, outp: *mut DefId, outa: *mut TypeId, cap: i32) i32 {
        let mut ifaces = BoundArr8 {};
        let ni = self.collect_param_bounds_full(pmod, pdecl, (&mut ifaces.b[0]) as *mut BoundIface, 8);
        for i in 0..ni {
            if self.trait_contains_method(ifaces.b[i as usize].iface, method, 0) {
                let ia = self.mod_ast(ifaces.b[i as usize].iface.module);
                let gens = unsafe (*ia).at_const(ifaces.b[i as usize].iface.node).as_data.interface_def.generics;
                let mut n: i32 = 0;
                let mut g: u32 = 0;
                while g < gens.len && (g as u8) < ifaces.b[i as usize].n && n < cap {
                    let gid = unsafe ((*ia).list(gens))[g as usize];
                    unsafe outp[n as usize] = DefId { module: ifaces.b[i as usize].iface.module, node: gid };
                    unsafe outa[n as usize] = ifaces.b[i as usize].args[g as usize];
                    n = n + 1;
                    g = g + 1;
                }
                return n;
    ...
    fn find_bound_method(self: &mut Self, pmod: ModuleId, pdecl: NodeId, name: tok::Span, iface: *mut DefId) DefId {
        ...
        let ni = self.collect_param_bounds_full(pmod, pdecl, (&mut ifaces.b[0]) as *mut BoundIface, 8);
        for b in 0..ni {
            let id = ifaces.b[b as usize].iface;
            let m = self.find_interface_method(id.module, id.node, name, 0);
            if m.node != NODE_NONE {
                if iface != null { unsafe *iface = id; }
                return m;
            }
        }
        return DefId { module: 0, node: NODE_NONE };
    ...
    fn interface_declares_cstr(self: &Self, iface: DefId, m: str, depth: i32) bool {
        if depth > 8 || iface.node == NODE_NONE { return false; }
        ...
        for i in 0..items.len {
            let mid = unsafe ((*a).list(items))[i as usize];
            let mn = unsafe (*a).at_const(mid);
            if mn.kind == NodeKind::NODE_FUNCTION && span_is(self.mod_src(iface.module), unsafe (*a).at_const(mn.as_data.function.name).as_data.name.text, m) { return true; }
        }
        let bounds = idn.as_data.interface_def.bounds;
        for i in 0..bounds.len {
            if self.interface_declares_cstr(unsafe (*a).resolution_def(unsafe ((*a).list(bounds))[i as usize]), m, depth + 1) { return true; }
        }
        return false;
    }
    fn tc_param_bound_provides(self: &mut Self, pmod: ModuleId, pdecl: NodeId, m: str) bool {
        ...
        let ni = self.collect_param_bounds_full(pmod, pdecl, (&mut ifaces.b[0]) as *mut BoundIface, 8);
        for b in 0..ni {
            if self.interface_declares_cstr(ifaces.b[b as usize].iface, m, 0) { return true; }
        }
        return false;
    }

    fn type_satisfies(self: &mut Self, ty: TypeId, iface: DefId, depth: i32) bool {
        if ty == TYPE_NONE || ty == TYPE_ERROR || depth > BOUND_MAX_DEPTH { return true; }
        let y = *self.type_at(ty);
        if y.kind == TypeKind::TYPE_GENERIC { return true; }
        if y.kind == TypeKind::TYPE_DYN { return y.module == iface.module && y.as_data.decl == iface.node; }
        let mut tmod: ModuleId = 0;
        let mut tdecl = NODE_NONE;
        let mut iargs = Tys4 {};
        let mut in2: i32 = 0;
        if y.kind == TypeKind::TYPE_STRUCT || y.kind == TypeKind::TYPE_ENUM { tmod = y.module; tdecl = y.as_data.decl; }
        else if y.kind == TypeKind::TYPE_INSTANCE {
            let inst = *unsafe (*self.cur_ast()).instance(y.as_data.inst);
            tmod = inst.module;
            tdecl = inst.decl;
            let mut k: u8 = 0;
            while k < inst.n && in2 < 4 { iargs.t[in2 as usize] = inst.args[k as usize]; in2 = in2 + 1; k = k + 1; }
        ...
        if extnode == NODE_NONE { return false; }
        let ia = self.mod_ast(imod);
        ...
        while g < gens.len && (g as i32) < in2 {
            ...
            for b in 0..gb.len {
                ...
                if gbi.node != NODE_NONE && !self.type_satisfies(iargs.t[g as usize], gbi, depth + 1) { return false; }
            }
            g = g + 1;
        }
        return true;
    ...
    fn is_free_iface(self: &Self, tr: DefId) bool {
        ...
        return trn.kind == NodeKind::NODE_INTERFACE && span_is(self.mod_src(tr.module), unsafe (*self.mod_ast(tr.module)).at_const(trn.as_data.interface_def.name).as_data.name.text, "Free");
    }
    fn tc_param_has_free_bound(self: &Self, m: ModuleId, gp: NodeId) bool {
        ...
        for i in 0..bs.len {
            ...
            if self.is_free_iface(bd) { return true; }
        }
        return false;
    }
    fn tc_type_is_free(self: &mut Self, ty: TypeId) bool {
        ...
        if y0.kind == TypeKind::TYPE_FUNCTION { return self.fn_owns(ty); }
        if y0.kind == TypeKind::TYPE_DYN { return y0.qualifier == (TypeQualifier::TYPE_QUAL_NONE as u8); }
        if y0.kind == TypeKind::TYPE_GENERIC {
            let fb = self.generic_fn_bound(y0.module, y0.as_data.decl);
            if fb != NODE_NONE { return unsafe (*self.mod_ast(y0.module)).at_const(fb).as_data.function_type.is_move; }
        }
        ...
        let mut ga = Tys4 {};
        let mut gn: i32 = 0;
        if !self.aggregate_of(self.strip(ty), (&mut om) as *mut ModuleId, (&mut od) as *mut NodeId, (&gp.d[0]) as *mut DefId, (&ga.t[0]) as *mut TypeId, (&mut gn) as *mut i32) { return false; }
        ...
        let mut have = false;
        if let Some(v) = self.free_ext_memo.get(&key) {
            if *v == 0u64 { return false; }
            fm = ((*v) >> 32) as ModuleId;
        ...
        if !have {
                ...
                while i < ne && !have {
                    ...
                    if it.as_data.extend_def.interface_type != NODE_NONE && it.as_data.extend_def.target_type != NODE_NONE {
                        ...
                        if tg.module == om && tg.node == od {
                            let tr = unsafe (*a).resolution_def(it.as_data.extend_def.interface_type);
                            if self.is_free_iface(tr) { fm = m; fx = iid; have = true; }
                        }
                    }
                    i = i + 1;
                }
            ...
            if !have { self.free_ext_memo.insert(key, 0u64); return false; }
        ...
        while k < gens.len && (k as i32) < gn {
            ...
            if self.tc_param_has_free_bound(fm, gid) && !self.tc_type_is_free(ga.t[k as usize]) { return false; }
            k = k + 1;
        }
        return true;
    }
    ...
    fn method_extend_bounds_hold(self: &mut Self, target: TypeId, md: DefId) bool {
        if target == TYPE_NONE || md.node == NODE_NONE { return true; }
        let extnode = self.enclosing_extend(md.module, md.node);
        if extnode == NODE_NONE { return true; }
        ...
        let mut ga = Tys4 {};
        let mut gn: i32 = 0;
        if !self.aggregate_of(self.strip(target), (&mut tm) as *mut ModuleId, (&mut td) as *mut NodeId, (&gp.d[0]) as *mut DefId, (&ga.t[0]) as *mut TypeId, (&mut gn) as *mut i32) { return true; }
        for g in 0..gens.len {
            if (g as i32) >= gn { return false; }
            let gid = unsafe ((*ia).list(gens))[g as usize];
            let gb = unsafe (*ia).at_const(gid).as_data.generic_param.bounds;
            for b in 0..gb.len {
                let bi = unsafe (*ia).resolution_def(unsafe ((*ia).list(gb))[b as usize]);
                if bi.node != NODE_NONE && !self.type_satisfies(ga.t[g as usize], bi, 0) { return false; }
            }
        }
        return true;
    }
    fn err_method_extend_bounds(self: &mut Self, at: tok::Span, target: TypeId, md: DefId) void {
        ...
        self.errors.emit(at.start, at.end - at.start, format("cannot call '{}::{}': unsatisfied interface bounds", diag::cstr((&ty.b[0]) as *const char), diag::span_str(self.mod_src(md.module), mn.start, mn.end)));
    ...
    fn mark_format_helpers(self: &mut Self) void {
        if self.package == null || self.fmt_marked { return; }
        ...
        names.n[14] = "pad_at".ptr() as *const char;
        names.n[15] = "len".ptr() as *const char;
        for i in 0..16 { self.find_method_cstr(sh.mid, sh.node, diag::cstr(names.n[i])); }
    }
    ...
    fn resolve_conversion(self: &mut Self, name: tok::Span, want: TypeId) DefId {
        ...
        let mut ga = Tys4 {};
        let mut gn: i32 = 0;
        if !self.aggregate_of(self.strip(want), (&mut m) as *mut ModuleId, (&mut decl) as *mut NodeId, (&gp.d[0]) as *mut DefId, (&ga.t[0]) as *mut TypeId, (&mut gn) as *mut i32) { return DefId { module: 0, node: NODE_NONE }; }
        if is_try {
    ...
    fn tc_find_from_for(self: &mut Self, target: TypeId, src: TypeId) DefId {
        ...
        if ty.kind != TypeKind::TYPE_STRUCT && ty.kind != TypeKind::TYPE_ENUM { return DefId { module: 0, node: NODE_NONE }; }
        let m = ty.module;
                ...
                if it.as_data.extend_def.target_type != NODE_NONE && it.as_data.extend_def.generics.len == 0 {
                    ...
                    if tg.module == m && tg.node == decl {
                        ...
                        for j in 0..ms.len {
                            let mid = unsafe ((*a).list(ms))[j as usize];
                            let mn = unsafe (*a).at_const(mid);
                            if mn.kind == NodeKind::NODE_FUNCTION && span_is(self.mod_src(mm), unsafe (*a).at_const(mn.as_data.function.name).as_data.name.text, "from") {
    ...
    fn dyn_coerce(self: &mut Self, node: NodeId, src: TypeId, dyn_ty: TypeId) bool {
        ...
        let mut ga = Tys4 {};
        let mut gn: i32 = 0;
        if !self.aggregate_of(src, (&mut tmod) as *mut ModuleId, (&mut tdecl) as *mut NodeId, (&gp.d[0]) as *mut DefId, (&ga.t[0]) as *mut TypeId, (&mut gn) as *mut i32) {
            if sy.kind == TypeKind::TYPE_BUILTIN && self.package != null && unsafe (*self.package).builtin_decl(sy.as_data.builtin) != NODE_NONE {
                tmod = unsafe (*self.package).core_module;
        ...
        for i in 0..items.len {
            ...
            if self.dyn_method(iface.module, mid) {
                ...
                if self.find_method_cstr(tmod, tdecl, diag::cstr((&nmb.b[0]) as *const char)).node != NODE_NONE { continue; }
                let mut emod: ModuleId = 0;
                if unsafe (*ia).at_const(mid).as_data.function.body != NODE_NONE && self.find_extend_as(tmod, tdecl, iface, (&mut emod) as *mut ModuleId) != NODE_NONE { continue; }
                let mut tn = Buf96 {};
                self.render_type(src, (&mut tn.b[0]) as *mut char, 96);
                self.errors.emit(sp.start, sp.end - sp.start, format("cannot erase '{}' to 'dyn': method '{}' has no emittable implementation", diag::cstr((&tn.b[0]) as *const char), diag::cstr((&nmb.b[0]) as *const char)));
    ...
    fn compatible(self: &mut Self, expected: TypeId, node: NodeId) bool {
        ...
        if expected == TYPE_NONE || actual == TYPE_NONE || expected == actual { return true; }
        ...
        if ex.kind == TypeKind::TYPE_ARRAY && ac.kind == TypeKind::TYPE_ARRAY {
            ...
            if ex.as_data.arr.elem == ac.as_data.arr.elem { return true; }
            let ee = self.type_at(ex.as_data.arr.elem).kind;
            let ae = self.type_at(ac.as_data.arr.elem).kind;
            return ee == TypeKind::TYPE_FUNCTION && ae == TypeKind::TYPE_FUNCTION && self.fn_compatible(ex.as_data.arr.elem, ac.as_data.arr.elem);
        }
        ...
        if ex.kind == TypeKind::TYPE_DYN {
            ...
            if ex.qualifier != (TypeQualifier::TYPE_QUAL_NONE as u8) && ac.kind == TypeKind::TYPE_REFERENCE {
                ...
                if rel.kind == TypeKind::TYPE_DYN {
                    if rel.qualifier != (TypeQualifier::TYPE_QUAL_NONE as u8) || !self.tc_dyn_same(&ex, &rel) { return false; }
                    unsafe (*self.cur_ast()).add_dyn_use(node, TYPE_NONE, expected);
                    return true;
                }
                if exsig != TYPE_NONE {
                    if rel.kind != TypeKind::TYPE_FUNCTION || !self.dynfn_sig_ok(exsig, ac.as_data.elem) { return false; }
                    unsafe (*self.cur_ast()).add_dyn_use(node, ac.as_data.elem, expected);
                    return true;
                }
                if ex.qualifier == (TypeQualifier::TYPE_QUAL_MUT as u8) && ac.qualifier != (TypeQualifier::TYPE_QUAL_MUT as u8) { return false; }
            ...
            if exsig != TYPE_NONE && ac.kind == TypeKind::TYPE_FUNCTION {
                if unsafe (*self.mod_ast(ac.module)).at_const(ac.as_data.decl).kind == NodeKind::NODE_FUNCTION_TYPE {
                    self.errors.emit(sp.start, sp.end - sp.start, format("a runtime 'fn(..)' pointer cannot erase to 'dyn fn'; wrap it in a closure"));
                    return true;
                }
                ...
                if ex.qualifier != (TypeQualifier::TYPE_QUAL_NONE as u8) && self.fn_is_capturing(actual) {
                    self.errors.emit(sp.start, sp.end - sp.start, format("a capturing closure must be borrowed to view it as '&dyn fn': write '&f'"));
                    return true;
                }
            ...
            if ex.qualifier == (TypeQualifier::TYPE_QUAL_NONE as u8) && ac.kind == TypeKind::TYPE_INSTANCE && exsig == TYPE_NONE {
                ...
                if self.tc_box_of(&ac, (&mut inner) as *mut TypeId, (&mut galloc) as *mut bool) {
                    if !galloc {
                        self.errors.emit(sp.start, sp.end - sp.start, format("only a default-allocated 'Box<T>' can be erased to 'Box<dyn I>'"));
                        return true;
                    }
        ...
        if ex.kind == TypeKind::TYPE_BUILTIN && ac.kind == TypeKind::TYPE_BUILTIN && bt_widens(ac.as_data.builtin, ex.as_data.builtin) { return true; }
        let mut vid = node;
        let v0 = unsafe (*self.cur_ast()).at_const(node);
        if v0.kind == NodeKind::NODE_UNARY && v0.as_data.unary.op == TokenType::Minus { vid = v0.as_data.unary.operand; }
        let v = unsafe (*self.cur_ast()).at_const(vid);
        if v.kind != NodeKind::NODE_LITERAL { return false; }
        let et = *self.type_at(expected);
        let tt = v.as_data.literal.token_type;
        if tt == TokenType::IntegerLiteral {
            if self.tc_literal_pinned(vid) { return false; }
            if et.kind != TypeKind::TYPE_BUILTIN || !(bt_is_int(et.as_data.builtin) || bt_is_float(et.as_data.builtin) || bt_is_complex(et.as_data.builtin)) { return false; }
            if bt_is_int(et.as_data.builtin) {
                ...
                if got && !tc_lit_in_range(et.as_data.builtin, mag, neg) {
                    ...
                    self.errors.emit(vsp.start, vsp.end - vsp.start, format("integer literal is out of range for '{}'", diag::cstr((&tn.b[0]) as *const char)));
                    return true;
                }
        ...
        if tt == TokenType::CharacterLiteral { return et.kind == TypeKind::TYPE_BUILTIN && bt_is_int(et.as_data.builtin); }
        if tt == TokenType::FloatLiteral {
            if self.tc_literal_pinned(vid) { return false; }
            return et.kind == TypeKind::TYPE_BUILTIN && (bt_is_float(et.as_data.builtin) || bt_is_complex(et.as_data.builtin));
        }
        if tt == TokenType::StringLiteral || tt == TokenType::RawStringLiteral {
            if et.kind != TypeKind::TYPE_POINTER || et.qualifier != (TypeQualifier::TYPE_QUAL_CONST as u8) { return false; }
            let pe = self.type_at(et.as_data.elem);
            if pe.kind != TypeKind::TYPE_BUILTIN || (pe.as_data.builtin != BuiltinType::BT_CHAR && pe.as_data.builtin != BuiltinType::BT_U8) { return false; }
            unsafe (*self.cur_ast()).set_type(node, expected);
            return true;
        }
        if tt == TokenType::Null { return et.kind == TypeKind::TYPE_POINTER || et.kind == TypeKind::TYPE_REFERENCE; }
        return false;
    }

    fn return_list_is_explicit_void(self: &mut Self, rets: NodeList) bool {
        if rets.len != 1 { return false; }
    ...
    fn range_type(self: &mut Self, id: NodeId, start: TypeId, end: TypeId) TypeId {
        ...
        if !has_end { return start; }
        if start == TYPE_NONE { return end; }
        if end == TYPE_NONE || start == end { return start; }
        if self.is_integer_literal_node(n.start) && self.compatible(end, n.start) { return end; }
        if self.is_integer_literal_node(n.end) && self.compatible(start, n.end) { return start; }
    ...
    fn is_assignable(self: &mut Self, node_in: NodeId) bool {
        ...
        if nk == NodeKind::NODE_IDENTIFIER {
            ...
            if d == NODE_NONE { return false; }
            let dn = unsafe (*a).at_const(d);
            if dn.kind == NodeKind::NODE_CONST { return dn.as_data.const_def.is_static_mut; }
            if dn.kind == NodeKind::NODE_LET { return dn.as_data.let_stmt.is_mutable; }
            if dn.kind == NodeKind::NODE_PARAMETER { return dn.as_data.parameter.is_mutable; }
            if dn.kind == NodeKind::NODE_PATTERN_NAME { return unsafe (*a).at_const(dn.as_data.pattern.name).as_data.name.is_mutable; }
            if dn.kind == NodeKind::NODE_IDENTIFIER {
                let letn = unsafe (*a).resolution(d);
                return letn != NODE_NONE && unsafe (*a).at_const(letn).kind == NodeKind::NODE_LET && unsafe (*a).at_const(letn).as_data.let_stmt.is_mutable;
            }
            return false;
        }
        if nk == NodeKind::NODE_UNARY {
            if unsafe (*a).at_const(node).as_data.unary.op != TokenType::Star { return false; }
        ...
        if nk == NodeKind::NODE_INDEX || nk == NodeKind::NODE_MEMBER {
            if nk == NodeKind::NODE_MEMBER && unsafe (*a).at_const(node).as_data.member.path { return self.tc_path_static_mut(node); }
            let obj = if_node(nk == NodeKind::NODE_INDEX, unsafe (*a).at_const(node).as_data.index.object, unsafe (*a).at_const(node).as_data.member.object);
            ...
            let mut sk: i32 = 0;
            if nk == NodeKind::NODE_INDEX { sk = self.slice_kind(oty, null); }
            if sk != 0 { return sk == 2; }
            ...
            if nk == NodeKind::NODE_INDEX && (ot.kind == TypeKind::TYPE_STRUCT || ot.kind == TypeKind::TYPE_INSTANCE) {
                ...
                if itn == NODE_NONE || unsafe (*ia).at_const(itn).kind != NodeKind::NODE_REFERENCE_TYPE { return false; }
                if via_ref { return ref_mut; }
                return self.is_assignable(obj);
            }
            if ot.kind == TypeKind::TYPE_POINTER || ot.kind == TypeKind::TYPE_REFERENCE { return ot.qualifier == (TypeQualifier::TYPE_QUAL_MUT as u8); }
            return self.is_assignable(obj);
        }
        return false;
    }
    fn is_place(self: &Self, id: NodeId) bool {
        ...
        if n.kind == NodeKind::NODE_MEMBER { return !n.as_data.member.path || self.tc_path_static_mut(node); }
        if n.kind == NodeKind::NODE_UNARY { return n.as_data.unary.op == TokenType::Star; }
        return false;
    }
    fn receiver_mutable(self: &mut Self, recv: NodeId) bool {
        ...
        if rt != TYPE_NONE {
            ...
            if rty.kind == TypeKind::TYPE_POINTER || rty.kind == TypeKind::TYPE_REFERENCE { return rty.qualifier == (TypeQualifier::TYPE_QUAL_MUT as u8); }
        }
        if self.is_place(recv) { return self.is_assignable(recv); }
        return true;
    }

    // ---- move / definite-init / free tracking ----
    fn tc_binding_depth(self: &Self, decl: NodeId) u32 {
        if decl == NODE_NONE || unsafe (*self.cur_ast()).at_const(decl).kind == NodeKind::NODE_PARAMETER { return 0; }
        if let Some(v) = self.binding_depth.get(&decl) { return *v; }
        return 0;
    }
    fn tc_record_binding_depth(self: &mut Self, decl: NodeId) void {
        if decl != NODE_NONE { self.binding_depth.insert(decl, self.scope_depth); }
    }

    fn tc_capture_move_guard(self: &mut Self, expr0: NodeId) bool {
        if self.nclos == 0 { return false; }
        let a = self.cur_ast();
        let mut expr = expr0;
        loop {
            let n = unsafe (*a).at_const(expr);
            if n.kind == NodeKind::NODE_UNARY && (n.as_data.unary.op == TokenType::Move || n.as_data.unary.op == TokenType::Unsafe) { expr = n.as_data.unary.operand; } else { break; }
        }
        if unsafe (*a).at_const(expr).kind != NodeKind::NODE_IDENTIFIER { return false; }
        let d = unsafe (*a).resolution_def(expr);
        ...
        self.errors.emit(sp.start, sp.end - sp.start, format("cannot move a captured value out of a closure (the closure's env owns it)"));
        return true;
    }
    ...
    fn ms_bit_set(self: &mut Self, d: NodeId) void {
        ...
        while self.moved_bits.len() <= idx { self.moved_bits.push(0u64); }
        self.moved_bits.set(idx, self.moved_bits[idx] | (1u64 << ((d & 63u32) as u64)));
    }
    fn ms_bit_clear(self: &mut Self, d: NodeId) void {
        let idx = (d >> 6) as usize;
        if idx < self.moved_bits.len() { self.moved_bits.set(idx, self.moved_bits[idx] & ~(1u64 << ((d & 63u32) as u64))); }
    }
    fn is_moved(self: &Self, decl: NodeId) bool {
        let idx = (decl >> 6) as usize;
        if idx >= self.moved_bits.len() { return false; }
    ...
    fn tc_mark_move(self: &mut Self, expr0: NodeId) void {
        ...
        if (dk != NodeKind::NODE_LET && dk != NodeKind::NODE_PARAMETER) || !self.tc_type_is_free(unsafe (*a).type_of(expr)) { return; }
        if self.tc_capture_move_guard(expr) { return; }
        ...
        if self.is_moved(d.node) { return; }
        if self.nmoved < 1024 { let k = self.nmoved; self.moved[k as usize] = d.node; self.nmoved = k + 1; self.ms_bit_set(d.node); }
        else {
            let sp = unsafe (*a).at_const(expr).span;
            self.errors.emit(sp.start, sp.end - sp.start, format("too many moved values in one function (move-analysis limit)"));
        }
    }

    fn tc_is_uninit(self: &Self, decl: NodeId) bool {
        for i in 0..self.nuninit { if self.uninit[i as usize] == decl { return true; } }
        return false;
    }
    fn tc_add_uninit(self: &mut Self, decl: NodeId) void {
        if self.tc_is_uninit(decl) { return; }
        if self.nuninit < 256 { let k = self.nuninit; self.uninit[k as usize] = decl; self.nuninit = k + 1; }
            ...
            self.errors.emit(sp.start, sp.end - sp.start, format("too many uninitialized bindings in one function (definite-init analysis limit)"));
        }
    }
    fn tc_init(self: &mut Self, decl: NodeId) void {
        let mut i: u32 = 0;
        while i < self.nuninit {
            if self.uninit[i as usize] == decl { self.nuninit = self.nuninit - 1; self.uninit[i as usize] = self.uninit[self.nuninit as usize]; return; }
            i = i + 1;
        }
    }
    fn tc_unmark_move(self: &mut Self, decl: NodeId) void {
        let mut i: u32 = 0;
        while i < self.nmoved { if self.moved[i as usize] == decl { self.nmoved = self.nmoved - 1; self.moved[i as usize] = self.moved[self.nmoved as usize]; break; } i = i + 1; }
        // moved[] can hold duplicates (closure captures push unguarded) -- only drop the bit once
        // no entry remains.
        let mut still = false;
        i = 0;
        while i < self.nmoved { if self.moved[i as usize] == decl { still = true; break; } i = i + 1; }
        if !still { self.ms_bit_clear(decl); }
        i = 0;
        while i < self.nfreed { if self.freed[i as usize] == decl { self.nfreed = self.nfreed - 1; self.freed[i as usize] = self.freed[self.nfreed as usize]; break; } i = i + 1; }
    }
    ...
    fn tc_flow_save(self: &Self, s: &mut FlowState) void {
        ...
        for i in 0..self.nmoved { s.moved[i as usize] = self.moved[i as usize]; }
        s.nuninit = self.nuninit;
        for i in 0..self.nuninit { s.uninit[i as usize] = self.uninit[i as usize]; }
        s.nfreed = self.nfreed;
        for i in 0..self.nfreed { s.freed[i as usize] = self.freed[i as usize]; }
        s.nborrows = self.nborrows;
        for i in 0..self.nborrows { s.borrows[i as usize] = self.borrows[i as usize]; }
    }
    fn tc_flow_set(self: &mut Self, s: &FlowState) void {
        for i in 0..self.nmoved { self.ms_bit_clear(self.moved[i as usize]); }
        self.nmoved = s.nmoved;
        for i in 0..s.nmoved { self.moved[i as usize] = s.moved[i as usize]; self.ms_bit_set(s.moved[i as usize]); }
        self.nuninit = s.nuninit;
        for i in 0..s.nuninit { self.uninit[i as usize] = s.uninit[i as usize]; }
        self.nfreed = s.nfreed;
        for i in 0..s.nfreed { self.freed[i as usize] = s.freed[i as usize]; }
        self.nborrows = s.nborrows;
        for i in 0..s.nborrows { self.borrows[i as usize] = s.borrows[i as usize]; }
    }
    ...
    fn tc_flow_collect(self: &Self, acc: *mut FlowState) bool {
        ...
        for i in 0..self.nmoved {
            let mut seen = false;
            for j in 0..unsafe (*acc).nmoved { if unsafe (*acc).moved[j as usize] == self.moved[i as usize] { seen = true; } }
            if !seen {
                if unsafe (*acc).nmoved < 256 { let k = unsafe (*acc).nmoved; unsafe (*acc).moved[k as usize] = self.moved[i as usize]; unsafe (*acc).nmoved = k + 1; }
                else { overflow = true; }
            }
        }
        for i in 0..self.nuninit {
            let mut seen = false;
            for j in 0..unsafe (*acc).nuninit { if unsafe (*acc).uninit[j as usize] == self.uninit[i as usize] { seen = true; } }
            if !seen {
                if unsafe (*acc).nuninit < 64 { let k = unsafe (*acc).nuninit; unsafe (*acc).uninit[k as usize] = self.uninit[i as usize]; unsafe (*acc).nuninit = k + 1; }
                else { overflow = true; }
            }
        }
        for i in 0..self.nfreed {
            let mut seen = false;
            for j in 0..unsafe (*acc).nfreed { if unsafe (*acc).freed[j as usize] == self.freed[i as usize] { seen = true; } }
            if !seen {
                if unsafe (*acc).nfreed < 64 { let k = unsafe (*acc).nfreed; unsafe (*acc).freed[k as usize] = self.freed[i as usize]; unsafe (*acc).nfreed = k + 1; }
                else { overflow = true; }
            }
        }
        for i in 0..self.nborrows {
            let mut seen = false;
            for j in 0..unsafe (*acc).nborrows { if self.borrow_same(unsafe (*acc).borrows[j as usize], self.borrows[i as usize]) { seen = true; } }
            if !seen {
                if unsafe (*acc).nborrows < 64 { let k = unsafe (*acc).nborrows; unsafe (*acc).borrows[k as usize] = self.borrows[i as usize]; unsafe (*acc).nborrows = k + 1; }
                else { overflow = true; }
            }
        }
        return overflow;
    ...
    fn tc_flow_overflow(self: &mut Self, at: NodeId) void {
        ...
        self.errors.emit(sp.start, sp.end - sp.start, format("too many flow facts to merge across branches in one function (analysis limit)"));
    }

    fn tc_stmt_returns(self: &Self, id: NodeId) bool {
        if id == NODE_NONE { return false; }
        let n = unsafe (*self.cur_ast()).at_const(id);
        if n.kind == NodeKind::NODE_RETURN { return true; }
        ...
        if n.kind == NodeKind::NODE_IF {
            return n.as_data.if_stmt.else_branch != NODE_NONE && self.tc_stmt_returns(n.as_data.if_stmt.then_branch) && self.tc_stmt_returns(n.as_data.if_stmt.else_branch);
        }
    ...
    // ---- scope / borrow set ----
    fn tc_scope_enter(self: &mut Self) void { self.scope_depth = self.scope_depth + 1; }
    fn tc_scope_exit(self: &mut Self) void {
        let d = self.scope_depth;
        let mut w: u32 = 0;
        for i in 0..self.nborrows {
            let b = self.borrows[i as usize];
            if (b.region as u32) >= d { continue; }
            if b.binding != NODE_NONE && b.root != NODE_NONE && self.tc_binding_depth(b.root) >= d {
                let sp = unsafe (*self.cur_ast()).at_const(b.origin).span;
                self.errors.emit(sp.start, sp.end - sp.start, format("borrowed value does not live long enough: it is destroyed at the end of this block while a reference to it is still stored"));
                continue;
            }
        ...
        self.nborrows = w;
        if self.scope_depth != 0 { self.scope_depth = self.scope_depth - 1; }
    }
    ...
    fn place_index_const(self: &Self, idx: NodeId, out: *mut i64) bool {
        ...
        for i in 0..len {
            ...
            let mut d: u64 = 0;
            if ch <= '9' as u8 { d = (ch - '0' as u8) as u64; } else { d = ((ch | 0x20u8) - 'a' as u8 + 10u8) as u64; }
            if d >= base || acc > ((0x7FFFFFFFFFFFFFFFi64 - (d as i64)) as u64) / base { return false; }
            acc = acc * base + d;
        }
    ...
    fn tc_type_is_union(self: &Self, ty: TypeId) bool {
        ...
        let mut ga = Tys4 {};
        let mut gn: i32 = 0;
        if ty == TYPE_NONE || !self.aggregate_of(ty, (&mut m) as *mut ModuleId, (&mut d) as *mut NodeId, (&gp.d[0]) as *mut DefId, (&ga.t[0]) as *mut TypeId, (&mut gn) as *mut i32) { return false; }
    ...
    fn place_decompose(self: &mut Self, place0: NodeId, steps: *mut PStep, nsteps: *mut i32, cap: i32) NodeId {
        ...
        loop {
            ...
            if pn.kind == NodeKind::NODE_UNARY && (pn.as_data.unary.op == TokenType::Move || pn.as_data.unary.op == TokenType::Unsafe) { place = pn.as_data.unary.operand; continue; }
            if pn.kind == NodeKind::NODE_UNARY && pn.as_data.unary.op == TokenType::Star {
                ...
                if ot == TYPE_NONE || self.type_at(ot).kind != TypeKind::TYPE_REFERENCE { return NODE_NONE; }
                if unsafe *nsteps < cap { let k = unsafe *nsteps; unsafe steps[k as usize] = PStep { kind: PS_DEREF }; unsafe *nsteps = k + 1; }
                place = op;
                continue;
            }
            ...
            if pn.kind == NodeKind::NODE_MEMBER && !pn.as_data.member.path {
                ...
                step.kind = PS_INDEX;
                let mut v: i64 = 0;
                if self.place_index_const(pn.as_data.index.index, (&mut v) as *mut i64) { step.index_const = true; step.index_val = v; }
            } else { break; }
            let bt = unsafe (*a).type_of(base);
            if bt == TYPE_NONE { return NODE_NONE; }
            let btk = *self.type_at(bt);
            let mut union_ty = bt;
            if btk.kind == TypeKind::TYPE_REFERENCE { union_ty = btk.as_data.elem; }
            let base_union = pn.kind == NodeKind::NODE_MEMBER && self.tc_type_is_union(union_ty);
            if base_union { unsafe *nsteps = 0; }
            if btk.kind == TypeKind::TYPE_REFERENCE {
                if pn.kind == NodeKind::NODE_INDEX { return NODE_NONE; }
                if !base_union && unsafe *nsteps < cap { let k = unsafe *nsteps; unsafe steps[k as usize] = step; unsafe *nsteps = k + 1; }
                if unsafe *nsteps < cap { let k = unsafe *nsteps; unsafe steps[k as usize] = PStep { kind: PS_DEREF }; unsafe *nsteps = k + 1; }
                place = base;
                continue;
            }
            if btk.kind == TypeKind::TYPE_POINTER { return NODE_NONE; }
            if pn.kind == NodeKind::NODE_INDEX && btk.kind != TypeKind::TYPE_ARRAY { return NODE_NONE; }
            if !base_union && unsafe *nsteps < cap { let k = unsafe *nsteps; unsafe steps[k as usize] = step; unsafe *nsteps = k + 1; }
            place = base;
        }
        if unsafe (*a).at_const(place).kind != NodeKind::NODE_IDENTIFIER { return NODE_NONE; }
        let d = unsafe (*a).resolution_def(place);
        if d.node == NODE_NONE || d.module != self.ast.module { return NODE_NONE; }
        let dk = unsafe (*a).at_const(d.node).kind;
        if dk == NodeKind::NODE_PARAMETER || dk == NodeKind::NODE_LET || dk == NodeKind::NODE_PATTERN_NAME || dk == NodeKind::NODE_IDENTIFIER || dk == NodeKind::NODE_FOR { return d.node; }
        return NODE_NONE;
    }
    fn borrow_place_root(self: &mut Self, place: NodeId) NodeId {
        let mut steps = Steps16 {};
        let mut n: i32 = 0;
        return self.place_decompose(place, (&mut steps.s[0]) as *mut PStep, (&mut n) as *mut i32, PLACE_MAX_STEPS);
    }
    fn places_overlap(self: &mut Self, aN: NodeId, bN: NodeId) bool {
        ...
        let mut na: i32 = 0;
        let mut nb: i32 = 0;
        let ra = self.place_decompose(aN, (&mut sa.s[0]) as *mut PStep, (&mut na) as *mut i32, PLACE_MAX_STEPS);
        let rb = self.place_decompose(bN, (&mut sb.s[0]) as *mut PStep, (&mut nb) as *mut i32, PLACE_MAX_STEPS);
        if ra == NODE_NONE || rb == NODE_NONE || ra != rb { return false; }
        let mut common = na;
        if nb < na { common = nb; }
        for i in 0..common {
            let pa = sa.s[(na - 1 - i) as usize];
            let pb = sb.s[(nb - 1 - i) as usize];
    ...
    fn place_through_binding(self: &Self, place0: NodeId) NodeId {
        ...
        loop {
            let pn = unsafe (*a).at_const(place);
            let mut base = NODE_NONE;
            if pn.kind == NodeKind::NODE_UNARY && pn.as_data.unary.op == TokenType::Star { base = pn.as_data.unary.operand; }
            else if pn.kind == NodeKind::NODE_MEMBER && !pn.as_data.member.path { base = pn.as_data.member.object; }
            else if pn.kind == NodeKind::NODE_INDEX { base = pn.as_data.index.object; }
            else { return NODE_NONE; }
            let bt = unsafe (*a).type_of(base);
            let mut is_ref = false;
            if bt != TYPE_NONE && self.type_at(bt).kind == TypeKind::TYPE_REFERENCE { is_ref = true; }
            if (pn.kind == NodeKind::NODE_UNARY || is_ref) && unsafe (*a).at_const(base).kind == NodeKind::NODE_IDENTIFIER {
                ...
                if d.node != NODE_NONE && d.module == self.ast.module { return d.node; }
                return NODE_NONE;
            }
            place = base;
        }
    ...
    fn borrow_mark(self: &Self) u32 { return self.nborrows; }
    fn borrow_release_to(self: &mut Self, mark: u32) void {
        if self.nborrows <= mark { return; }
        let mut w = mark;
        let mut i = mark;
        while i < self.nborrows { if self.borrows[i as usize].binding != NODE_NONE { self.borrows[w as usize] = self.borrows[i as usize]; w = w + 1; } i = i + 1; }
        self.nborrows = w;
    }
    ...
    fn tc_build_last_use(self: &mut Self) void {
        ...
        self.last_use.clear();
        let mut i: usize = 0;
        while i < n { self.last_use.push(NODE_NONE); i = i + 1; }
        let mut nid: NodeId = 0;
        while (nid as usize) < n {
            let rd = unsafe (*self.cur_ast()).resolution_def(nid);
            if rd.node != NODE_NONE && rd.module == self.ast.module && (rd.node as usize) < n { self.last_use.set(rd.node as usize, nid); }
            nid = nid + 1;
        }
    }
    fn tc_note_resolution(self: &mut Self, ref_id: NodeId, decl: NodeId) void {
        if self.last_use_built && (decl as usize) < self.last_use.len() && ref_id > self.last_use[decl as usize] { self.last_use.set(decl as usize, ref_id); }
    }

    fn borrow_dead_after(self: &mut Self, b: Borrow, after: NodeId) bool {
        if b.binding == NODE_NONE || self.loop_depth != 0 { return false; }
        let bn = unsafe (*self.cur_ast()).at_const(b.binding);
        if bn.kind == NodeKind::NODE_LET && unsafe (*self.cur_ast()).at_const(bn.as_data.let_stmt.name).kind == NodeKind::NODE_PATTERN_TUPLE { return false; }
        for i in 0..self.nborrows {
            if self.borrows[i as usize].binding != b.binding && self.place_through_binding(self.borrows[i as usize].place) == b.binding { return false; }
        }
        if !self.last_use_built { self.tc_build_last_use(); }
        if (b.binding as usize) < self.last_use.len() && self.last_use[b.binding as usize] > after { return false; }
        return true;
    }

    fn borrow_report_conflict(self: &mut Self, place: NodeId, kind: u8, origin: NodeId) bool {
        let root = self.borrow_place_root(place);
        if root == NODE_NONE { return false; }
        for i in 0..self.nborrows {
            let b = self.borrows[i as usize];
            if b.root != root || (kind == BORROW_SHARED && b.kind == BORROW_SHARED) || !self.places_overlap(place, b.place) { continue; }
            if self.borrow_dead_after(b, origin) { self.borrow_tombstone_at(i); continue; }
            ...
            self.errors.emit(sp.start, sp.end - sp.start, format("cannot borrow this value as {} while it is already borrowed as {}", diag::cstr(k1), diag::cstr(k2)));
            self.errors.note(format("a value may have many '&' borrows or a single '&mut', not both; the earlier borrow must end first"));
            return true;
        }
        return false;
    }
    fn borrow_push(self: &mut Self, root: NodeId, kind: u8, place: NodeId, origin: NodeId) void {
        if self.nborrows < 256 {
            let k = self.nborrows;
            self.borrows[k as usize] = Borrow { root: root, place: place, kind: kind, region: self.scope_depth as u16, origin: origin, binding: NODE_NONE };
            self.nborrows = k + 1;
        } else {
            let sp = unsafe (*self.cur_ast()).at_const(origin).span;
            self.errors.emit(sp.start, 1, format("too many simultaneous borrows in one function (borrow-checker limit)"));
        }
    }
    fn borrow_create(self: &mut Self, place: NodeId, kind: u8, origin: NodeId) void {
        let root = self.borrow_place_root(place);
        if root == NODE_NONE { return; }
        if !self.borrow_report_conflict(place, kind, origin) { self.borrow_push(root, kind, place, origin); }
    }
    fn borrow_conflicting_read(self: &mut Self, place: NodeId) bool {
        ...
        for i in 0..self.nborrows {
            let b = self.borrows[i as usize];
            if b.root != root || b.kind != BORROW_MUT || !self.places_overlap(place, b.place) { continue; }
            if self.borrow_dead_after(b, place) { self.borrow_tombstone_at(i); continue; }
            return true;
        }
        return false;
    }
    fn borrow_conflicting_write(self: &mut Self, place: NodeId, after: NodeId) bool {
        ...
        for i in 0..self.nborrows {
            let b = self.borrows[i as usize];
            if b.root != root || b.kind != BORROW_SHARED || b.binding == NODE_NONE || !self.places_overlap(place, b.place) { continue; }
            if self.borrow_dead_after(b, after) { self.borrow_tombstone_at(i); continue; }
            return true;
        }
        return false;
    }
    fn borrow_transfer_ref(self: &mut Self, init: NodeId, binding: NodeId) void {
        ...
        loop {
            ...
            if n.kind == NodeKind::NODE_UNARY && (n.as_data.unary.op == TokenType::Move || n.as_data.unary.op == TokenType::Unsafe) { e = n.as_data.unary.operand; } else { break; }
        }
        if unsafe (*a).at_const(e).kind != NodeKind::NODE_IDENTIFIER { return; }
        let rd = unsafe (*a).resolution_def(e);
        if rd.node == NODE_NONE || rd.module != self.ast.module { return; }
        ...
        if moved && !self.is_moved(rd.node) && self.nmoved < 1024 { let k = self.nmoved; self.moved[k as usize] = rd.node; self.nmoved = k + 1; self.ms_bit_set(rd.node); }
    }
    fn tuple_binds_reference(self: &Self, name: NodeId) bool {
        ...
        for i in 0..names.len {
            ...
            if et != TYPE_NONE && self.type_at(et).kind == TypeKind::TYPE_REFERENCE { return true; }
        }
        return false;
    }
    fn borrow_nll_drop(self: &mut Self, block_id: NodeId, ids: *const NodeId, si: u32) void {
        ...
        for k in 0..self.nborrows {
            ...
            if b.binding != NODE_NONE && b.region == (self.scope_depth as u16) {
                ...
                if !tuple {
                    keep.k[k as usize] = false;
                    let mut nid = unsafe ids[si as usize] + 1;
                    while nid < block_id && !keep.k[k as usize] {
                        let rd = unsafe (*self.cur_ast()).resolution_def(nid);
                        keep.k[k as usize] = rd.node == b.binding && rd.module == self.ast.module;
                        nid = nid + 1;
                    }
                }
            }
        ...
        while kk >= 0 {
            if keep.k[kk as usize] {
                let thru = self.place_through_binding(self.borrows[kk as usize].place);
                if thru != NODE_NONE {
                    for j in 0..self.nborrows { if self.borrows[j as usize].binding == thru { keep.k[j as usize] = true; } }
                }
            }
            kk = kk - 1;
        }
        let mut w: u32 = 0;
        for k in 0..self.nborrows { if keep.k[k as usize] { self.borrows[w as usize] = self.borrows[k as usize]; w = w + 1; } }
        self.nborrows = w;
    }
    ...
    fn place_escape(self: &mut Self, place: NodeId, depth: u32) i32 {
        let mut steps = Steps16 {};
        let mut ns: i32 = 0;
        let root = self.place_decompose(place, (&mut steps.s[0]) as *mut PStep, (&mut ns) as *mut i32, PLACE_MAX_STEPS);
        if root == NODE_NONE { return 0; }
        let mut thru = false;
        for i in 0..ns { if steps.s[i as usize].kind == PS_DEREF { thru = true; } }
        if thru {
            if unsafe (*self.cur_ast()).at_const(root).kind == NodeKind::NODE_PARAMETER { return 0; }
            return self.borrow_escape_of_binding(root, depth + 1);
        }
        if unsafe (*self.cur_ast()).at_const(root).kind == NodeKind::NODE_PARAMETER { return 2; }
        return 1;
    }
    ...
    fn addr_escape_at(self: &mut Self, e0: NodeId, depth: u32) i32 {
        ...
        loop {
            let n = unsafe (*a).at_const(e);
            if n.kind == NodeKind::NODE_CAST { e = n.as_data.cast.expression; }
            else if n.kind == NodeKind::NODE_UNARY && (n.as_data.unary.op == TokenType::Move || n.as_data.unary.op == TokenType::Unsafe) { e = n.as_data.unary.operand; }
            else { break; }
        }
        let n = unsafe (*a).at_const(e);
        if n.kind == NodeKind::NODE_UNARY && n.as_data.unary.op == TokenType::Ampersand { return self.place_escape(n.as_data.unary.operand, depth); }
        if n.kind == NodeKind::NODE_IDENTIFIER && depth < BORROW_ESCAPE_MAX_DEPTH {
            ...
            if d.module == self.ast.module && d.node != NODE_NONE {
                ...
                if dt != TYPE_NONE && self.type_at(dt).kind == TypeKind::TYPE_REFERENCE { return self.borrow_escape_of_binding(d.node, depth); }
                if dn.kind == NodeKind::NODE_LET && dn.as_data.let_stmt.value != NODE_NONE { return self.addr_escape_at(dn.as_data.let_stmt.value, depth + 1); }
            }
        }
        return 0;
    }
    fn addr_escape(self: &mut Self, e: NodeId) i32 { return self.addr_escape_at(e, 0); }
    ...
    fn find_extend_item_named(self: &Self, extnode: NodeId, name: tok::Span, nmod: ModuleId) NodeId {
        ...
        for j in 0..have.len {
            ...
            if hm.kind == NodeKind::NODE_FUNCTION && spans_eq2(self.mod_src(nmod), name, self.source, unsafe (*a).at_const(hm.as_data.function.name).as_data.name.text) { return hid; }
        }
        return NODE_NONE;
    }
    fn extend_method_signature_matches(self: &mut Self, req: DefId, have: NodeId, subp: *const DefId, suba: *const TypeId, nsub: i32) bool {
        ...
        if rf.params.len != hf.params.len { return false; }
        for i in 0..rf.params.len {
            ...
            if rt != ht && (i != 0 || !self.receiver_type_eq(rt, ht)) { return false; }
        }
        if rf.returns.len != hf.returns.len {
            if rf.returns.len > 1 || hf.returns.len > 1 { return false; }
            let mut rt: TypeId = TYPE_NONE;
            let mut ht: TypeId = TYPE_NONE;
            if rf.returns.len == 1 {
                let rr = unsafe ((*ra).list(rf.returns))[0];
                let rn = unsafe (*ra).at_const(rr);
                rt = self.lower_type_in(req.module, if_node(rn.kind == NodeKind::NODE_PARAMETER, rn.as_data.parameter.ty, rr));
                rt = self.subst_type(rt, subp, suba, nsub);
            }
            if hf.returns.len == 1 {
                let hr = unsafe ((*self.cur_ast()).list(hf.returns))[0];
                let hn = unsafe (*self.cur_ast()).at_const(hr);
                ht = self.lower_type_in(self.ast.module, if_node(hn.kind == NodeKind::NODE_PARAMETER, hn.as_data.parameter.ty, hr));
            }
        ...
        for k in 0..rf.returns.len {
            ...
            let rn = unsafe (*ra).at_const(rr);
            let hn = unsafe (*self.cur_ast()).at_const(hr);
            let mut rt = self.lower_type_in(req.module, if_node(rn.kind == NodeKind::NODE_PARAMETER, rn.as_data.parameter.ty, rr));
            rt = self.subst_type(rt, subp, suba, nsub);
            let ht = self.lower_type_in(self.ast.module, if_node(hn.kind == NodeKind::NODE_PARAMETER, hn.as_data.parameter.ty, hr));
            if !self.ret_eq(rt, ht) { return false; }
        }
        return true;
    }
    fn check_interface_requirements(self: &mut Self, extnode: NodeId, iface: DefId, self_ty: TypeId, subp: *const DefId, suba: *const TypeId, nsub: i32, depth: i32) void {
        if iface.node == NODE_NONE || depth > 8 { return; }
        let ia = self.mod_ast(iface.module);
        if unsafe (*ia).at_const(iface.node).kind != NodeKind::NODE_INTERFACE { return; }
        ...
        for i in 0..req.len {
            ...
            if rm.kind == NodeKind::NODE_FUNCTION && rm.as_data.function.body == NODE_NONE {
                ...
                if hm == NODE_NONE {
                    ...
                    self.errors.emit(at.start, at.end - at.start, format("missing method '{}' required by this interface", diag::span_str(self.mod_src(iface.module), rn.start, rn.end)));
                } else if !self.extend_method_signature_matches(reqdef, hm, subp, suba, nsub) {
                    let at = unsafe (*self.cur_ast()).at_const(hm).span;
                    self.errors.emit(at.start, at.end - at.start, format("method '{}' does not match interface signature", diag::span_str(self.mod_src(iface.module), rn.start, rn.end)));
                }
            }
        }
    ...
    fn check_extend_conformance(self: &mut Self, id: NodeId) void {
        let iface = unsafe (*self.cur_ast()).resolution_def(unsafe (*self.cur_ast()).at_const(id).as_data.extend_def.interface_type);
        if iface.node == NODE_NONE { return; }
        let target = unsafe (*self.cur_ast()).at_const(id).as_data.extend_def.target_type;
        let self_ty = self.resolve_type(target);
        let mut subp = Defs8 {};
        let mut suba = Tys8 {};
        let mut nsub: i32 = 0;
        subp.d[0] = DefId { module: iface.module, node: iface.node };
        suba.t[0] = self_ty;
        nsub = 1;
        let ia = self.mod_ast(iface.module);
        let itype = unsafe (*self.cur_ast()).at_const(id).as_data.extend_def.interface_type;
        if unsafe (*ia).at_const(iface.node).kind == NodeKind::NODE_INTERFACE && unsafe (*self.cur_ast()).at_const(itype).kind == NodeKind::NODE_TYPE_PATH {
            ...
            while i < gens.len && i < targs.len && nsub < 8 {
                ...
                subp.d[nsub as usize] = DefId { module: iface.module, node: gid };
                suba.t[nsub as usize] = self.resolve_type(unsafe ((*self.cur_ast()).list(targs))[i as usize]);
                nsub = nsub + 1;
                i = i + 1;
            }
        }
        self.check_interface_requirements(id, iface, self_ty, (&subp.d[0]) as *const DefId, (&suba.t[0]) as *const TypeId, nsub, 0);
    }
    ...
    fn method_recv_subst(self: &mut Self, recv: TypeId, md: DefId, rsubp: *mut DefId, rsuba: *mut TypeId) i32 {
        ...
        let mut ga = Tys4 {};
        let mut sn: i32 = 0;
        let agok = self.aggregate_of(self.strip(recv), (&mut rmod) as *mut ModuleId, (&mut rdecl) as *mut NodeId, (&gp.d[0]) as *mut DefId, (&ga.t[0]) as *mut TypeId, (&mut sn) as *mut i32);
        if agok && sn > 0 {
            let extnode = self.enclosing_extend(md.module, md.node);
            if extnode != NODE_NONE {
                ...
                while i < g && nrsub < 4 {
                    ...
                    unsafe rsuba[nrsub as usize] = ga.t[i as usize];
                    nrsub = nrsub + 1;
                    i = i + 1;
                }
            }
    ...
    fn tc_method_ret(self: &mut Self, recv: TypeId, md: DefId) TypeId {
        ...
        if fnn.kind != NodeKind::NODE_FUNCTION || fnn.as_data.function.returns.len != 1 { return TYPE_NONE; }
        let mut rsubp = Defs4 {};
        let mut rsuba = Tys4 {};
        let nrsub = self.method_recv_subst(recv, md, (&mut rsubp.d[0]) as *mut DefId, (&mut rsuba.t[0]) as *mut TypeId);
        let r0 = unsafe ((*fa).list(fnn.as_data.function.returns))[0];
        let rn = unsafe (*fa).at_const(r0);
        let ret = self.lower_type_in(md.module, if_node(rn.kind == NodeKind::NODE_PARAMETER, rn.as_data.parameter.ty, r0));
        return self.subst_type(ret, (&rsubp.d[0]) as *const DefId, (&rsuba.t[0]) as *const TypeId, nrsub);
    }
    fn tc_method_param(self: &mut Self, recv: TypeId, md: DefId, idx: i32) TypeId {
        let fa = self.mod_ast(md.module);
        let fnn = unsafe (*fa).at_const(md.node);
        if fnn.kind != NodeKind::NODE_FUNCTION || (fnn.as_data.function.params.len as i32) <= idx { return TYPE_NONE; }
        let mut rsubp = Defs4 {};
        let mut rsuba = Tys4 {};
        let nrsub = self.method_recv_subst(recv, md, (&mut rsubp.d[0]) as *mut DefId, (&mut rsuba.t[0]) as *mut TypeId);
        ...
        return self.subst_type(pt, (&rsubp.d[0]) as *const DefId, (&rsuba.t[0]) as *const TypeId, nrsub);
    }
    fn method_self_kind(self: &mut Self, md: DefId) i32 {
        let fa = self.mod_ast(md.module);
        let fnn = unsafe (*fa).at_const(md.node);
        if fnn.kind != NodeKind::NODE_FUNCTION || fnn.as_data.function.params.len == 0 { return 0; }
        let pt = self.decl_type_in(md.module, unsafe ((*fa).list(fnn.as_data.function.params))[0]);
        let y = self.type_at(pt);
        if y.kind != TypeKind::TYPE_REFERENCE && y.kind != TypeKind::TYPE_POINTER { return 0; }
        if y.qualifier == (TypeQualifier::TYPE_QUAL_MUT as u8) { return 2; }
        return 1;
    }
    fn operand_fits_param(self: &mut Self, pt: TypeId, operand: NodeId) bool {
        if pt == TYPE_NONE || self.compatible(pt, operand) { return true; }
...
fn arith_method_name(op: TokenType) str {
    if op == TokenType::Plus { return "add"; }
    if op == TokenType::Minus { return "sub"; }
    if op == TokenType::Star { return "mul"; }
    if op == TokenType::Slash { return "div"; }
    if op == TokenType::Percent { return "rem"; }
    return "";
}
...
extend TypeChecker {
    fn check_unary(self: &mut Self, id: NodeId) TypeId {
        ...
        if op == TokenType::Ampersand { self.addr_ctx = true; }
        ...
        if op == TokenType::Minus {
            if opnd != TYPE_NONE && !self.is_numeric(opnd) { self.errors.emit(sp.start, sp.end - sp.start, format("unary '-' requires a numeric operand")); }
            return opnd;
        }
        if op == TokenType::Bang {
            if opnd != TYPE_NONE && !self.is_bool(opnd) { self.errors.emit(sp.start, sp.end - sp.start, format("unary '!' requires a 'bool' operand")); }
            return Ast::builtin(BuiltinType::BT_BOOL);
        }
        if op == TokenType::Tilde {
            if opnd != TYPE_NONE && !self.is_int(opnd) { self.errors.emit(sp.start, sp.end - sp.start, format("unary '~' requires an integer operand")); }
            return opnd;
        }
        if op == TokenType::Star {
            ...
            if ot.kind == TypeKind::TYPE_POINTER || ot.kind == TypeKind::TYPE_REFERENCE {
                if ot.kind == TypeKind::TYPE_POINTER && self.unsafe_depth == 0 { self.err_unsafe(sp, "dereferencing a raw pointer"); }
        ...
        if op == TokenType::Ampersand {
            ...
            if mut2 && self.is_place(operand) && !self.is_assignable(operand) {
                ...
                self.errors.emit(osp.start, osp.end - osp.start, format("cannot take '&mut' of an immutable binding (bind it with 'mut')"));
            }
            if mut2 { self.tc_mark_capture_mut(operand); }
            let mut opw = operand;
            loop {
                let onn = unsafe (*a).at_const(opw);
                if onn.kind == NodeKind::NODE_UNARY && (onn.as_data.unary.op == TokenType::Move || onn.as_data.unary.op == TokenType::Unsafe) { opw = onn.as_data.unary.operand; } else { break; }
            }
            let onk = unsafe (*a).at_const(opw).kind;
            if mut2 && onk == NodeKind::NODE_IDENTIFIER {
                let od = unsafe (*a).resolution_def(opw);
                if od.module == self.ast.module && od.node != NODE_NONE { self.tc_init(od.node); }
            }
            if !self.is_place(opw) && opnd != TYPE_NONE && self.type_at(opnd).kind != TypeKind::TYPE_BUILTIN
                ...
                self.errors.emit(osp.start, osp.end - osp.start, format("cannot take the address of a temporary value; bind it to a 'let' first"));
            }
            let mut bk = BORROW_SHARED;
            if mut2 { bk = BORROW_MUT; }
            self.borrow_create(operand, bk, id);
            return unsafe (*self.cur_ast()).intern_type(Ty { kind: TypeKind::TYPE_REFERENCE, qualifier: qual as u8, as_data: TyAs { elem: opnd } });
        }
        if op == TokenType::Question {
            ...
            let mut oa = Tys4 {};
            let mut fa = Tys4 {};
            let nopt = self.prelude_instance_args_hit(os, self.ph_option, (&mut oa.t[0]) as *mut TypeId, 2);
            let mut nres: i32 = -1;
            if nopt < 0 { nres = self.prelude_instance_args_hit(os, self.ph_result, (&mut oa.t[0]) as *mut TypeId, 2); }
            if nopt < 0 && nres < 0 {
            ...
            if nopt >= 0 {
                if self.prelude_instance_args_hit(frs, self.ph_option, (&mut fa.t[0]) as *mut TypeId, 2) < 0 {
                    self.errors.emit(sp.start, sp.end - sp.start, format("'?' on an Option requires the function to return an Option"));
                    self.errors.note(format("change the function return type or handle None explicitly"));
                }
                return oa.t[0];
            }
            if self.prelude_instance_args_hit(frs, self.ph_result, (&mut fa.t[0]) as *mut TypeId, 2) < 0 {
                self.errors.emit(sp.start, sp.end - sp.start, format("'?' on a Result requires the function to return a Result"));
                ...
                if conv.node != NODE_NONE { unsafe (*self.cur_ast()).set_resolution_def(id, conv); }
                    ...
                    self.errors.emit(sp.start, sp.end - sp.start, format("'?' error type '{}' does not match the function's error type '{}'", diag::cstr((&a1.b[0]) as *const char), diag::cstr((&a2.b[0]) as *const char)));
                }
            }
            return oa.t[0];
        }
    ...
    fn binary_numeric(self: &mut Self, id: NodeId, l: TypeId, ln: NodeId, r: TypeId, rn: NodeId, require_int: bool) TypeId {
        if l == TYPE_NONE || r == TYPE_NONE { return TYPE_NONE; }
        let sp = unsafe (*self.cur_ast()).at_const(id).span;
        ...
        if unsafe (*self.cur_ast()).at_const(ln).kind == NodeKind::NODE_LITERAL && !self.tc_literal_pinned(ln) {
            if self.is_int(r) {
                ...
                if got && !tc_lit_in_range(rb, mag, false) {
                    ...
                    self.errors.emit(lsp.start, lsp.end - lsp.start, format("integer literal is out of range for '{}'", diag::cstr((&tn.b[0]) as *const char)));
        ...
        if unsafe (*self.cur_ast()).at_const(rn).kind == NodeKind::NODE_LITERAL && !self.tc_literal_pinned(rn) {
            if self.is_int(l) {
                ...
                if got && !tc_lit_in_range(lb, mag, false) {
                    ...
                    self.errors.emit(rsp.start, rsp.end - rsp.start, format("integer literal is out of range for '{}'", diag::cstr((&tn.b[0]) as *const char)));
                } else { unsafe (*self.cur_ast()).set_type(rn, l); }
            }
            return l;
        }
        if bt_widens(self.bt_of(l), self.bt_of(r)) { return r; }
        if bt_widens(self.bt_of(r), self.bt_of(l)) { return l; }
    ...
    fn check_ptr_arith(self: &mut Self, id: NodeId, l: TypeId, r: TypeId, handled: *mut bool) TypeId {
        ...
        unsafe *handled = true;
        let sp = unsafe (*self.cur_ast()).at_const(id).span;
        if self.unsafe_depth == 0 { self.err_unsafe(sp, "raw pointer arithmetic"); }
    ...
    fn check_arith_overload(self: &mut Self, id: NodeId, l: TypeId, out: *mut TypeId) bool {
        ...
        let mut ls = l;
        while ls != TYPE_NONE && self.type_at(ls).kind == TypeKind::TYPE_REFERENCE { ls = self.type_at(ls).as_data.elem; }
        if ls != TYPE_NONE && self.type_at(ls).kind == TypeKind::TYPE_POINTER { return false; }
        if ls == TYPE_NONE { return false; }
        ...
        if lt.kind == TypeKind::TYPE_GENERIC {
            if !self.tc_param_bound_provides(lt.module, lt.as_data.decl, m) {
                ...
                self.errors.emit(sp.start, sp.end - sp.start, format("type parameter '{}' has no '{}' method for this operator (add a bound that provides it)", diag::cstr((&ty.b[0]) as *const char), m));
                unsafe *out = TYPE_NONE;
            }
            return true;
        }
        if lt.kind != TypeKind::TYPE_STRUCT && lt.kind != TypeKind::TYPE_INSTANCE { return false; }
        let mut om: ModuleId = 0;
        let mut od = NODE_NONE;
        let mut gp = Defs4 {};
        let mut ga = Tys4 {};
        ...
        if self.aggregate_of(ls, (&mut om) as *mut ModuleId, (&mut od) as *mut NodeId, (&gp.d[0]) as *mut DefId, (&ga.t[0]) as *mut TypeId, (&mut gn) as *mut i32) {
            ...
            if md.node == NODE_NONE {
                ...
                let mut ty = Buf96 {};
                self.render_type(ls, (&mut ty.b[0]) as *mut char, 96);
                self.errors.emit(sp.start, sp.end - sp.start, format("'{}' has no '{}' method for this operator", diag::cstr((&ty.b[0]) as *const char), m));
                unsafe *out = TYPE_NONE;
            } else {
                ...
                if !self.operand_fits_param(p1, right) { self.err_mismatch(right, p1); }
                let ret = self.tc_method_ret(ls, md);
                if ret != TYPE_NONE { unsafe *out = ret; }
            }
        }
        return true;
    }
    ...
    fn check_binary(self: &mut Self, id: NodeId) TypeId {
        ...
        if op == TokenType::Plus || op == TokenType::Minus {
            ...
            if self.check_arith_overload(id, l, (&mut ov) as *mut TypeId) { return ov; }
            let mut handled = false;
            let pt = self.check_ptr_arith(id, l, r, (&mut handled) as *mut bool);
            if handled { return pt; }
        ...
        if op == TokenType::AmpersandAmpersand || op == TokenType::PipePipe {
            if (l != TYPE_NONE && !self.is_bool(l)) || (r != TYPE_NONE && !self.is_bool(r)) { self.errors.emit(sp.start, sp.end - sp.start, format("logical operator requires 'bool' operands")); }
            return Ast::builtin(BuiltinType::BT_BOOL);
        }
        // comparisons
        ...
        if ls != TYPE_NONE && self.type_at(ls).kind == TypeKind::TYPE_POINTER {
            if l != TYPE_NONE && r != TYPE_NONE && l != r && !self.compatible(l, rn) && !self.compatible(r, ln) { self.err_mismatch(rn, l); }
            return Ast::builtin(BuiltinType::BT_BOOL);
        }
        if ls != TYPE_NONE && (self.type_at(ls).kind == TypeKind::TYPE_STRUCT || self.type_at(ls).kind == TypeKind::TYPE_INSTANCE) {
            ...
            if self.aggregate_of(ls, (&mut om) as *mut ModuleId, (&mut od) as *mut NodeId, (&gp.d[0]) as *mut DefId, (&ga.t[0]) as *mut TypeId, (&mut gn) as *mut i32) {
                ...
                if md.node == NODE_NONE {
                    ...
                    self.errors.emit(sp.start, sp.end - sp.start, format("'{}' has no '{}' method for this operator (implement {})", diag::cstr((&ty.b[0]) as *const char), mm, diag::cstr(iface)));
                    ...
                    let p1 = self.tc_method_param(ls, md, 1);
                    if !self.operand_fits_param(p1, rn) { self.err_mismatch(rn, p1); }
                }
            }
            return Ast::builtin(BuiltinType::BT_BOOL);
        }
        if ls != TYPE_NONE && self.type_at(ls).kind == TypeKind::TYPE_GENERIC {
            ...
            if !self.tc_param_bound_provides(gy.module, gy.as_data.decl, mm) {
                let mut iface = "Eq".ptr() as *const char;
                if ord { iface = "Ord".ptr() as *const char; }
                self.errors.emit(sp.start, sp.end - sp.start, format("type parameter has no '{}' method for this operator (add a `{}` bound)", mm, diag::cstr(iface)));
            }
            return Ast::builtin(BuiltinType::BT_BOOL);
        }
        if l != TYPE_NONE && r != TYPE_NONE && l != r && !self.compatible(l, rn) && !self.compatible(r, ln) { self.err_mismatch(rn, l); }
    ...
    fn check_variant_call(self: &mut Self, id: NodeId, vmod: ModuleId, variant: NodeId, enum_ty: TypeId) TypeId {
        ...
        for i in 0..args.len { self.check_expr(unsafe ((*a).list(args))[i as usize]); }
        ...
        if args.len != payload.len {
            ...
            self.errors.emit(sp.start, sp.end - sp.start, format("variant expects {} argument{}, found {}", payload.len, diag::cstr(s2), args.len));
        } else {
            for k in 0..args.len {
                ...
                let mut pt = raw;
                if inst { pt = self.subst_type(raw, (&gp.d[0]) as *const DefId, (&ga.t[0]) as *const TypeId, gn); }
                let aid = unsafe ((*a).list(args))[k as usize];
                if !self.compatible(pt, aid) { self.err_mismatch(aid, pt); }
            }
        }
        return enum_ty;
    }
    ...
    fn tc_is_iface_assoc_call(self: &Self, e: NodeId) bool {
        ...
        if en.kind != NodeKind::NODE_CALL { return false; }
        let cn = unsafe (*a).at_const(en.as_data.call.callee);
        if cn.kind != NodeKind::NODE_MEMBER || !cn.as_data.member.path { return false; }
        let ob = unsafe (*a).resolution_def(cn.as_data.member.object);
        return ob.node != NODE_NONE && (ob.module == self.ast.module || (self.package != null && (ob.module as usize) < self.pkg_count())) && unsafe (*self.mod_ast(ob.module)).at_const(ob.node).kind == NodeKind::NODE_INTERFACE;
    }

    fn tc_param_expected(self: &mut Self, callee: TypeId, callee_node: NodeId, argi: u32) TypeId {
        if callee == TYPE_NONE { return TYPE_NONE; }
        let ct = *self.type_at(callee);
        if ct.kind != TypeKind::TYPE_FUNCTION { return TYPE_NONE; }
        let fa = self.mod_ast(ct.module);
        let fnn = unsafe (*fa).at_const(ct.as_data.decl);
        if fnn.kind != NodeKind::NODE_FUNCTION { return TYPE_NONE; }
        ...
        if cnn.kind == NodeKind::NODE_MEMBER && !cnn.as_data.member.path && fnn.as_data.function.params.len > 0 {
            ...
            if md.node != NODE_NONE && unsafe (*self.mod_ast(md.module)).at_const(md.node).kind == NodeKind::NODE_FUNCTION { skip = 1; }
        }
        if argi + skip >= fnn.as_data.function.params.len { return TYPE_NONE; }
        ...
        if pt != TYPE_NONE && self.type_at(pt).kind == TypeKind::TYPE_GENERIC {
            ...
            if fb != NODE_NONE { pt = self.lower_type_in(ct.module, fb); }
        }
        if pt != TYPE_NONE && !unsafe (*self.cur_ast()).type_concrete(pt) && skip != 0 {
            let md = unsafe (*self.cur_ast()).resolution_def(cnn.as_data.member.member);
            let mut extnode = NODE_NONE;
            if md.node != NODE_NONE { extnode = self.enclosing_extend(md.module, md.node); }
            let mut rm: ModuleId = 0;
            let mut rd = NODE_NONE;
            let mut gp = Defs4 {};
            let mut ga = Tys4 {};
            let mut gn: i32 = 0;
            let agok = extnode != NODE_NONE && self.aggregate_of(self.strip(unsafe (*self.cur_ast()).type_of(cnn.as_data.member.object)), (&mut rm) as *mut ModuleId, (&mut rd) as *mut NodeId, (&gp.d[0]) as *mut DefId, (&ga.t[0]) as *mut TypeId, (&mut gn) as *mut i32);
            if agok && gn > 0 {
                let ma = self.mod_ast(md.module);
                let ig = unsafe (*ma).at_const(extnode).as_data.extend_def.generics;
                let mut sp2 = Defs4 {};
                ...
                let mut ns: i32 = 0;
                let mut i: u32 = 0;
                while i < ig.len && (i as i32) < gn && ns < 4 {
                    let gid = unsafe ((*ma).list(ig))[i as usize];
                    sp2.d[ns as usize] = DefId { module: md.module, node: gid };
                    sa.t[ns as usize] = ga.t[i as usize];
                    ns = ns + 1;
                    i = i + 1;
                }
                pt = self.subst_type(pt, (&sp2.d[0]) as *const DefId, (&sa.t[0]) as *const TypeId, ns);
            }
        }
        if pt != TYPE_NONE && unsafe (*self.cur_ast()).type_concrete(pt) { return pt; }
        return TYPE_NONE;
    }
    ...
    fn iter_elem_type(self: &mut Self, it: TypeId) TypeId {
        ...
        let mut ret = self.lower_type_in(nx.module, if_node(rn.kind == NodeKind::NODE_PARAMETER, rn.as_data.parameter.ty, r0));
        ...
        if extnode != NODE_NONE && gn > 0 {
            ...
            let mut in2: i32 = 0;
            let mut i: u32 = 0;
            while i < ig.len && (i as i32) < gn && in2 < 4 {
                let gid = unsafe ((*na).list(ig))[i as usize];
                ip.d[in2 as usize] = DefId { module: nx.module, node: gid };
                ia.t[in2 as usize] = ga.t[i as usize];
                in2 = in2 + 1;
                i = i + 1;
            }
            ret = self.subst_type(ret, (&ip.d[0]) as *const DefId, (&ia.t[0]) as *const TypeId, in2);
        }
        let rt = self.type_at(ret);
        if rt.kind == TypeKind::TYPE_INSTANCE {
            let oi = *unsafe (*self.cur_ast()).instance(rt.as_data.inst);
            if oi.n >= 1 { return oi.args[0]; }
        }
        return TYPE_NONE;
    }
    ...
    fn tc_check_assert(self: &mut Self, id: NodeId, kind: i32) TypeId {
        ...
        if kind == 1 {
            if args.len < 1 || args.len > 2 {
                self.errors.emit(sp.start, sp.end - sp.start, format("'assert' takes a condition and an optional str message"));
                for i in 0..args.len { self.check_expr(unsafe ((*a).list(args))[i as usize]); }
                return Ast::builtin(BuiltinType::BT_VOID);
            }
            let ct = self.check_expr(unsafe ((*a).list(args))[0]);
            if ct != TYPE_NONE && !self.is_bool(ct) { self.errors.emit(sp.start, sp.end - sp.start, format("'assert' condition must be 'bool'")); }
            if args.len == 2 {
                let mt = self.check_expr(unsafe ((*a).list(args))[1]);
                if mt != TYPE_NONE && self.strip(mt) != self.prelude_str_type() { self.errors.emit(sp.start, sp.end - sp.start, format("'assert' message must be a 'str'")); }
            }
            return Ast::builtin(BuiltinType::BT_VOID);
        }
        ...
        if lt == TYPE_NONE || rt == TYPE_NONE { return Ast::builtin(BuiltinType::BT_VOID); }
        if lt != rt && !self.compatible(lt, unsafe ((*a).list(args))[1]) && !self.compatible(rt, unsafe ((*a).list(args))[0]) { self.err_mismatch(unsafe ((*a).list(args))[1], lt); }
        ...
        if base == self.prelude_str_type() { comparable = true; }
        if !comparable && (y.kind == TypeKind::TYPE_STRUCT || y.kind == TypeKind::TYPE_INSTANCE || y.kind == TypeKind::TYPE_ENUM) {
            ...
            if self.aggregate_of(base, (&mut om) as *mut ModuleId, (&mut od) as *mut NodeId, (&gp.d[0]) as *mut DefId, (&ga.t[0]) as *mut TypeId, (&mut gn) as *mut i32) {
                ...
                if comparable { self.find_method_cstr(om, od, "as_str"); }
            }
        }
        if !comparable {
            let mut ty = Buf96 {};
            self.render_type(lt, (&mut ty.b[0]) as *mut char, 96);
            self.errors.emit(sp.start, sp.end - sp.start, format("'{}' cannot compare values of type '{}'", diag::cstr(nm), diag::cstr((&ty.b[0]) as *const char)));
            self.errors.note(format("implement 'Eq' (an 'eq' method) for the type, or compare a field/element instead"));
        }
        return Ast::builtin(BuiltinType::BT_VOID);
    }

    fn infer_from_bounds(self: &mut Self, fmod: ModuleId, fdecl: NodeId, gids: *const NodeId, gparams: *const DefId, bound: *mut TypeId, g: i32) void {
        ...
        for pass in 0..2 {
            for i in 0..g {
                if unsafe bound[i as usize] != TYPE_NONE {
                    ...
                    let mut nb: i32 = 0;
                    self.add_bound_ifaces_full(fmod, unsafe (*fa).at_const(unsafe gids[i as usize]).as_data.generic_param.bounds, (&mut bs.b[0]) as *mut BoundIface, (&mut nb) as *mut i32, 8);
                    ...
                    for w in 0..wc.len {
                        ...
                        if unsafe (*fa).resolution(wp.ty) == unsafe gids[i as usize] { self.add_bound_ifaces_full(fmod, wp.bounds, (&mut bs.b[0]) as *mut BoundIface, (&mut nb) as *mut i32, 8); }
                    }
                    for b in 0..nb {
                        if bs.b[b as usize].n != 0 {
                            let mut tm: ModuleId = 0;
                            let mut td = NODE_NONE;
                            let mut sp = Defs4 {};
                            let mut sa = Tys4 {};
                            let mut sn: i32 = 0;
                            if self.aggregate_of(self.strip(unsafe bound[i as usize]), (&mut tm) as *mut ModuleId, (&mut td) as *mut NodeId, (&sp.d[0]) as *mut DefId, (&sa.t[0]) as *mut TypeId, (&mut sn) as *mut i32) {
                                let mut imod: ModuleId = 0;
                                let extn = self.find_extend_as(tm, td, bs.b[b as usize].iface, (&mut imod) as *mut ModuleId);
                                if extn != NODE_NONE {
                                    let ia = self.mod_ast(imod);
                                    ...
                                    let mut egn: i32 = 0;
                                    let mut k: u32 = 0;
                                    while k < egids.len && (k as i32) < sn && egn < 4 {
                                        let xg = unsafe ((*ia).list(egids))[k as usize];
                                        egp.d[egn as usize] = DefId { module: imod, node: xg };
                                        ega.t[egn as usize] = sa.t[k as usize];
                                        egn = egn + 1;
                                        k = k + 1;
                                    }
                                    ...
                                    if itf.kind == NodeKind::NODE_TYPE_PATH {
                                        ...
                                        let mut kk: u32 = 0;
                                        while kk < iargs.len && kk < (bs.b[b as usize].n as u32) {
                                            let lowered = self.lower_type_in(imod, unsafe ((*ia).list(iargs))[kk as usize]);
                                            let subst = self.subst_type(lowered, (&egp.d[0]) as *const DefId, (&ega.t[0]) as *const TypeId, egn);
                                            self.unify_infer(bs.b[b as usize].args[kk as usize], subst, gparams, bound, g);
                                            kk = kk + 1;
                                        }
                                    }
                                }
    ...
    fn fn_compatible_subst(self: &mut Self, exid: TypeId, acid: TypeId, gp: *const DefId, ga: *const TypeId, gn: i32, rp: *const DefId, ra: *const TypeId, rn: i32) bool {
        if exid != acid && self.fn_owns(acid) {
            let exy = *self.type_at(exid);
            let exn = unsafe (*self.mod_ast(exy.module)).at_const(exy.as_data.decl);
            if exn.kind != NodeKind::NODE_FUNCTION_TYPE || !exn.as_data.function_type.is_move { return false; }
        }
        let mut ep = Tys4 {};
        let mut ap = Tys4 {};
        let mut er: TypeId = TYPE_NONE;
        ...
        let en = self.fn_sig(exid, (&mut ep.t[0]) as *mut TypeId, 4, (&mut er) as *mut TypeId);
        let an = self.fn_sig(acid, (&mut ap.t[0]) as *mut TypeId, 4, (&mut ar) as *mut TypeId);
        if en != an || en > 4 { return false; }
        let er2 = self.subst_type(self.subst_type(er, gp, ga, gn), rp, ra, rn);
        if !self.ret_eq(er2, ar) { return false; }
        for i in 0..en {
            let ep2 = self.subst_type(self.subst_type(ep.t[i as usize], gp, ga, gn), rp, ra, rn);
            if ep2 != ap.t[i as usize] { return false; }
        }
        return true;
    }
    ...
    fn check_field_visibility(self: &mut Self, m: ModuleId, field: NodeId, owner: NodeId, at: tok::Span) void {
        ...
        if f.kind == NodeKind::NODE_FIELD && !f.as_data.field.is_public && !inside_owner {
            self.errors.emit(at.start, at.end - at.start, format("field '{}' is private", diag::span_str(self.source, at.start, at.end)));
        }
    }
    ...
    fn check_call(self: &mut Self, id: NodeId, want: TypeId) TypeId {
            ...
            if span_is(self.mod_src(self.ast.module), unsafe (*a).at_const(mem).as_data.name.text, "free") {
                ...
                if rt != TYPE_NONE {
                    ...
                    if rty.kind == TypeKind::TYPE_STRUCT || rty.kind == TypeKind::TYPE_INSTANCE {
                        ...
                        let mut ga = Tys4 {};
                        let mut gn: i32 = 0;
                        if self.aggregate_of(self.strip(rt), (&mut om) as *mut ModuleId, (&mut od) as *mut NodeId, (&gp.d[0]) as *mut DefId, (&ga.t[0]) as *mut TypeId, (&mut gn) as *mut i32) { resolvable = self.find_method_cstr(om, od, "free").node != NODE_NONE; }
                ...
                if !resolvable { return Ast::builtin(BuiltinType::BT_VOID); }
            }
        }
        if pck == NodeKind::NODE_MEMBER && unsafe (*a).at_const(callee_id).as_data.member.path {
            ...
            if vd.node != NODE_NONE && unsafe (*self.mod_ast(vd.module)).at_const(vd.node).kind == NodeKind::NODE_VARIANT { return self.check_variant_call(id, vd.module, vd.node, callee); }
            ...
            if fd.node != NODE_NONE && (fd.module == self.ast.module || (self.package != null && (fd.module as usize) < self.pkg_count())) && unsafe (*self.mod_ast(fd.module)).at_const(fd.node).kind == NodeKind::NODE_FIELD {
        ...
        if pck == NodeKind::NODE_MEMBER {
            ...
            if md.node != NODE_NONE && addressable && unsafe (*self.mod_ast(md.module)).at_const(md.node).kind == NodeKind::NODE_FUNCTION { self.tc_check_test_ref(md, unsafe (*a).at_const(id).span); }
        }
        // assert builtins
        if pck == NodeKind::NODE_IDENTIFIER && self.package != null {
            let ad = unsafe (*a).resolution_def(callee_id);
            if ad.node != NODE_NONE && (ad.module as usize) < self.pkg_count() && unsafe (*self.package).modules[ad.module as usize].prelude && unsafe (*self.mod_ast(ad.module)).at_const(ad.node).kind == NodeKind::NODE_FUNCTION {
                ...
                let mut akind: i32 = 0;
                if span_is(self.mod_src(ad.module), anm, "assert") { akind = 1; }
                else if span_is(self.mod_src(ad.module), anm, "assert_eq") { akind = 2; }
                else if span_is(self.mod_src(ad.module), anm, "assert_ne") { akind = 3; }
        ...
        for i in 0..args.len {
            let aid = unsafe ((*a).list(args))[i as usize];
            if self.tc_is_iface_assoc_call(aid) || unsafe (*a).at_const(aid).kind == NodeKind::NODE_CLOSURE { self.expected = self.tc_param_expected(callee, callee_id, i); }
            self.check_expr(aid);
        ...
        if ct.kind == TypeKind::TYPE_GENERIC {
            ...
            if fb != NODE_NONE { callee = self.lower_type_in(ct.module, fb); ct = *self.type_at(callee); }
        }
        if ct.kind == TypeKind::TYPE_DYN {
            let ds = self.tc_dyn_fn_sig(&ct);
            if ds != TYPE_NONE { callee = ds; ct = *self.type_at(callee); }
        }
        if ct.kind == TypeKind::TYPE_STRUCT {
            ...
            if sd.kind == NodeKind::NODE_STRUCT && sd.as_data.aggregate.is_tuple {
                ...
                if args.len != members.len {
                    ...
                    self.errors.emit(sp.start, sp.end - sp.start, format("tuple struct takes {} element{}, got {}", members.len, diag::cstr(s2), args.len));
                    return TYPE_NONE;
                }
                for k in 0..args.len {
                    let et = self.lower_type_in(sm, unsafe ((*self.mod_ast(sm)).list(members))[k as usize]);
                    let aid = unsafe ((*a).list(args))[k as usize];
                    if !self.compatible(et, aid) { self.err_mismatch(aid, et); }
                }
        ...
        if named && unsafe (*fa).at_const(fdecl).as_data.function.is_extern && self.unsafe_depth == 0 { self.err_unsafe(sp, "calling an extern \"C\" function"); }
        let clos = fk == NodeKind::NODE_CLOSURE;
        let mut params = NodeList { start: 0, len: 0 };
        let mut returns = NodeList { start: 0, len: 0 };
        if named { params = unsafe (*fa).at_const(fdecl).as_data.function.params; returns = unsafe (*fa).at_const(fdecl).as_data.function.returns; }
        else if clos { params = unsafe (*fa).at_const(fdecl).as_data.closure.params; returns = unsafe (*fa).at_const(fdecl).as_data.closure.returns; }
        else { params = unsafe (*fa).at_const(fdecl).as_data.function_type.params; returns = unsafe (*fa).at_const(fdecl).as_data.function_type.returns; }
        ...
        let ret = self.check_call_finish(id, callee, callee_id, fmod, fdecl, named, clos, params, returns, args, skip, want, fmt_builtin);
        return ret;
    }

    fn check_call_receiver(self: &mut Self, id: NodeId, callee_id: NodeId, fmod: ModuleId, params: NodeList, returns: NodeList) void {
        let a = self.cur_ast();
        let mem = unsafe (*a).at_const(callee_id).as_data.member.member;
        ...
        if is_free {
            ...
            if rty.kind == TypeKind::TYPE_REFERENCE && unsafe (*a).at_const(recv).kind == NodeKind::NODE_IDENTIFIER {
                ...
                if rd.module == self.ast.module { thru = rd.node; }
            ...
            while thru != NODE_NONE && i < self.nborrows && !through_owner {
                ...
                if b.binding == thru && b.root != NODE_NONE {
                    ...
                    if rk == NodeKind::NODE_LET || rk == NodeKind::NODE_PATTERN_NAME || rk == NodeKind::NODE_IDENTIFIER || rk == NodeKind::NODE_FOR {
                        ...
                        self.errors.emit(rsp.start, rsp.end - rsp.start, format("cannot free a borrowed value: its owning binding frees it again at scope exit"));
                        through_owner = true;
                    }
                }
                i = i + 1;
            }
            if !through_owner && rty.kind != TypeKind::TYPE_POINTER && rty.kind != TypeKind::TYPE_REFERENCE && self.tc_type_is_free(unsafe (*a).type_of(recv)) {
                ...
                if unsafe (*a).at_const(recv).kind == NodeKind::NODE_IDENTIFIER {
                    ...
                    if rd.module == self.ast.module && rd.node != NODE_NONE {
                        if self.nfreed < 256 { let k = self.nfreed; self.freed[k as usize] = rd.node; self.nfreed = k + 1; }
                    }
                }
            }
            return;
        ...
        if pt != NODE_NONE { ptk = unsafe (*fa).at_const(pt).kind; }
        if ptk != NodeKind::NODE_POINTER_TYPE && ptk != NodeKind::NODE_REFERENCE_TYPE {
            if unsafe (*a).deref_use_at(mem) != null { self.borrow_report_conflict(recv, BORROW_SHARED, recv); }
            else { self.tc_mark_move(recv); }
        } else {
            let mut bk = BORROW_SHARED;
            if unsafe (*fa).at_const(pt).as_data.indirect_type.qualifier == TypeQualifier::TYPE_QUAL_MUT { bk = BORROW_MUT; }
            ...
            if ret_ref { self.borrow_create(recv, bk, recv); }
            else { self.borrow_report_conflict(recv, bk, recv); }
        }
    }

    fn check_call_finish(self: &mut Self, id: NodeId, callee: TypeId, callee_id: NodeId, fmod: ModuleId, fdecl: NodeId, named: bool, clos: bool, params: NodeList, returns: NodeList, args: NodeList, skip: u32, want: TypeId, fmt_builtin: bool) TypeId {
        let a = self.cur_ast();
        let fa = self.mod_ast(fmod);
        let sp = unsafe (*a).at_const(id).span;
        ...
        if cn_kind == NodeKind::NODE_MEMBER && !cn_path { cdu = unsafe (*a).deref_use_at(unsafe (*a).at_const(callee_id).as_data.member.member); }
        let mut rsubp = Defs4 {};
        let mut rsuba = Tys4 {};
        let mut nrsub: i32 = 0;
        if cn_kind == NodeKind::NODE_MEMBER {
            ...
            if cdu != null { recvbase = unsafe (*cdu).target; }
            let mut rmod: ModuleId = 0;
            let mut rdecl = NODE_NONE;
            let mut gp = Defs4 {};
            let mut ga = Tys4 {};
            ...
            let agok = mdfn && self.aggregate_of(recvbase, (&mut rmod) as *mut ModuleId, (&mut rdecl) as *mut NodeId, (&gp.d[0]) as *mut DefId, (&ga.t[0]) as *mut TypeId, (&mut sn) as *mut i32);
            if agok && sn > 0 {
                let extnode = self.enclosing_extend(md.module, md.node);
                if extnode != NODE_NONE {
                    let ma = self.mod_ast(md.module);
                    ...
                    while i < g && nrsub < 4 {
                        let gid = unsafe ((*ma).list(ig))[i as usize];
                        rsubp.d[nrsub as usize] = DefId { module: md.module, node: gid };
                        rsuba.t[nrsub as usize] = ga.t[i as usize];
                        nrsub = nrsub + 1;
                        i = i + 1;
                    }
                }
        ...
        if cn_kind == NodeKind::NODE_MEMBER && !cn_path && nrsub < 4 {
            let md = unsafe (*a).resolution_def(unsafe (*a).at_const(callee_id).as_data.member.member);
            let mut tr = NODE_NONE;
            if md.node != NODE_NONE { tr = self.enclosing_trait(md.module, md.node); }
            if tr != NODE_NONE {
                let mut target = self.strip(unsafe (*a).type_of(unsafe (*a).at_const(callee_id).as_data.member.object));
                if cdu != null { target = unsafe (*cdu).target; }
                rsubp.d[nrsub as usize] = DefId { module: md.module, node: tr };
                rsuba.t[nrsub as usize] = target;
                nrsub = nrsub + 1;
                let ro = *self.type_at(target);
                if ro.kind == TypeKind::TYPE_GENERIC && nrsub < 4 {
                    nrsub = nrsub + self.bound_method_subst(ro.module, ro.as_data.decl, md, unsafe (((&mut rsubp.d[0]) as *mut DefId) + nrsub as usize), unsafe (((&mut rsuba.t[0]) as *mut TypeId) + nrsub as usize), 4 - nrsub);
                }
            }
        }
        ...
        if cn_kind == NodeKind::NODE_MEMBER && cn_path && nrsub < 4 {
            ...
            if ob.node != NODE_NONE { ob_kind = unsafe (*self.mod_ast(ob.module)).at_const(ob.node).kind; }
            if tr != NODE_NONE && ob.node != NODE_NONE && ob_kind == NodeKind::NODE_GENERIC_PARAM {
                rsubp.d[nrsub as usize] = DefId { module: md.module, node: tr };
                rsuba.t[nrsub as usize] = self.named_type_of(ob.module, ob.node);
                nrsub = nrsub + 1;
                if nrsub < 4 {
                    nrsub = nrsub + self.bound_method_subst(ob.module, ob.node, md, unsafe (((&mut rsubp.d[0]) as *mut DefId) + nrsub as usize), unsafe (((&mut rsuba.t[0]) as *mut TypeId) + nrsub as usize), 4 - nrsub);
                }
            }
            ...
            if ob.node != NODE_NONE && md.node != NODE_NONE && ob_kind == NodeKind::NODE_INTERFACE {
                ...
                if want != TYPE_NONE {
                    ...
                    agg = self.aggregate_of(sw, (&mut em) as *mut ModuleId, (&mut ed) as *mut NodeId, (&egp2.d[0]) as *mut DefId, (&ega2.t[0]) as *mut TypeId, (&mut egn2) as *mut i32);
                }
                if agg && egn2 > 0 {
                    ...
                    if ext != NODE_NONE {
                        ...
                        let mut i: u32 = 0;
                        while i < ig.len && (i as i32) < egn2 && nrsub < 4 {
                            rsubp.d[nrsub as usize] = DefId { module: md.module, node: unsafe gids[i as usize] };
                            rsuba.t[nrsub as usize] = ega2.t[i as usize];
                            nrsub = nrsub + 1;
                            i = i + 1;
                        }
                    }
        ...
        if cn_kind == NodeKind::NODE_MEMBER {
            ...
            if md.node != NODE_NONE && unsafe (*self.mod_ast(md.module)).at_const(md.node).kind == NodeKind::NODE_FUNCTION {
                ...
                if !cn_path { if cdu != null { mt = unsafe (*cdu).target; } else { mt = self.strip(unsafe (*a).type_of(unsafe (*a).at_const(callee_id).as_data.member.object)); } }
                else {
                    let ob = unsafe (*a).resolution_def(unsafe (*a).at_const(callee_id).as_data.member.object);
                    if ob.node != NODE_NONE && unsafe (*self.mod_ast(ob.module)).at_const(ob.node).kind == NodeKind::NODE_INTERFACE && want != TYPE_NONE { mt = self.strip(want); }
                    else { mt = self.strip(unsafe (*a).type_of(unsafe (*a).at_const(callee_id).as_data.member.object)); }
                }
                if mt != TYPE_NONE && !self.method_extend_bounds_hold(mt, md) { self.err_method_extend_bounds(sp, mt, md); return TYPE_NONE; }
            }
        }
        ...
        if named && unsafe (*fa).at_const(fdecl).as_data.function.generics.len != 0 {
            ...
            if g > 8 { g = 8; }
            for ii in 0..g { gparams.d[ii as usize] = DefId { module: fmod, node: unsafe ((*fa).list(gens))[ii as usize] }; gargs.t[ii as usize] = TYPE_NONE; }
            let mut bound = Tys8 {};
            ...
            if cn_kind == NodeKind::NODE_GENERIC_SPECIALIZATION {
                ...
                let mut i: u32 = 0;
                while i < tas.len && nexplicit < g { bound.t[nexplicit as usize] = unsafe (*a).type_of(unsafe ((*a).list(tas))[i as usize]); nexplicit = nexplicit + 1; i = i + 1; }
            }
            if nexplicit < g && args.len == params.len - skip {
                for i in 0..args.len {
                    let pid = unsafe ((*fa).list(params))[(i + skip) as usize];
                    self.unify_infer(self.decl_type_in(fmod, pid), unsafe (*a).type_of(unsafe ((*a).list(args))[i as usize]), (&gparams.d[0]) as *const DefId, (&mut bound.t[0]) as *mut TypeId, g);
                }
                for k in 0..g {
                    if bound.t[k as usize] != TYPE_NONE && self.type_at(bound.t[k as usize]).kind == TypeKind::TYPE_FUNCTION {
                        let fb = self.generic_fn_bound(fmod, unsafe ((*fa).list(gens))[k as usize]);
                        if fb != NODE_NONE { self.unify_infer(self.lower_type_in(fmod, fb), bound.t[k as usize], (&gparams.d[0]) as *const DefId, (&mut bound.t[0]) as *mut TypeId, g); }
                    }
                }
                self.infer_from_bounds(fmod, fdecl, unsafe (*fa).list(gens), (&gparams.d[0]) as *const DefId, (&mut bound.t[0]) as *mut TypeId, g);
                nexplicit = g;
            }
            for i in 0..nexplicit { gargs.t[i as usize] = bound.t[i as usize]; }
            gn = nexplicit;
            if gn == g {
                unsafe (*self.cur_ast()).set_type_args(id, (&gargs.t[0]) as *const TypeId, gn as u8);
                // enforce bounds (best-effort; diagnostics)
                self.check_generic_bounds(id, fmod, fdecl, gens, (&gparams.d[0]) as *const DefId, (&gargs.t[0]) as *const TypeId, gn, (&rsubp.d[0]) as *const DefId, (&rsuba.t[0]) as *const TypeId, nrsub);
            }
        }
        ...
        if bad {
            ...
            if variadic { self.errors.emit(sp.start, sp.end - sp.start, format("expected at least {} argument{}, found {}", expected, diag::cstr(s2), args.len)); }
            else { self.errors.emit(sp.start, sp.end - sp.start, format("expected {} argument{}, found {}", expected, diag::cstr(s2), args.len)); }
        } else {
            for i in 0..expected {
                let pid = unsafe ((*fa).list(params))[(i + skip) as usize];
                let raw = if_ty(named, self.decl_type_in(fmod, pid), self.lower_type_in(fmod, pid));
                let pt = self.subst_type(self.subst_type(raw, (&gparams.d[0]) as *const DefId, (&gargs.t[0]) as *const TypeId, gn), (&rsubp.d[0]) as *const DefId, (&rsuba.t[0]) as *const TypeId, nrsub);
                let aid = unsafe ((*a).list(args))[i as usize];
                if self.type_at(pt).kind == TypeKind::TYPE_FUNCTION {
                    let at = unsafe (*a).type_of(aid);
                    if pt != at && at != TYPE_NONE && self.fn_is_capturing(at) {
                        let asp = unsafe (*a).at_const(aid).span;
                        self.errors.emit(asp.start, asp.end - asp.start, format("a capturing closure cannot be passed as a bare 'fn' pointer"));
                ...
                } else if !self.compatible(pt, aid) { self.err_mismatch(aid, pt); }
            }
        }
        // C-vararg string-literal default to *const char
        if variadic && !fmt_builtin && args.len >= expected {
            let cstr = unsafe (*self.cur_ast()).intern_type(Ty { kind: TypeKind::TYPE_POINTER, qualifier: TypeQualifier::TYPE_QUAL_CONST as u8, as_data: TyAs { elem: Ast::builtin(BuiltinType::BT_CHAR) } });
            let mut i = expected;
            while i < args.len {
                let aid = unsafe ((*a).list(args))[i as usize];
                let an = unsafe (*a).at_const(aid);
                if an.kind == NodeKind::NODE_LITERAL && (an.as_data.literal.token_type == TokenType::StringLiteral || an.as_data.literal.token_type == TokenType::RawStringLiteral) { unsafe (*self.cur_ast()).set_type(aid, cstr); }
                i = i + 1;
            }
        }
        ...
        if skip == 1 && cn_kind == NodeKind::NODE_MEMBER && params.len > 0 {
            ...
            if (selfp.kind == TypeKind::TYPE_REFERENCE || selfp.kind == TypeKind::TYPE_POINTER) && selfp.qualifier == (TypeQualifier::TYPE_QUAL_MUT as u8) {
                ...
                if rvt.kind == TypeKind::TYPE_DYN {
                    if rvt.qualifier == (TypeQualifier::TYPE_QUAL_CONST as u8) { let rsp = unsafe (*a).at_const(recv).span; self.errors.emit(rsp.start, rsp.end - rsp.start, format("cannot call a '&mut self' method through '&dyn' (use '&mut dyn')")); }
                    else if rvt.qualifier == (TypeQualifier::TYPE_QUAL_NONE as u8) && !self.receiver_mutable(recv) { let rsp = unsafe (*a).at_const(recv).span; self.errors.emit(rsp.start, rsp.end - rsp.start, format("cannot call a '&mut self' method on an immutable binding (bind it with 'mut')")); }
                } else {
                    let consuming_free = rvt.kind != TypeKind::TYPE_REFERENCE && rvt.kind != TypeKind::TYPE_POINTER && span_is(self.mod_src(self.ast.module), unsafe (*a).at_const(unsafe (*a).at_const(callee_id).as_data.member.member).as_data.name.text, "free");
                    if !consuming_free {
                        self.tc_mark_capture_mut(recv);
                        if !self.receiver_mutable(recv) { let rsp = unsafe (*a).at_const(recv).span; self.errors.emit(rsp.start, rsp.end - rsp.start, format("cannot call a '&mut self' method on an immutable binding (bind it with 'mut')")); }
                    }
                }
            }
        }
        if clos && unsafe (*fa).at_const(fdecl).as_data.closure.expr_body { return unsafe (*fa).type_of(unsafe (*fa).at_const(fdecl).as_data.closure.body); }
        if named && self.tc_attr(fmod, fdecl, AttrKind::ATTR_NORETURN) != null { return unsafe (*self.cur_ast()).intern_type(Ty { kind: TypeKind::TYPE_NEVER }); }
        if returns.len != 1 {
            if returns.len > 1 {
                ...
                for i in 0..self.mret_n {
                    ...
                    let mrt = self.lower_type_in(fmod, if_node(mrn.kind == NodeKind::NODE_PARAMETER, mrn.as_data.parameter.ty, mrid));
                    self.mret_types[i as usize] = self.subst_type(self.subst_type(mrt, (&gparams.d[0]) as *const DefId, (&gargs.t[0]) as *const TypeId, gn), (&rsubp.d[0]) as *const DefId, (&rsuba.t[0]) as *const TypeId, nrsub);
                }
                self.mret_call = id;
            }
            return TYPE_NONE;
        ...
        return self.subst_type(self.subst_type(ret, (&gparams.d[0]) as *const DefId, (&gargs.t[0]) as *const TypeId, gn), (&rsubp.d[0]) as *const DefId, (&rsuba.t[0]) as *const TypeId, nrsub);
    }

    fn check_generic_bounds(self: &mut Self, id: NodeId, fmod: ModuleId, fdecl: NodeId, gens: NodeList, gparams: *const DefId, gargs: *const TypeId, gn: i32, rsubp: *const DefId, rsuba: *const TypeId, nrsub: i32) void {
        let a = self.cur_ast();
        let fa = self.mod_ast(fmod);
        let sp = unsafe (*a).at_const(id).span;
        for i in 0..gn {
            ...
            for b in 0..pb.len {
                ...
                if unsafe (*fa).at_const(bid).kind == NodeKind::NODE_FUNCTION_TYPE {
                    ...
                    if garg != TYPE_NONE {
                        ...
                        if gy.kind != TypeKind::TYPE_GENERIC && (gy.kind != TypeKind::TYPE_FUNCTION || !self.fn_compatible_subst(bt, garg, gparams, gargs, gn, rsubp, rsuba, nrsub)) {
                            let mut tn = Buf96 {};
                            self.render_type(garg, (&mut tn.b[0]) as *mut char, 96);
                            let bsp = unsafe (*fa).at_const(bid).span;
                            self.errors.emit(sp.start, sp.end - sp.start, format("type '{}' does not satisfy bound '{}'", diag::cstr((&tn.b[0]) as *const char), diag::span_str(self.mod_src(fmod), bsp.start, bsp.end)));
                        }
                    }
                } else {
                ...
                if unsafe (*fa).at_const(wbid).kind == NodeKind::NODE_FUNCTION_TYPE {
                    ...
                    if wt != TYPE_NONE {
                        ...
                        if wy.kind != TypeKind::TYPE_GENERIC && (wy.kind != TypeKind::TYPE_FUNCTION || !self.fn_compatible_subst(bt, wt, gparams, gargs, gn, rsubp, rsuba, nrsub)) {
                            ...
                            self.errors.emit(sp.start, sp.end - sp.start, format("type '{}' does not satisfy a where-clause bound", diag::cstr((&tn.b[0]) as *const char)));
                        }
                    }
                } else {
...
fn if_u8(c: bool, a: u8, b: u8) u8 { if c { return a; } return b; }
...
extend TypeChecker {
    fn check_member(self: &mut Self, id: NodeId, prefer_method: bool) TypeId {
        ...
        if obj == TYPE_NONE { return TYPE_NONE; }
        ...
        let mut ga = Tys4 {};
        let mut gn: i32 = 0;
        if self.aggregate_of(base, (&mut bmod) as *mut ModuleId, (&mut bdecl) as *mut NodeId, (&gp.d[0]) as *mut DefId, (&ga.t[0]) as *mut TypeId, (&mut gn) as *mut i32) {
            ...
            while di < name.end && digits { if unsafe (self.source)[di as usize] < '0' as u8 || unsafe (self.source)[di as usize] > '9' as u8 { digits = false; } di = di + 1; }
            if digits {
                ...
                if bd0.kind == NodeKind::NODE_STRUCT && bd0.as_data.aggregate.is_tuple {
                    ...
                    if idx >= bd0.as_data.aggregate.members.len {
                        self.errors.emit(name.start, name.end - name.start, format("tuple struct has no element {}", idx));
                        return TYPE_NONE;
                    }
                ...
                fhit = self.find_member_cstr(bmod, bdecl, diag::cstr((&fld.b[0]) as *const char));
            }
            if fhit == NODE_NONE && prefer_method {
                mhit = self.find_method(bmod, bdecl, name);
                if mhit.node == NODE_NONE { fhit = self.find_member(bmod, bdecl, name); }
            } else if fhit == NODE_NONE {
                fhit = self.find_member(bmod, bdecl, name);
                if fhit == NODE_NONE { mhit = self.find_method(bmod, bdecl, name); }
            }
            if mhit.node != NODE_NONE {
                unsafe (*self.cur_ast()).set_resolution_def(mname, mhit);
                return self.subst_type(self.decl_type_in(mhit.module, mhit.node), (&gp.d[0]) as *const DefId, (&ga.t[0]) as *const TypeId, gn);
            }
            if fhit != NODE_NONE {
                ...
                if unsafe (*self.mod_ast(bmod)).at_const(fhit).kind == NodeKind::NODE_FIELD {
                    ...
                    if self.unsafe_depth == 0 && self.through_raw_pointer(obj) { self.err_unsafe(unsafe (*a).at_const(id).span, "accessing a field through a raw pointer"); }
                }
            ...
            if prefer_method {
                ...
                if dm.node != NODE_NONE {
                    ...
                    return self.subst_type(self.decl_type_in(dm.module, dm.node), (&gp.d[0]) as *const DefId, (&ga.t[0]) as *const TypeId, gn);
                }
            }
        }
        // builtin method
        ...
        if bty.kind == TypeKind::TYPE_BUILTIN && self.package != null {
            ...
            if bd != NODE_NONE {
                ...
                if mhit.node == NODE_NONE { mhit = self.find_default_method(unsafe (*self.package).core_module, bd, name); }
                if mhit.node != NODE_NONE { unsafe (*self.cur_ast()).set_resolution_def(mname, mhit); return self.decl_type_in(mhit.module, mhit.node); }
            }
        }
        ...
        if bt2.kind == TypeKind::TYPE_GENERIC || bt2.kind == TypeKind::TYPE_DYN {
            ...
            if gd.kind == NodeKind::NODE_INTERFACE {
                ...
                if m.node != NODE_NONE { unsafe (*self.cur_ast()).set_resolution_def(mname, m); return self.decl_type_in(m.module, m.node); }
            } else {
                let mut iface = DefId { module: 0, node: NODE_NONE };
        ...
        if prefer_method {
            ...
            if conv.node != NODE_NONE { unsafe (*self.cur_ast()).set_resolution_def(mname, conv); return self.decl_type_in(conv.module, conv.node); }
        }
        // auto-deref chain
        ...
        if prefer_method {
            ...
            loop {
                ...
                if dry.kind != TypeKind::TYPE_REFERENCE && dry.kind != TypeKind::TYPE_POINTER { break; }
                let target = dry.as_data.elem;
                let mut cyc = false;
                for z in 0..nseen { if seen.t[z as usize] == target { cyc = true; } }
                if cyc { self.errors.emit(name.start, name.end - name.start, format("cyclic deref chain while resolving '{}'", diag::span_str(self.source, name.start, name.end))); return TYPE_NONE; }
                du.recv[du.n as usize] = cur;
                du.method[du.n as usize] = dm;
                du.n = du.n + 1;
                seen.t[nseen as usize] = target;
                nseen = nseen + 1;
                let mut tm: ModuleId = 0;
                let mut td = NODE_NONE;
                let mut tgp = Defs4 {};
                ...
                if self.aggregate_of(target, (&mut tm) as *mut ModuleId, (&mut td) as *mut NodeId, (&tgp.d[0]) as *mut DefId, (&tga.t[0]) as *mut TypeId, (&mut tgn) as *mut i32) {
                    mhit = self.find_method(tm, td, name);
                    if mhit.node == NODE_NONE { mhit = self.find_default_method(tm, td, name); }
                } else if tty.kind == TypeKind::TYPE_BUILTIN && self.package != null {
                    let bd = unsafe (*self.package).builtin_decl(tty.as_data.builtin);
                    if bd != NODE_NONE { mhit = self.find_method(unsafe (*self.package).core_module, bd, name); if mhit.node == NODE_NONE { mhit = self.find_default_method(unsafe (*self.package).core_module, bd, name); } }
                }
                if mhit.node != NODE_NONE {
                    let sk = self.method_self_kind(mhit);
                    if sk == 0 && tty.kind != TypeKind::TYPE_BUILTIN {
                        self.errors.emit(name.start, name.end - name.start, format("cannot call a by-value 'self' method through auto-deref"));
                        return TYPE_NONE;
                    }
                    if sk == 2 {
                        for hi in 0..du.n {
                            ...
                            if dmm.node == NODE_NONE {
                                ...
                                self.errors.emit(name.start, name.end - name.start, format("cannot call a '&mut self' method through '{}': it has 'deref' but no 'deref_mut'", diag::cstr((&tn.b[0]) as *const char)));
                                return TYPE_NONE;
                            }
                    ...
                    unsafe (*self.cur_ast()).set_resolution_def(mname, mhit);
                    return self.subst_type(self.decl_type_in(mhit.module, mhit.node), (&tgp.d[0]) as *const DefId, (&tga.t[0]) as *const TypeId, tgn);
                }
                cur = target;
            }
            deref_capped = du.n == 8;
        ...
        self.errors.emit(name.start, name.end - name.start, format("no field or method '{}' on '{}'", diag::span_str(self.source, name.start, name.end), diag::cstr((&ty.b[0]) as *const char)));
        if deref_capped { self.errors.note(format("auto-deref stopped after its maximum of 8 hops")); }
        else { self.errors.note(format("fields are accessed as 'value.name'; methods must be declared in an 'extend' block or provided by a bound")); }
        return TYPE_NONE;
    }
    ...
    fn check_path_member(self: &mut Self, id: NodeId, expected: TypeId) TypeId {
        ...
        if direct.node != NODE_NONE {
            if unsafe (*self.mod_ast(direct.module)).at_const(direct.node).kind == NodeKind::NODE_FUNCTION {
        ...
        if on_kind == NodeKind::NODE_IDENTIFIER {
            ...
            if bdecl == NODE_NONE && self.package != null {
                ...
                if bb >= 0 { bnd = unsafe (*self.package).builtin_decl(bb as BuiltinType); }
                if bnd != NODE_NONE { bmod = unsafe (*self.package).core_module; bdecl = bnd; }
            }
            if bdecl != NODE_NONE && unsafe (*self.mod_ast(bmod)).at_const(bdecl).kind == NodeKind::NODE_TYPE_ALIAS {
                ...
                if at != TYPE_NONE && at != TYPE_ERROR {
                    ...
                    if aty.kind == TypeKind::TYPE_BUILTIN && self.package != null { bmod = unsafe (*self.package).core_module; bdecl = unsafe (*self.package).builtin_decl(aty.as_data.builtin); }
                    else if aty.kind == TypeKind::TYPE_STRUCT || aty.kind == TypeKind::TYPE_ENUM { bmod = aty.module; bdecl = aty.as_data.decl; }
                    else if aty.kind == TypeKind::TYPE_INSTANCE { let ai = *unsafe (*self.cur_ast()).instance(aty.as_data.inst); bmod = ai.module; bdecl = ai.decl; inst_ty = at; unsafe (*self.cur_ast()).set_type(obj, at); }
                }
            }
            if bdecl != NODE_NONE {
                let bdn = unsafe (*self.mod_ast(bmod)).at_const(bdecl);
                if (bdn.kind == NodeKind::NODE_STRUCT || bdn.kind == NodeKind::NODE_ENUM) && bdn.as_data.aggregate.generics.len > 0 && self.agg_has_default_at(bmod, bdecl, 0) {
                    let mut ta = Tys4 {};
                    let mut tn: u8 = 0;
                    self.apply_default_args(bmod, bdecl, (&mut ta.t[0]) as *mut TypeId, (&mut tn) as *mut u8);
                    if tn == (bdn.as_data.aggregate.generics.len as u8) { inst_ty = unsafe (*self.cur_ast()).intern_instance(bmod, bdecl, (&ta.t[0]) as *const TypeId, tn); unsafe (*self.cur_ast()).set_type(obj, inst_ty); }
                }
            }
        } else {
            let bt = self.check_expr(obj);
            let ty = *self.type_at(bt);
            if ty.kind == TypeKind::TYPE_INSTANCE { let it = *unsafe (*self.cur_ast()).instance(ty.as_data.inst); bmod = it.module; bdecl = it.decl; inst_ty = bt; }
            else if ty.kind == TypeKind::TYPE_BUILTIN && self.package != null { bmod = unsafe (*self.package).core_module; bdecl = unsafe (*self.package).builtin_decl(ty.as_data.builtin); }
            else { bmod = ty.module; bdecl = if_node(bt != TYPE_NONE && (ty.kind == TypeKind::TYPE_STRUCT || ty.kind == TypeKind::TYPE_ENUM), ty.as_data.decl, NODE_NONE); }
        }
        let mname = self.name_span(mem);
        let mut bd_kind = NodeKind::NODE_NONE_KIND;
        if bdecl != NODE_NONE { bd_kind = unsafe (*self.mod_ast(bmod)).at_const(bdecl).kind; }
        if bdecl != NODE_NONE && bd_kind == NodeKind::NODE_GENERIC_PARAM {
            ...
            if m.node != NODE_NONE { unsafe (*self.cur_ast()).set_resolution_def(mem, m); return self.decl_type_in(m.module, m.node); }
        }
        if bdecl != NODE_NONE && bd_kind == NodeKind::NODE_INTERFACE && expected != TYPE_NONE {
            ...
            let mut ega = Tys4 {};
            let mut egn: i32 = 0;
            if self.aggregate_of(self.strip(expected), (&mut emod) as *mut ModuleId, (&mut edecl) as *mut NodeId, (&egp.d[0]) as *mut DefId, (&ega.t[0]) as *mut TypeId, (&mut egn) as *mut i32) {
        ...
        if bdecl == NODE_NONE || (bd_kind != NodeKind::NODE_STRUCT && bd_kind != NodeKind::NODE_ENUM) {
            ...
            if bdecl != NODE_NONE && bd_kind == NodeKind::NODE_INTERFACE {
                self.errors.emit(sp.start, sp.end - sp.start, format("cannot infer the implementing type for this interface call"));
            } else {
        ...
        self.errors.emit(mname.start, mname.end - mname.start, format("no {} '{}' on this type", diag::cstr(kindw), diag::span_str(self.source, mname.start, mname.end)));
        return TYPE_NONE;
    }
    ...
    fn check_struct_init(self: &mut Self, id: NodeId) TypeId {
        ...
        let mut ga = Tys4 {};
        let mut gn: i32 = 0;
        if !self.aggregate_of(sty, (&mut smod) as *mut ModuleId, (&mut decl) as *mut NodeId, (&gp.d[0]) as *mut DefId, (&ga.t[0]) as *mut TypeId, (&mut gn) as *mut i32) { decl = NODE_NONE; }
        ...
        if unsafe (*a).at_const(stn).kind == NodeKind::NODE_TYPE_PATH {
            ...
            if parts.len >= 2 {
                ...
                if vd.node != NODE_NONE && unsafe (*self.mod_ast(vd.module)).at_const(vd.node).kind == NodeKind::NODE_VARIANT { variant = vd.node; vmod = vd.module; }
            }
        }
        ...
        for i in 0..fields.len {
            ...
            if variant == NODE_NONE && decl != NODE_NONE && self.tc_is_iface_assoc_call(fval) {
                ...
                let ft = if_ty(field != NODE_NONE, self.subst_type(self.decl_type_in(smod, field), (&gp.d[0]) as *const DefId, (&ga.t[0]) as *const TypeId, gn), TYPE_NONE);
                if ft != TYPE_NONE && unsafe (*self.cur_ast()).type_concrete(ft) { self.expected = ft; }
            }
            ...
            if variant != NODE_NONE {
                ...
                for j in 0..vpl.len {
                    ...
                    if pf.kind == NodeKind::NODE_FIELD && spans_eq2(self.source, fname, self.mod_src(vmod), unsafe (*va).at_const(pf.as_data.field.name).as_data.name.text) { field = pfid; break; }
                }
                if field == NODE_NONE {
                    let mut ty = Buf96 {};
                    self.render_type(sty, (&mut ty.b[0]) as *mut char, 96);
                    self.errors.emit(fname.start, fname.end - fname.start, format("no field '{}' on '{}'", diag::span_str(self.source, fname.start, fname.end), diag::cstr((&ty.b[0]) as *const char)));
                } else {
                    ...
                    if !self.compatible(ft, fval) { self.err_mismatch(fval, ft); }
                }
                continue;
            }
            if decl == NODE_NONE { continue; }
            let field = self.find_member(smod, decl, fname);
            if field == NODE_NONE {
                let mut ty = Buf96 {};
                self.render_type(sty, (&mut ty.b[0]) as *mut char, 96);
            ...
            let ft = self.subst_type(self.decl_type_in(smod, field), (&gp.d[0]) as *const DefId, (&ga.t[0]) as *const TypeId, gn);
            if !self.compatible(ft, fval) { self.err_mismatch(fval, ft); }
        }
        return sty;
    }
    ...
    fn check_if_stmt(self: &mut Self, id: NodeId) void {
        ...
        if c != TYPE_NONE && !self.is_bool(c) {
            ...
            self.errors.emit(sp.start, sp.end - sp.start, format("if condition must be 'bool', found '{}'", diag::cstr((&ty.b[0]) as *const char)));
        }
        ...
        let mut ovf = false;
        if !self.tc_stmt_returns(ifd.then_branch) { if self.tc_flow_collect((&mut acc) as *mut FlowState) { ovf = true; } }
        self.tc_flow_set(&pre);
        self.check_stmt(ifd.else_branch);
        if !self.tc_stmt_returns(ifd.else_branch) { if self.tc_flow_collect((&mut acc) as *mut FlowState) { ovf = true; } }
        self.tc_flow_set(&acc);
        if ovf { self.tc_flow_overflow(ifd.condition); }
    }
    ...
    fn pattern_irrefutable(self: &Self, id: NodeId) bool {
        ...
        if p.kind == NodeKind::NODE_PATTERN_WILDCARD || p.kind == NodeKind::NODE_IDENTIFIER { return true; }
        if p.kind == NodeKind::NODE_PATTERN_NAME || p.kind == NodeKind::NODE_PATTERN_TUPLE || p.kind == NodeKind::NODE_PATTERN_STRUCT {
            ...
            if nameId != NODE_NONE {
                ...
                if d.node != NODE_NONE && unsafe (*self.mod_ast(d.module)).at_const(d.node).kind == NodeKind::NODE_VARIANT { return false; }
            }
            if p.kind == NodeKind::NODE_PATTERN_NAME { return true; }
            let children = p.as_data.pattern.children;
            for i in 0..children.len { if !self.pattern_irrefutable(unsafe ((*a).list(children))[i as usize]) { return false; } }
            return true;
        }
        ...
        if p.kind == NodeKind::NODE_PATTERN_OR {
            let children = p.as_data.pattern.children;
            for i in 0..children.len { if self.pattern_irrefutable(unsafe ((*a).list(children))[i as usize]) { return true; } }
            return false;
        }
        return false;
    }
    fn pattern_covered_variant(self: &Self, id: NodeId) NodeId {
        let a = self.cur_ast();
        let p = unsafe (*a).at_const(id);
        if p.kind != NodeKind::NODE_PATTERN_NAME && p.kind != NodeKind::NODE_PATTERN_TUPLE && p.kind != NodeKind::NODE_PATTERN_STRUCT { return NODE_NONE; }
        let nameId = p.as_data.pattern.name;
        if nameId == NODE_NONE { return NODE_NONE; }
        let d = unsafe (*a).resolution_def(nameId);
        if d.node == NODE_NONE || unsafe (*self.mod_ast(d.module)).at_const(d.node).kind != NodeKind::NODE_VARIANT { return NODE_NONE; }
        let children = p.as_data.pattern.children;
        for i in 0..children.len { if !self.pattern_irrefutable(unsafe ((*a).list(children))[i as usize]) { return NODE_NONE; } }
        return d.node;
    }
    fn match_arm_coverage(self: &Self, pid: NodeId, emod: ModuleId, has_ea: bool, variants: NodeList, covered: *mut u64, catchall: *mut bool, tcov: *mut bool, fcov: *mut bool) void {
        if pid == NODE_NONE { return; }
        let a = self.cur_ast();
        let p = unsafe (*a).at_const(pid);
        if p.kind == NodeKind::NODE_PATTERN_OR {
            let children = p.as_data.pattern.children;
            for i in 0..children.len { self.match_arm_coverage(unsafe ((*a).list(children))[i as usize], emod, has_ea, variants, covered, catchall, tcov, fcov); }
            return;
        }
        if self.pattern_irrefutable(pid) { unsafe *catchall = true; return; }
        if p.kind == NodeKind::NODE_PATTERN_LITERAL {
            let v = unsafe (*a).at_const(p.as_data.single.value);
            if v.kind == NodeKind::NODE_LITERAL && v.as_data.literal.token_type == TokenType::True { unsafe *tcov = true; }
            else if v.kind == NodeKind::NODE_LITERAL && v.as_data.literal.token_type == TokenType::False { unsafe *fcov = true; }
            return;
        }
        let var = self.pattern_covered_variant(pid);
        if var == NODE_NONE || !has_ea { return; }
    ...
    fn check_match_exhaustive(self: &mut Self, id: NodeId, scrut: TypeId) void {
        ...
        let mut ga = Tys4 {};
        let mut gn: i32 = 0;
        let agok = self.aggregate_of(base, (&mut emod) as *mut ModuleId, (&mut edecl) as *mut NodeId, (&gp.d[0]) as *mut DefId, (&ga.t[0]) as *mut TypeId, (&mut gn) as *mut i32);
        let is_enum = agok && unsafe (*self.mod_ast(emod)).at_const(edecl).kind == NodeKind::NODE_ENUM;
        let mut variants = NodeList { start: 0, len: 0 };
        if is_enum { variants = unsafe (*self.mod_ast(emod)).at_const(edecl).as_data.aggregate.members; }
        ...
        for i in 0..arms.len {
            ...
            if arm.as_data.match_arm.guard == NODE_NONE { self.match_arm_coverage(arm.as_data.match_arm.pattern, emod, is_enum, variants, (&mut covered.c[0]) as *mut u64, (&mut catchall) as *mut bool, (&mut tcov) as *mut bool, (&mut fcov) as *mut bool); }
        }
        if catchall { return; }
        let sp = unsafe (*a).at_const(unsafe (*a).at_const(id).as_data.match_expr.value).span;
        if is_enum && variants.len <= MATCH_MAX_VARIANTS {
            let mut nmiss: u32 = 0;
            for k in 0..variants.len { if (covered.c[(k >> 6) as usize] & (1u64 << ((k & 63) as u64))) == 0 { nmiss = nmiss + 1; } }
            if nmiss == 0 { return; }
            let mut s2 = "s".ptr() as *const char;
            if nmiss == 1 { s2 = "".ptr() as *const char; }
            self.errors.emit(sp.start, sp.end - sp.start, format("switch is not exhaustive: missing {} variant{}", nmiss, diag::cstr(s2)));
    ...
    fn check_assignment(self: &mut Self, id: NodeId) TypeId {
        ...
        if plain && unsafe (*a).at_const(bd.left).kind == NodeKind::NODE_IDENTIFIER { ld = unsafe (*a).resolution_def(bd.left); }
        let lhs_local = ld.node != NODE_NONE && ld.module == self.ast.module;
        if lhs_local { self.tc_init(ld.node); self.tc_unmark_move(ld.node); }
        ...
        if ref_rebind {
            for i in 0..self.nborrows { if self.borrows[i as usize].binding == ld.node { self.borrow_tombstone_at(i); } }
        }
        let bm = self.borrow_mark();
        ...
        if ref_rebind {
            if self.nborrows > bm {
                ...
                let mut k = bm;
                while k < self.nborrows { self.borrows[k as usize].binding = ld.node; self.borrows[k as usize].region = region; k = k + 1; }
            } else { self.borrow_transfer_ref(bd.right, ld.node); }
        }
    ...
    fn check_closure(self: &mut Self, id: NodeId, cwant: TypeId) TypeId {
        ...
        for i in 0..params.len {
            ...
            if unsafe (*a).at_const(pid).as_data.parameter.ty != NODE_NONE { continue; }
            if sn < 0 {
                if cwant != TYPE_NONE && self.type_at(cwant).kind == TypeKind::TYPE_FUNCTION { sn = self.fn_sig(cwant, (&mut sigp.t[0]) as *mut TypeId, 8, (&mut sigr) as *mut TypeId); }
                else { sn = 0; }
            }
            if (i as i32) < sn && sigp.t[i as usize] != TYPE_NONE { unsafe (*self.cur_ast()).set_type(pid, sigp.t[i as usize]); }
                ...
                self.errors.emit(psp.start, psp.end - psp.start, format("closure parameter needs a type annotation (no expected function type supplies one)"));
                self.errors.note(format("annotate it (`|x: i32| ..`) or bind the closure where a 'fn' type is expected"));
            }
        }
        ...
        for i in 0..caps.len {
            ...
            if cty != TYPE_NONE && !is_mut && self.type_at(cty).kind == TypeKind::TYPE_ARRAY {
                let sp = unsafe (*a).at_const(id).span;
                self.errors.emit(sp.start, sp.end - sp.start, format("closure cannot capture a fixed-size array by copy; capture a slice instead"));
                continue;
            }
            if cty == TYPE_NONE || is_mut || !self.tc_type_is_free(cty) { continue; }
            if self.is_moved(cid) {
                let sp = unsafe (*a).at_const(id).span;
                self.errors.emit(sp.start, sp.end - sp.start, format("closure captures a moved value (use of moved value)"));
            }
            for f in 0..self.nclos { if self.tc_capture_index(self.clos_stack[f as usize], cid) >= 0 { let sp = unsafe (*a).at_const(id).span; self.errors.emit(sp.start, sp.end - sp.start, format("cannot take ownership of a value also captured by an enclosing closure")); break; } }
            for b in 0..self.nborrows {
                if self.borrows[b as usize].root == cid {
                    ...
                    else { let sp = unsafe (*a).at_const(id).span; self.errors.emit(sp.start, sp.end - sp.start, format("cannot capture this value while it is borrowed")); break; }
                }
            }
            if self.nmoved < 1024 { let k = self.nmoved; self.moved[k as usize] = cid; self.nmoved = k + 1; self.ms_bit_set(cid); }
        }
        return unsafe (*self.cur_ast()).intern_type(Ty { kind: TypeKind::TYPE_FUNCTION, module: self.ast.module, as_data: TyAs { decl: id } });
    }
    ...
    fn check_index(self: &mut Self, id: NodeId, addr_ctx: bool, place_use: bool) TypeId {
        ...
        if self.type_at(obj).kind == TypeKind::TYPE_REFERENCE {
            ...
            while y.kind == TypeKind::TYPE_REFERENCE { p = y.as_data.elem; y = *self.type_at(p); }
            if y.kind != TypeKind::TYPE_ARRAY { obj = p; }
        }
        ...
        if idxn_kind == NodeKind::NODE_RANGE {
            ...
            if ot.kind == TypeKind::TYPE_ARRAY || ot.kind == TypeKind::TYPE_POINTER {
                if ot.kind == TypeKind::TYPE_POINTER && self.unsafe_depth == 0 { self.err_unsafe(unsafe (*a).at_const(obj_n).span, "slicing a raw pointer"); }
                ...
                if self.aggregate_of(self.strip(obj), (&mut om) as *mut ModuleId, (&mut od) as *mut NodeId, (&gp.d[0]) as *mut DefId, (&ga.t[0]) as *mut TypeId, (&mut gn) as *mut i32) {
                    ...
                    if md.node == NODE_NONE {
                        ...
                        self.errors.emit(sp.start, sp.end - sp.start, format("'{}' has no 'index_range' method for '[..]'", diag::cstr((&ty.b[0]) as *const char)));
                    } else if !self.method_extend_bounds_hold(self.strip(obj), md) { self.err_method_extend_bounds(sp, self.strip(obj), md); }
                    else {
                        if unsafe (*a).at_const(index_n).as_data.pattern_range.end == NODE_NONE && self.find_method_cstr(om, od, "len").node == NODE_NONE {
                            let mut ty = Buf96 {};
                            self.render_type(self.strip(obj), (&mut ty.b[0]) as *mut char, 96);
                            self.errors.emit(sp.start, sp.end - sp.start, format("an open-ended range index needs a 'len' method on '{}' to supply the end", diag::cstr((&ty.b[0]) as *const char)));
                        }
                        self.prelude_range_type(Ast::builtin(BuiltinType::BT_USIZE));
                        user_result = self.tc_method_ret(self.strip(obj), md);
                    }
                }
            } else if obj != TYPE_NONE { let sp = unsafe (*a).at_const(obj_n).span; self.errors.emit(sp.start, sp.end - sp.start, format("cannot slice this expression")); }
            ...
            if bstart != NODE_NONE { let bt = self.check_expr(bstart); if bt != TYPE_NONE && !self.is_int(bt) && unsafe (*a).at_const(bstart).kind != NodeKind::NODE_LITERAL { let sp = unsafe (*a).at_const(bstart).span; self.errors.emit(sp.start, sp.end - sp.start, format("range bound must be an integer")); } }
            if bend != NODE_NONE { let bt = self.check_expr(bend); if bt != TYPE_NONE && !self.is_int(bt) && unsafe (*a).at_const(bend).kind != NodeKind::NODE_LITERAL { let sp = unsafe (*a).at_const(bend).span; self.errors.emit(sp.start, sp.end - sp.start, format("range bound must be an integer")); } }
        ...
        if obj != TYPE_NONE {
            let mut selem: TypeId = TYPE_NONE;
            if ot.kind == TypeKind::TYPE_ARRAY || ot.kind == TypeKind::TYPE_POINTER {
                if ot.kind == TypeKind::TYPE_POINTER && self.unsafe_depth == 0 { self.err_unsafe(unsafe (*a).at_const(obj_n).span, "indexing a raw pointer"); }
                ...
                if self.aggregate_of(self.strip(obj), (&mut om) as *mut ModuleId, (&mut od) as *mut NodeId, (&gp.d[0]) as *mut DefId, (&ga.t[0]) as *mut TypeId, (&mut gn) as *mut i32) {
                    ...
                    if md.node == NODE_NONE {
                        let mut ty = Buf96 {};
                        self.render_type(self.strip(obj), (&mut ty.b[0]) as *mut char, 96);
                        self.errors.emit(sp.start, sp.end - sp.start, format("'{}' has no 'index' method for '[]'", diag::cstr((&ty.b[0]) as *const char)));
                        ...
                        if !self.operand_fits_param(p1, index_n) { self.err_mismatch(index_n, p1); }
            ...
            } else { let sp = unsafe (*a).at_const(obj_n).span; self.errors.emit(sp.start, sp.end - sp.start, format("cannot index this expression")); }
        }
        if !overloaded && idx != TYPE_NONE && !self.is_int(idx) && unsafe (*a).at_const(index_n).kind != NodeKind::NODE_LITERAL { let sp = unsafe (*a).at_const(index_n).span; self.errors.emit(sp.start, sp.end - sp.start, format("index must be an integer")); }
        return result;
    }
    ...
    fn check_match_expr(self: &mut Self, id: NodeId) TypeId {
        ...
        if sy.kind == TypeKind::TYPE_REFERENCE || sy.kind == TypeKind::TYPE_POINTER { if sy.qualifier == (TypeQualifier::TYPE_QUAL_MUT as u8) { bind_ref = 2; } else { bind_ref = 1; } }
        if bind_ref == 0 { self.borrow_release_to(bm); self.tc_mark_move(unsafe (*a).at_const(id).as_data.match_expr.value); }
        let arms = unsafe (*a).at_const(id).as_data.match_expr.arms;
        let mut first = true;
        let mut ovf = false;
        ...
        for i in 0..arms.len {
            ...
            if arm.guard != NODE_NONE && g != TYPE_NONE && !self.is_bool(g) { let sp = unsafe (*a).at_const(arm.guard).span; self.errors.emit(sp.start, sp.end - sp.start, format("match guard must be 'bool'")); }
            ...
            if first { result = body; first = false; }
            else if result != TYPE_NONE && self.type_at(result).kind == TypeKind::TYPE_NEVER { result = body; }
            else if !body_never && result != body && body != TYPE_NONE && result != TYPE_NONE { self.err_mismatch(arm.body, result); result = TYPE_NONE; }
        }
        if arms.len != 0 { self.tc_flow_set(&acc); }
        if ovf { self.tc_flow_overflow(id); }
        self.borrow_release_to(bm);
        ...
        switch nk {
            ...
            NODE_IDENTIFIER => {
                ...
                if d.module == self.ast.module && d.node != NODE_NONE {
                    ...
                    if !addr_ctx && self.tc_is_uninit(d.node) { let sp = unsafe (*a).at_const(id).span; self.errors.emit(sp.start, sp.end - sp.start, format("use of possibly uninitialized value")); }
                    if !addr_ctx && !place_use && self.borrow_conflicting_read(id) { let sp = unsafe (*a).at_const(id).span; self.errors.emit(sp.start, sp.end - sp.start, format("cannot use this value while it is mutably borrowed")); }
                }
            },
            NODE_UNARY => {
                result = self.check_unary(id);
                if unsafe (*a).at_const(id).as_data.unary.op == TokenType::Star && !addr_ctx && !place_use && self.borrow_conflicting_read(id) { let sp = unsafe (*a).at_const(id).span; self.errors.emit(sp.start, sp.end - sp.start, format("cannot use this value while it is mutably borrowed")); }
            },
            ...
            NODE_MEMBER => {
                ...
                if value_read && !place_use && self.borrow_conflicting_read(id) { let sp = unsafe (*a).at_const(id).span; self.errors.emit(sp.start, sp.end - sp.start, format("cannot use this value while it is mutably borrowed")); }
            },
            NODE_CAST => {
                ...
                if src != TYPE_NONE && dst != TYPE_NONE && src != dst {
                    ...
                    if (aggregate && !enum_int) || complex_lossy {
                        ...
                        let sp = unsafe (*a).at_const(id).span;
                        self.errors.emit(sp.start, sp.end - sp.start, format("invalid cast from '{}' to '{}'", diag::cstr((&s.b[0]) as *const char), diag::cstr((&d.b[0]) as *const char)));
                    }
                }
                result = dst;
            },
            NODE_SIZEOF | NODE_ALIGNOF => {
                ...
                if dk == NodeKind::NODE_LET || dk == NodeKind::NODE_PARAMETER || dk == NodeKind::NODE_FOR || dk == NodeKind::NODE_IDENTIFIER || dk == NodeKind::NODE_PATTERN_NAME || dk == NodeKind::NODE_CONST {
                    ...
                    if vt == TYPE_NONE { let vsp = unsafe (*a).at_const(v).span; self.errors.emit(vsp.start, vsp.end - vsp.start, format("cannot take the size of this value here")); }
            ...
            NODE_VA_EXPR => {
                ...
                if apt != TYPE_NONE {
                    ...
                    if !(ay.kind == TypeKind::TYPE_BUILTIN && ay.as_data.builtin == BuiltinType::BT_VALIST) {
                        ...
                        self.errors.emit(sp.start, sp.end - sp.start, format("expected a 'va_list', found '{}'", diag::cstr((&ty.b[0]) as *const char)));
                    }
                }
    ...
    fn check_literal(self: &mut Self, id: NodeId) TypeId {
        ...
        if tt == TokenType::IntegerLiteral {
            ...
            if sb != BuiltinType::BT_COUNT { result = Ast::builtin(sb); }
            let mut p = unsafe (self.source + lr.start as usize);
            ...
            while i < len && !overflow {
                ...
                let mut d: u64 = 0;
                if ch <= '9' as u8 { d = (ch - '0' as u8) as u64; } else { d = ((ch | 0x20u8) - 'a' as u8 + 10u8) as u64; }
                if acc > (0xFFFFFFFFFFFFFFFFu64 - d) / base { overflow = true; } else { acc = acc * base + d; }
                i = i + 1;
            }
            let sp = unsafe (*a).at_const(id).span;
            if overflow { self.errors.emit(sp.start, sp.end - sp.start, format("integer literal is too large to fit in a 64-bit integer")); }
            else if sb != BuiltinType::BT_COUNT && bt_int_max(sb) != 0 && acc > bt_int_max(sb) { self.errors.emit(sp.start, sp.end - sp.start, format("integer literal does not fit in its suffixed type")); }
            return result;
        }
        if tt == TokenType::FloatLiteral {
            let sb = unsafe ast_numeric_suffix(self.source, lr.start, lr.end, null);
            if sb == BuiltinType::BT_F64 { return Ast::builtin(BuiltinType::BT_F64); }
            return Ast::builtin(BuiltinType::BT_F32);
        }
        if tt == TokenType::CharacterLiteral {
            if self.char_literal_cp(lr) > 0xFF { let sp = unsafe (*a).at_const(id).span; self.errors.emit(sp.start, sp.end - sp.start, format("character literal does not fit in 'char' (one byte); use a string or an integer")); }
            return Ast::builtin(BuiltinType::BT_CHAR);
        }
        if tt == TokenType::ByteCharacterLiteral { return Ast::builtin(BuiltinType::BT_U8); }
        if tt == TokenType::True || tt == TokenType::False { return Ast::builtin(BuiltinType::BT_BOOL); }
        if tt == TokenType::StringLiteral || tt == TokenType::RawStringLiteral { return self.prelude_str_type(); }
        if tt == TokenType::ByteStringLiteral { return self.prelude_slice_type(Ast::builtin(BuiltinType::BT_U8), false); }
        return TYPE_NONE;
    }
    ...
    fn check_array_literal(self: &mut Self, id: NodeId) TypeId {
        ...
        if elements.len == 0 { let sp = unsafe (*a).at_const(id).span; self.errors.emit(sp.start, sp.end - sp.start, format("cannot infer the element type of an empty array literal")); return TYPE_NONE; }
        let mut elem = TYPE_NONE;
        for i in 0..elements.len {
            ...
            if el.kind == NodeKind::NODE_FIELD_INITIALIZER {
                ...
                if it != TYPE_NONE {
                    ...
                    if !(iy.kind == TypeKind::TYPE_BUILTIN && (bt_is_int(iy.as_data.builtin) || iy.as_data.builtin == BuiltinType::BT_CHAR)) { let sp = unsafe (*a).at_const(el.as_data.field_initializer.name).span; self.errors.emit(sp.start, sp.end - sp.start, format("array designator index must be an integer")); }
                    else {
                        let ceptr = self.ceval();
                        if ceptr != null && unsafe (*ceptr).eval(self.ast.module, el.as_data.field_initializer.name).kind == ce::CONST_NONE { let sp = unsafe (*a).at_const(el.as_data.field_initializer.name).span; self.errors.emit(sp.start, sp.end - sp.start, format("array designator index must be a constant expression")); }
                    }
                }
                et = self.check_expr(el.as_data.field_initializer.value);
            } else { et = self.check_expr(eid); }
            if i == 0 { elem = et; }
            else if et != elem && et != TYPE_NONE && elem != TYPE_NONE {
                let e0 = self.type_at(elem).kind;
                let ei = self.type_at(et).kind;
                if !(e0 == TypeKind::TYPE_FUNCTION && ei == TypeKind::TYPE_FUNCTION && self.fn_compatible(elem, et)) { self.err_mismatch(eid, elem); elem = TYPE_NONE; }
            }
        }
        if elem != TYPE_NONE { return unsafe (*self.cur_ast()).intern_type(Ty { kind: TypeKind::TYPE_ARRAY, as_data: TyAs { arr: TyArr { elem: elem, len: 0 } } }); }
        return TYPE_NONE;
    }
    ...
    fn check_block_value(self: &mut Self, id: NodeId) TypeId {
        ...
        if stmts.len > 0 {
            ...
            if lv != NODE_NONE && unsafe (*a).at_const(lv).kind != NodeKind::NODE_ASSIGNMENT { return unsafe (*a).type_of(lv); }
            return Ast::builtin(BuiltinType::BT_VOID);
        }
        return Ast::builtin(BuiltinType::BT_VOID);
    }
    ...
    fn check_if_value(self: &mut Self, id: NodeId) TypeId {
        ...
        let bm = self.borrow_mark();
        let c = self.check_expr(ifd.condition);
        if c != TYPE_NONE && !self.is_bool(c) { let sp = unsafe (*a).at_const(ifd.condition).span; let mut ty = Buf96 {}; self.render_type(c, (&mut ty.b[0]) as *mut char, 96); self.errors.emit(sp.start, sp.end - sp.start, format("if condition must be 'bool', found '{}'", diag::cstr((&ty.b[0]) as *const char))); }
        self.borrow_release_to(bm);
        ...
        if then_never || else_never { return if_ty(then_never, else_ty, then_ty); }
        if then_ty != else_ty && then_ty != TYPE_NONE && else_ty != TYPE_NONE { self.err_mismatch(ifd.else_branch, then_ty); return TYPE_NONE; }
        return then_ty;
    }
    ...
    fn check_tuple_value(self: &mut Self, id: NodeId, expected: TypeId) TypeId {
        let a = self.cur_ast();
        let elems = unsafe (*a).at_const(id).as_data.array_literal.elements;
        if elems.len > 4 { let sp = unsafe (*a).at_const(id).span; self.errors.emit(sp.start, sp.end - sp.start, format("tuple arity is limited to 4 elements")); return TYPE_NONE; }
        let mut wargs = Tys4 {};
        let mut wn: i32 = -1;
        ...
        for i in 0..elems.len {
            ...
            targs.t[i as usize] = et;
            if (i as i32) < wn && et != wargs.t[i as usize] && self.compatible(wargs.t[i as usize], eid) { targs.t[i as usize] = wargs.t[i as usize]; unsafe (*self.cur_ast()).set_type(eid, wargs.t[i as usize]); }
    ...
    fn check_tuple_let(self: &mut Self, id: NodeId) void {
        ...
        if !stashed && value != NODE_NONE {
            tn = self.tuple_args_of(self.strip(unsafe (*a).type_of(value)), (&mut targs.t[0]) as *mut TypeId, 4);
            if tn >= 0 { self.tc_mark_move(value); }
        }
        let mut returns = NodeList { start: 0, len: 0 };
        let mut ok = false;
        if value != NODE_NONE && unsafe (*a).at_const(value).kind == NodeKind::NODE_CALL {
            ...
            if callee != TYPE_NONE {
                ...
                if ct.kind == TypeKind::TYPE_FUNCTION {
                    ...
                    returns = if_nl(fnn.kind == NodeKind::NODE_FUNCTION, fnn.as_data.function.returns, fnn.as_data.function_type.returns);
                    ok = true;
                }
            } else {
                ...
                if cn.kind == NodeKind::NODE_MEMBER {
                    ...
                    if md.node != NODE_NONE && md.module == self.ast.module && unsafe (*self.mod_ast(md.module)).at_const(md.node).kind == NodeKind::NODE_FUNCTION { returns = unsafe (*self.mod_ast(md.module)).at_const(md.node).as_data.function.returns; ok = true; }
                }
            }
        }
        if !ok && !stashed && tn < 0 {
            let sp = unsafe (*a).at_const(id).span;
            self.errors.emit(sp.start, sp.end - sp.start, format("a tuple binding requires a multi-value function call or a tuple value"));
            return;
        }
        ...
        if names.len != nret {
            ...
            self.errors.emit(sp.start, sp.end - sp.start, format("expected {} binding{}, the call returns {} value{}", names.len, diag::cstr(s1), nret, diag::cstr(s2)));
        }
        for i in 0..names.len {
            let mut et = TYPE_NONE;
            if stashed && i < (self.mret_n as u32) { et = self.mret_types[i as usize]; }
            else if tn >= 0 { et = if_ty(i < (tn as u32), targs.t[i as usize], TYPE_NONE); }
    ...
    fn check_return(self: &mut Self, id: NodeId) void {
        ...
        for i in 0..values.len {
            ...
            if esc != 0 {
                ...
                self.errors.emit(sp.start, sp.end - sp.start, format("returning a pointer/reference to a {}, which does not outlive the call", diag::cstr(w)));
            }
        }
        ...
        if values.len != expected {
            ...
            let mut s2 = "s".ptr() as *const char;
            if expected == 1 { s2 = "".ptr() as *const char; }
            self.errors.emit(sp.start, sp.end - sp.start, format("expected {} return value{}, found {}", expected, diag::cstr(s2), values.len));
            return;
        }
        ...
        for i in 0..values.len {
            ...
            if !self.compatible(rt, vid) { self.err_mismatch(vid, rt); }
        }
    }
    ...
    fn check_static_assert(self: &mut Self, id: NodeId) void {
        ...
        if c != TYPE_NONE && !self.is_bool(c) {
            let mut ty = Buf96 {};
            self.render_type(c, (&mut ty.b[0]) as *mut char, 96);
            self.errors.emit(sp.start, sp.end - sp.start, format("static_assert condition must be 'bool', found '{}'", diag::cstr((&ty.b[0]) as *const char)));
            return;
        }
        let ceptr = self.ceval();
        if ceptr == null { return; }
        let v = unsafe (*ceptr).eval(self.ast.module, left);
        if v.kind == ce::CONST_BOOL && v.as_data.i == 0 { self.errors.emit(sp.start, sp.end - sp.start, format("static assertion failed")); }
        else if v.kind == ce::CONST_NONE {
            let trap = unsafe (*ceptr).ce_trap_get();
            if trap.len() != 0 { self.errors.emit(sp.start, sp.end - sp.start, format("static assertion cannot be evaluated: {}", trap)); }
        ...
        switch nk {
            ...
            NODE_LET => {
                ...
                if annotated {
                    ...
                    self.errors.emit(sp.start, sp.end - sp.start, format("cannot infer type of '{}'", diag::span_str(self.source, sp.start, sp.end)));
                }
                unsafe (*self.cur_ast()).set_type(id, binding);
                if annotated && !valued {
                    if self.tc_type_is_free(binding) { let sp = self.name_span(nm); self.errors.emit(sp.start, sp.end - sp.start, format("a Free-typed binding must be initialized when declared (it is freed at scope exit)")); }
            ...
            NODE_DEFER => {
                ...
                else { let sp = unsafe (*a).at_const(id).span; self.errors.emit(sp.start, sp.end - sp.start, format("too many pending 'defer' statements in one function (analysis limit)")); }
            },
            ...
            NODE_WHILE => {
                ...
                if c != TYPE_NONE && !self.is_bool(c) {
                    ...
                    let mut ty = Buf96 {};
                    self.render_type(c, (&mut ty.b[0]) as *mut char, 96);
                    self.errors.emit(sp.start, sp.end - sp.start, format("while condition must be 'bool', found '{}'", diag::cstr((&ty.b[0]) as *const char)));
                }
                self.borrow_release_to(bm);
            ...
            NODE_FOR => {
                ...
                if unsafe (*a).at_const(iter).kind == NodeKind::NODE_RANGE {
                    ...
                    if elem == TYPE_NONE && it != TYPE_NONE {
                        ...
                        self.errors.emit(sp.start, sp.end - sp.start, format("cannot iterate over this value (need an array, slice, range, or an Iterator)"));
                    }
                }
            ...
            NODE_BREAK | NODE_CONTINUE => {
                ...
                if le < 0 {
                    let sp = unsafe (*a).at_const(id).span;
                    if lb.end > lb.start { self.errors.emit(sp.start, sp.end - sp.start, format("no enclosing loop is labeled {}", diag::span_str(self.source, lb.start, lb.end))); }
                    else { let mut w = "continue".ptr() as *const char; if nk == NodeKind::NODE_BREAK { w = "break".ptr() as *const char; } self.errors.emit(sp.start, sp.end - sp.start, format("'{}' outside of a loop", diag::cstr(w))); }
                ...
                let sp = unsafe (*self.cur_ast()).at_const(id).span;
                if !self.loop_stack[le as usize].value_loop { self.errors.emit(sp.start, sp.end - sp.start, format("'break' can only carry a value inside a 'loop' expression")); }
    ...
    fn check_pattern(self: &mut Self, id: NodeId, expected: TypeId, bind_ref: i32) void {
        ...
        if nk == NodeKind::NODE_IDENTIFIER {
            ...
            let mut ga = Tys4 {};
            let mut gn: i32 = 0;
            let agok = self.aggregate_of(self.strip(expected), (&mut bmod) as *mut ModuleId, (&mut decl) as *mut NodeId, (&gp.d[0]) as *mut DefId, (&ga.t[0]) as *mut TypeId, (&mut gn) as *mut i32);
            if agok && unsafe (*self.mod_ast(bmod)).at_const(decl).kind == NodeKind::NODE_ENUM {
                ...
                if v != NODE_NONE && unsafe (*self.mod_ast(bmod)).at_const(v).kind == NodeKind::NODE_VARIANT && unsafe (*self.mod_ast(bmod)).at_const(v).as_data.variant.payload.len == 0 {
            ...
            for i in 0..subs.len { self.check_pattern(unsafe ((*self.cur_ast()).list(subs))[i as usize], expected, bind_ref); }
        }
            ...
            let mut ga = Tys4 {};
            let mut gn: i32 = 0;
            let agg = self.aggregate_of(base, (&mut bmod) as *mut ModuleId, (&mut decl) as *mut NodeId, (&gp.d[0]) as *mut DefId, (&ga.t[0]) as *mut TypeId, (&mut gn) as *mut i32);
            let mut variant = NODE_NONE;
            if agg && nameId != NODE_NONE && unsafe (*self.mod_ast(bmod)).at_const(decl).kind == NodeKind::NODE_ENUM {
                let v = self.find_member(bmod, decl, self.name_span(nameId));
                if v != NODE_NONE && unsafe (*self.mod_ast(bmod)).at_const(v).kind == NodeKind::NODE_VARIANT { variant = v; unsafe (*self.cur_ast()).set_resolution_def(nameId, DefId { module: bmod, node: variant }); }
            }
            ...
            if variant != NODE_NONE {
                ...
                for i in 0..children.len {
                    ...
                    for j in 0..vpl.len {
                        let pfid = unsafe ((*va).list(vpl))[j as usize];
                        let pf = unsafe (*va).at_const(pfid);
                        if pf.kind == NodeKind::NODE_FIELD && spans_eq2(self.source, fn2, self.mod_src(bmod), unsafe (*va).at_const(pf.as_data.field.name).as_data.name.text) { matchId = pfid; ft = self.subst_type(self.lower_type_in(bmod, pf.as_data.field.ty), (&gp.d[0]) as *const DefId, (&ga.t[0]) as *const TypeId, gn); break; }
                    }
                    if matchId != NODE_NONE { unsafe (*self.cur_ast()).set_resolution_def(fldName, DefId { module: bmod, node: matchId }); }
                    else { let mut ty = Buf96 {}; self.render_type(base, (&mut ty.b[0]) as *mut char, 96); self.errors.emit(fn2.start, fn2.end - fn2.start, format("no field '{}' on '{}'", diag::span_str(self.source, fn2.start, fn2.end), diag::cstr((&ty.b[0]) as *const char))); }
                    let fc = unsafe (*self.cur_ast()).at_const(fid).as_data.pattern.children;
                    for k in 0..fc.len { self.check_pattern(unsafe ((*self.cur_ast()).list(fc))[k as usize], ft, bind_ref); }
                }
            } else {
                if agg && nameId != NODE_NONE { unsafe (*self.cur_ast()).set_resolution_def(nameId, DefId { module: bmod, node: decl }); }
                for i in 0..children.len { self.check_pattern(unsafe ((*self.cur_ast()).list(children))[i as usize], base, bind_ref); }
            }
        }
            ...
            if agg && nameId != NODE_NONE {
                ...
                else { let mut ty = Buf96 {}; self.render_type(base, (&mut ty.b[0]) as *mut char, 96); self.errors.emit(fname.start, fname.end - fname.start, format("no field '{}' on '{}'", diag::span_str(self.source, fname.start, fname.end), diag::cstr((&ty.b[0]) as *const char))); }
            }
            let children = unsafe (*self.cur_ast()).at_const(id).as_data.pattern.children;
            for i in 0..children.len { self.check_pattern(unsafe ((*self.cur_ast()).list(children))[i as usize], field_type, bind_ref); }
        }
            ...
            let mut ga = Tys4 {};
            let mut gn: i32 = 0;
            let agok = self.aggregate_of(base, (&mut bmod) as *mut ModuleId, (&mut decl0) as *mut NodeId, (&gp.d[0]) as *mut DefId, (&ga.t[0]) as *mut TypeId, (&mut gn) as *mut i32);
            ...
            if nameId != NODE_NONE {
                ...
                if decl == NODE_NONE { let mut ty = Buf96 {}; self.render_type(base, (&mut ty.b[0]) as *mut char, 96); self.errors.emit(vname.start, vname.end - vname.start, format("pattern '{}(..)' expects an enum, but the value has type '{}'", diag::span_str(self.source, vname.start, vname.end), diag::cstr((&ty.b[0]) as *const char))); }
                else {
                    ...
                    else { let mut ty = Buf96 {}; self.render_type(base, (&mut ty.b[0]) as *mut char, 96); self.errors.emit(vname.start, vname.end - vname.start, format("no variant '{}' on '{}'", diag::span_str(self.source, vname.start, vname.end), diag::cstr((&ty.b[0]) as *const char))); }
                }
            }
            let children = unsafe (*self.cur_ast()).at_const(id).as_data.pattern.children;
            let mut payload = NodeList { start: 0, len: 0 };
            let mut va: *mut Ast = a;
            if variant != NODE_NONE { va = self.mod_ast(bmod); payload = unsafe (*va).at_const(variant).as_data.variant.payload; }
            for i in 0..children.len {
                let mut pt = TYPE_NONE;
                if i < payload.len {
                    let plid = unsafe ((*va).list(payload))[i as usize];
                    let pe = unsafe (*va).at_const(plid);
                    pt = self.subst_type(self.lower_type_in(bmod, if_node(pe.kind == NodeKind::NODE_FIELD, pe.as_data.field.ty, plid)), (&gp.d[0]) as *const DefId, (&ga.t[0]) as *const TypeId, gn);
                }
            ...
            for i in 0..children.len { self.check_pattern(unsafe ((*self.cur_ast()).list(children))[i as usize], expected, bind_ref); }
        }
    }
    ...
    fn tc_embeds_by_value(self: &mut Self, m: ModuleId, d: NodeId, tm: ModuleId, td: NodeId, depth: i32) bool {
        ...
        let a = self.mod_ast(m);
        let dn = *unsafe (*a).at_const(d);
        if (dn.kind != NodeKind::NODE_STRUCT && dn.kind != NodeKind::NODE_ENUM) || dn.as_data.aggregate.generics.len != 0 { return false; }
        ...
        for i in 0..members.len {
            ...
            let mut ntn: u32 = 0;
            if dn.as_data.aggregate.is_tuple { tns.n[0] = mid; ntn = 1; }
            else if mn.kind == NodeKind::NODE_FIELD { tns.n[0] = mn.as_data.field.ty; ntn = 1; }
                ...
                while j < pl.len && ntn < 16 {
                    ...
                    tns.n[ntn as usize] = if_node(pe.kind == NodeKind::NODE_FIELD, pe.as_data.field.ty, plid);
                    ntn = ntn + 1;
                    j = j + 1;
                }
            }
            for j in 0..ntn {
                let mut ft = self.lower_type_in(m, tns.n[j as usize]);
                let mut y = *self.type_at(ft);
                while y.kind == TypeKind::TYPE_ARRAY { ft = y.as_data.elem; y = *self.type_at(ft); }
                if y.kind == TypeKind::TYPE_STRUCT || y.kind == TypeKind::TYPE_ENUM {
                    if (y.module == tm && y.as_data.decl == td) || self.tc_embeds_by_value(y.module, y.as_data.decl, tm, td, depth + 1) { return true; }
                }
            }
        }
        return false;
    ...
    fn check_item(self: &mut Self, id: NodeId) void {
        ...
        switch nk {
            ...
            NODE_FUNCTION => {
                ...
                if fnd.is_variadic && !fnd.is_extern && params.len == 0 { let sp = self.name_span(fnd.name); self.errors.emit(sp.start, sp.end - sp.start, format("a variadic function needs at least one fixed parameter before '...'")); }
                ...
                self.current_fn = id;
                for mi in 0..self.nmoved { self.ms_bit_clear(self.moved[mi as usize]); }
            ...
            NODE_STRUCT | NODE_ENUM => {
                    ...
                    for i in 0..members.len {
                        ...
                        if mn.kind == NodeKind::NODE_FIELD { self.resolve_type(mn.as_data.field.ty); }
                            ...
                            if mn.as_data.variant.value != NODE_NONE {
                                ...
                                if vt != TYPE_NONE && !self.is_int(vt) { let sp = unsafe (*self.cur_ast()).at_const(mn.as_data.variant.value).span; self.errors.emit(sp.start, sp.end - sp.start, format("enum discriminant must be an integer")); }
                            }
                ...
                if agg.generics.len == 0 && self.tc_embeds_by_value(self.ast.module, id, self.ast.module, id, 0) {
                    let sp = unsafe (*self.cur_ast()).at_const(id).span;
                    self.errors.emit(sp.start, sp.end - sp.start, format("this type embeds itself by value, so it would have infinite size"));
            ...
            NODE_CONST => {
                ...
                if cd.is_static_mut && self.tc_type_is_free(declared) {
                    let sp = unsafe (*self.cur_ast()).at_const(id).span;
                    self.errors.emit(sp.start, sp.end - sp.start, format("a 'static mut' cannot hold an owning (Free) type"));
    ...
    fn close_instances(self: &mut Self) void {
        ...
        while ii < unsafe (*self.cur_ast()).instances.len() {
            ...
            for k in 0..it.n { if !unsafe (*self.cur_ast()).type_concrete(it.args[k as usize]) { concrete = false; } }
            ...
            for i in 0..ne {
                ...
                if itn.as_data.extend_def.generics.len != 0 && unsafe (*ma).resolution(itn.as_data.extend_def.target_type) == it.decl {
                    let gens = itn.as_data.extend_def.generics;
                    let mut ip = Defs4 {};
                    let mut ia = Tys4 {};
                    let mut ipn: i32 = 0;
                    let mut g: u32 = 0;
                    while g < gens.len && (g as u8) < it.n && ipn < 4 {
                        ip.d[ipn as usize] = DefId { module: it.module, node: unsafe ((*ma).list(gens))[g as usize] };
                        ia.t[ipn as usize] = it.args[g as usize];
                        ipn = ipn + 1;
                        g = g + 1;
                    }
                    ...
                    for j in 0..ms.len {
                        ...
                        if mn.kind == NodeKind::NODE_FUNCTION && mn.as_data.function.generics.len == 0 {
                            ...
                            for p in 0..ps.len {
                                ...
                                self.subst_type(self.lower_type_in(it.module, unsafe (*ma).at_const(pid).as_data.parameter.ty), (&ip.d[0]) as *const DefId, (&ia.t[0]) as *const TypeId, ipn);
                            }
                            let rs = mn.as_data.function.returns;
                            if rs.len == 1 {
                                let r0 = unsafe ((*ma).list(rs))[0];
                                let rn = unsafe (*ma).at_const(r0);
                                self.subst_type(self.lower_type_in(it.module, if_node(rn.kind == NodeKind::NODE_PARAMETER, rn.as_data.parameter.ty, r0)), (&ip.d[0]) as *const DefId, (&ia.t[0]) as *const TypeId, ipn);
                            }
                        }
                    }
                }
