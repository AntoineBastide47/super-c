...
// Every .spc under bench/, as `import bench::<path>;` lines. Returns how many were found. This is the same
// convention `super-c test` uses for tests/: a file is part of the suite because of where it lives.
fn bench_import_root(out: &mut String) usize {
    let mut rels = Vector::<String>::new();
    walk_files("bench", 5, &mut rels);
    rels.sort_by(name_cmp);
...
fn bench_run_root(found: &Vector<String>, out: &mut String) {
    out.push_str("// generated by `super-c bench` -- do not edit\n");
    ...
    for i in 0..found.len() {
        ...
        // The `bench::` every one of these starts with says nothing: strip it from the reported name.
        out.push_str(
            if full.starts_with("bench::") {
                full.slice(7, full.len());
            } else {
                full;
            },
        );
...
/// `super-c bench`: discover `@bench` functions under bench/, build a runner for them under the bench
/// profile (by default) into
...
pub fn manifest_bench(
    ...
    if unsafe shim::sc_stat_isdir("bench".ptr() as *const char) != 1 {
        eprintln("bench: no bench/ directory next to src/");
        return 1;
    }
    ...
    let nmods = bench_import_root(&mut listing);
    if nmods == 0 {
        eprintln("bench: no .spc files under bench/");
        return 1;
    }
    mkdirs(m.out_dir.as_str());
    ...
    if !bench_collect(scanp.as_str(), dirname_of(m.root.as_str()), std_dir, bootstrap_tags, target, &mut found) {
        return 1;
    }
    if found.len() == 0 {
